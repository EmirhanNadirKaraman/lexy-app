...
- **Record a change note as ONE NEW LINE. Never grow an existing line.** This is the shape both trackers above are written in, and it is a merge requirement, not a style preference. `.gitattributes` gives `docs/SUMMARY.md` and `docs/TESTS.md` `merge=union` so two parallel branches each recording a note both land instead of stopping the merge sweep — but union resolves per LINE. Two branches that append their sentences to the SAME line duplicate that whole line rather than merging the two additions, and that is exactly what the old habit produced: a 19,410-character row in `SUMMARY.md`, a 15,729-character row in `TESTS.md`, and a merge sweep that halted three times in one evening (2026-08-18), leaving five reviewed tasks unmerged for a day. So:
  - Adding a **new table row** for a file or a test you touched — the normal case — is a new line and is fine.
  - Anything else goes at the very end of the file, in the **"Change notes"** section each tracker now ends with: one line, `| date | task-id | note |`, appended after the last line.
  ...
  - Do not edit or reorder a line someone else wrote. Under `merge=union` a concurrent edit to the same line silently duplicates it; nothing warns you.
  - After a union merge the notes are in arrival order, not date order. Read the date column, don't infer from position.
  - The same rules apply to the other trackers (`CLAUDE.md`, `docs/SECURITY.md`, `docs/COMMON_ERRORS.md`, `docs/SCHEMA.md`) as good practice, but they are **not** union-merged — a conflict there still stops the sweep, deliberately, because they carry claims that need a human when two branches disagree.
  - Pinned by `autoloop/tests/test_docs_merge.py`, which merges real branches and demonstrates the duplication above rather than asserting it.
