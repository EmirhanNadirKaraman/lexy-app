...
def test_no_scheduling_advice_moved_into_the_context_block(window_config):
    ...
    preference lives in CONTRACT_INSTRUCTIONS, which is pinned at 5,300
    characters (`test_contract.test_contract_stays_within_its_budget`, which
    carries the accounting for every move of that number — most recently
    split-03's tenth decision); these rows state what is true and never restate
    what to do about it, and the contract gains no text from them."""
    store = detailed_store(
