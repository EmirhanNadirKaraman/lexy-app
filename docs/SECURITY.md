...
- **An autonomous set-aside can only quarantine the round in flight, never a task a park site merely names (added 2026-08-26, `setaside-01`).** The quarantine in the bullet above is the loop taking an availability decision about a unit of work without an operator, so WHICH unit it picks is the whole of its blast radius. `orchestrator._autonomous_set_aside_task` returned any explicit `task_id` a park site passed, unconditionally, before it ever consulted `state.task_execution` — so a fault raised during t1's round could set aside an eligible registry task **t2**, which `cli._handle_parked_task` then marks `blocked_by_operator` while t1 carries on. `publisher_url_drift` is the live site with that shape (`RECOVER_UNAVAILABLE`, so it reaches the set-aside on its first occurrence, and it names `binding.task_id` — the identity captured when the packet was SENT, which an approval naming an older packet resolves for a task that is not the one executing). Three properties bound it now. (1) **The explicit id is VALIDATED, not trusted and not dropped.** It is honoured only when it matches `_round_task_ids()` — `state.task_execution`, then `state.current_task` — compared exactly after `.strip()` and never folded; the id that reaches `park_task_id` and the blocker record is read off the round's own record, so a caller-supplied string never passes through. The parameter stays because its rationale is real: a park site is the only thing that can know a fault belongs to a task other than the executing one. (2) **A mismatch preserves the SITE's own terminal.** The recovery plan is dropped, so a `loop_fatal` site still ends the loop instead of being converted into a `task_fatal` about a bystander — availability is never bought by blocking work that did nothing wrong, and every path here can only ever quarantine fewer tasks than before, never more. (3) **The refusal is loud** — `autonomous_set_aside_refused` carries the named id and the round's own ids, so the guard leaves evidence rather than looking like it never fires. Nothing else moved: the allowlist, the two hard-halt locks, `publisher_url_drift`'s zero budget and `BlockerStore.close_recovered`'s inability to forge an answer are all untouched. **Verification:** `pytest autoloop/tests/test_autonomous_recovery.py -k "round_in_flight or bystander or quarantine or set_aside"`, and `rg -n '_autonomous_set_aside_task' autoloop/orchestrator.py` must still show ONE definition and ONE call site, inside `_to_needs_user`. **Do not** restore an unconditional `if task_id: return task_id`, do not fall back to the site's id when `state.task_execution` is missing or unreadable (an unreadable round is "no round", never agreement), and do not widen the no-task-named fallback to `current_task` — that would quarantine tasks in states the loop parks `loop_fatal` for today.
