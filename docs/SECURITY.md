...
- **A restored review binding is evidence the loop itself recorded, and it can only ever name a candidate the loop actually presented (added 2026-08-21, `bind-01`).** Two new routes now supply the `PostcommitBinding` a `push` publishes from — a corrective re-prompt inheriting the binding of the request it corrects (`orchestrator._carry_postcommit_binding`, `state.LoopState.outbox_postcommit`), and a stamped approval reconciled against the request its `reviewed.request_id` names (`orchestrator._resolve_reviewed_packet`, `state.LoopState.postcommit_packets`). Four properties keep that from widening what a `push` may publish. (1) **Neither route can mint a binding.** Both return a `PostcommitBinding` this loop built in `_current_pending_postcommit` and actually sent; nothing is derived from the directive (a `push` still cannot carry a task_id — `contract._forbid`), and nothing is re-derived from a fresh "latest state" lookup. (2) **Every push-time check is unchanged and runs on either route.** `_dispatch_task_push` still refuses unless the live `TaskExecutionStore` record holds the SAME `candidate_sha`, the candidate is still on the task line, still resolves, and its tree still equals `candidate_tree_sha` — so a stamp reaching back to a SUPERSEDED packet is refused `push_candidate_stale` rather than published. (3) **`verify_review` is not relaxed.** When the stamp names an earlier request the expected `request_id`/`head_sha`/`report_sha256` come from THAT request's ledger entry — a real packet, digest for digest — and a stamp naming a request with no entry has no expected values to fall back on and is refused `review_mismatch:request_id` exactly as before. The HEAD-moved staleness check is deliberately NOT given §4f's changeset exemption. (4) **The unbound case still refuses.** A `push` resolving to no binding publishes nothing (`push_missing_review_binding`), and `commit`/`commit_and_push` remain retired (S21). **Verification:** `pytest autoloop/tests/test_review_binding_carry.py autoloop/tests/test_postcommit_review.py -k "binding or unbound or stale or legacy"`, and `rg -n '_resolve_postcommit_binding\|_carry_postcommit_binding' autoloop/` should show the definitions, their orchestrator call sites and tests, and no other writer of `outbox_postcommit`/`postcommit_packets`. **Do not** let either route fall back to a fresh `TaskExecutionStore` lookup, do not skip the HEAD-moved check for a ledger-resolved push, and do not add a third source of a binding without giving it the same push-time re-verification.
...
> **Amendment 2026-08-21 (bind-01) — the retirement still holds; only the
> refusal MESSAGE for an unbound `push` changed.** A corrective re-prompt now
> inherits the postcommit binding of the request it corrects, and a stamped
> `push` can be reconciled against the request its `reviewed` stamp NAMES
> rather than only against the most recent response (`docs/AUTOLOOP.md`
> §4b-bis). Neither invents a binding: both can only produce one this loop
> itself built and sent, `_dispatch_task_push`'s candidate/ancestry/tree checks
> run unchanged on either, and a `push` that resolves to no binding is still
> refused with nothing published. `_dispatch`'s branch for `commit` /
> `commit_and_push` — the actually retired decisions — is untouched and still
> denies `legacy_git_path_retired`; the unbound-`push` case moved to
> `push_missing_review_binding`, whose message names an actionable recovery
> instead of dead-ending. **Verification check:** `rg -n
> 'legacy_git_path_retired' autoloop/orchestrator.py` must show it reached
> only from the `COMMIT_DECISIONS` branch, and `pytest
