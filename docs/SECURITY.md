...
enforcement ADDED is a refusal: `_dispatch_executor` denies an
`implement`/`revise` of a different task while a pin is live, through the
existing budget-capped policy-denial re-prompt.
...
`_dispatch_executor`'s `urgent_target_pending` denial).
