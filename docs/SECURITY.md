...
| 2026-08-24 | split-02 | No new finding, and of §14's triggers only one is adjacent: a new file the loop writes under `state_dir` whose CONTENT is authored from a reviewer directive. Recorded because the reviewer gained a second registry-mutating capability — a `plan` carrying `split_of` retires a task, creates its successors, archives its execution record and quarantines its worker, with no operator step. It is `recut`'s shape one verb over and is bounded the same way. |
| 2026-08-24 | split-02 | Six bounds, none of them trust: only a `pending`/`in_progress` parent (never completed, retired, shipped-elsewhere, or an operator hold with an open blocker), never a published candidate, never one whose verdict is outstanding, never an audit unit, never over a successor id that already exists, and nothing is deleted — `retire_execution` MOVES both halves and the registry keeps the row with its supersession chain. Every fire is a `task_split` transcript entry carrying the reviewer's reason. |
| 2026-08-24 | split-02 | The reviewer-authored parts are bounded as INPUT because they land in durable files. The retirement reason is whitespace-folded and truncated at `MAX_SPLIT_REASON_CHARS` before it reaches `tasks.json`; successor ids, titles, descriptions and `approved_paths` go through `TaskRegistry.add_many`'s existing validators, so a split cannot mint a scope an ordinary `plan` could not. The intent file itself is written only by the loop and carries no path from the directive. |
| 2026-08-24 | split-02 | One value in `<state_dir>/split-intents/<parent>.json` becomes a path COMPONENT twice — `executions/archive/<task>-<label>.json` and `quarantine/<task>-<label>` — so `SplitIntentStore.load` refuses any label outside `worktask.LABEL_RE` before `retire_execution` can see it. Refused at LOAD rather than at use: a raise inside the retirement would land after the archive had already moved. Filename and stored `parent_id` must also agree. |
| 2026-08-24 | split-02 | The availability direction is the one to get wrong here, and it fails CLOSED deliberately: an intent that cannot be read or cannot be completed PARKS the loop `loop_fatal` naming the file, rather than being skipped or cleared. Skipping would leave the loop running with a registry, an execution record and a worker repository that may already disagree, with nothing saying so — which is exactly the state this record exists to make impossible. |
| 2026-08-24 | split-02 | Nothing was weakened. `TaskRegistry.retire` is unchanged and still written-once with no reverse and no strand; `retire_execution`'s new `label=` defaults to empty, which derives the label exactly as before, so every existing caller is byte-identical; `_forget_task_pointers` is `_dispatch_recut`'s own four statements extracted, not relaxed. No agent capability was added — the split is performed by the loop, and no tool list changed. |
| 2026-08-24 | split-02 | Revision round, and it tightens a real exposure rather than adding a finding. Split acceptance now verifies each successor's `approved_paths` — the task's WRITE AUTHORIZATION — and its `depends_on` against the durable intent, on the replay AND on the final read-back. Before this, an operator editing `tasks.json` between a crash and the restart could widen a successor's scope; the replay compared title/description only, the read-back checked mere presence, so the widened task was adopted, certified and the intent deleted over it. |
| 2026-08-24 | split-02 | It now parks `loop_fatal` and KEEPS the intent, which still carries the scope that was approved. Two deliberate limits: it refuses rather than repairing (rewriting a task the operator edited is a bigger overreach than refusing to finish), and `priority`/`status`/timestamps are NOT compared — none is in the intent and `TaskStore.save` intentionally adopts on-disk priority, so comparing them would turn a fail-closed check into one that parks a split forever over a supported edit. |
| 2026-08-24 | split-02 | Amending the two lines above: `status` IS compared now, and the exposure it closes is a superseded parent rather than a widened scope. A successor left with the DEFAULT empty `depends_on` reached neither the rewrite substitution nor `retire`'s live filter, so hand-editing it to `retired` between a crash and the restart matched every compared field — the replay adopted it, the read-back certified it, and the intent was cleared with the parent superseded by a row that satisfies no dependency and has no un-retire. |
| 2026-08-24 | split-02 | Fail-closed on unrecognised input too, which is the reason it is an ALLOW-LIST (`pending`/`in_progress`/`blocked`) and not a test against `tasks._TERMINAL_STATUSES`. `TaskRegistry.from_dict` does not validate `status` at all, so a typo'd `Retired` in a hand-edited `tasks.json` is not terminal; a deny-list would read it as live and adopt the row. Verification check: `rg -n '_ADOPTABLE_SUCCESSOR_STATUSES' autoloop/orchestrator.py` must show the frozenset and its use as the FIRST test in `_successor_disagreement`. |
| 2026-08-24 | split-02 | Third revision round, availability again and fail-closed again. A registry recording a split whose execution record or worker repo never moved — reachable by DELETING the intent, the remedy the park message itself offers — used to be reported to the reviewer as an applied split, over a record that holds the merge window shut and that nothing revisits. `_split_residue` now asks both artefact stores (unreadable counts as surviving) and parks `loop_fatal` naming each leftover. Nothing was weakened: no new capability, no widened scope, no store written. |
| 2026-08-24 | split-02 | Fourth revision round, availability and fail-closed again; no new finding. `SPLIT ALREADY APPLIED` is a claim about three stores, and a loop whose execution store or worker-repository manager is `None` — or whose `path_for` raises — used to answer "no residue" and emit that claim from the registry alone. Those are now UNKNOWN residue and park `loop_fatal` under their own code `split_residue_uninspectable`. Verification check: `rg -n 'def _split_worker_residue' -A 25 autoloop/orchestrator.py` must show a `None` manager and a raising `path_for` each returning an unknown-residue sentence rather than an empty one. |
| 2026-08-24 | split-02 | Fifth round, one new exposure needing no crash: the intent filename is `<state_dir>/split-intents/<parent_id>.json`, and `TaskRegistry.from_dict` does not re-validate a stored id, so a hand-edited `tasks.json` holding `../escapee` and named in `split_of` passed every refusal and would have written the record outside the directory `parent_ids` lists — then retired the parent. `SplitIntentStore.save` now refuses a parent id whose file would not land directly in that directory; the dispatch parks `split_intent_unwritable`, nothing moved. Check: `rg -n 'path.parent != self.directory' autoloop/worktask.py`. |
