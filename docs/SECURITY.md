...
| S41 | MEDIUM | The escape detector watches a LOOP-OWNED clone since esc-02 (2026-08-26), not the checkout the operator also works in — two loop-fatal parks that day were the operator's own `ruff` cache and their own Claude Code rules file, neither an isolation failure. Scope was NOT narrowed (tracked + untracked + ignored still, since `.gitignore` is agent-writable); the observed TREE moved. Residual: a write into the operator's checkout is now unobserved, bounded by the worker's fetch source now naming the clone. Every synchronisation failure refuses and parks rather than observing a stale tree | `autoloop/worker_env.py`, `autoloop/orchestrator.py`, `autoloop/config.py`, `autoloop/cli.py` |
...
### S41 — The escape detector watches a loop-owned clone, so a write into the OPERATOR's checkout is no longer observed — MEDIUM — OPEN (accepted residual, 2026-08-26, esc-02)

**Where:** `autoloop/worker_env.py` (`ObservedCheckout`, `validate_observed_checkout`), `autoloop/orchestrator.py` (`_observation_git`, `_synchronise_observed_checkout`, `_worker_fetch_root`, `_execute_with_escape_detection`), `autoloop/config.py` (`resolve_observed_checkout`), `autoloop/cli.py`.

**Severity:** MEDIUM — a narrowing of what is observed, taken deliberately, in exchange for removing a whole class of false loop-fatal parks. Read alongside S24, which still owns the "detection, not prevention" bound this does not change.

**What changed.** Before esc-02 the detector snapshotted the PRIMARY checkout — the same working tree the operator edits in, runs `ruff` in, and points their own Claude Code sessions at. It reported those writes truthfully and could not attribute them, so two loop-fatal `checkout_escape_detected` parks on 2026-08-26 were caused by the operator: 14 files under `.ruff_cache/0.14.1/` (ignored only because ruff writes a `.gitignore` containing `*` into its own cache), and an untracked `.claude/rules/evidence-first.md`. Neither was an isolation failure. Since esc-02 the detector snapshots a dedicated LOOP-OWNED clone instead, synchronised from the primary checkout at one controlled boundary per round and written to by nothing else.

**What was NOT narrowed, deliberately.** `enumerate_checkout_paths` still unions tracked + untracked + ignored. `.gitignore` is writable by the very agent being watched, so an ignored-path exemption would sell unlimited invisible writes for one line of config — that is why the tree moved and the scope did not. The `__pycache__` exemption (S24's 2026-08-16 addendum) is untouched and still narrow.

**The residual, stated plainly.** A write into the operator's checkout is now invisible to this mechanism. It is bounded by where an escaping agent can land: a worker's cwd is its own repository, and the one absolute path to a non-worker tree that a worker carries on disk is the fetch source recorded in `.git/FETCH_HEAD`, which esc-02 repointed at the clone (`_worker_fetch_root`). An agent that goes looking for "the repo" therefore finds a watched tree. An agent that already knows the operator's absolute path from elsewhere does not — and never did in any way this mechanism could have proven, because a write there was indistinguishable from the operator's own.

**BOTH fetch paths, not just worker creation.** A worker repository is written a fetch source twice, and the sentence above is only true if both name the clone: once when it is created (`WorkerRepoManager.create`), and once when a REVIEWED candidate is carried past a moved head and the new head is fetched INTO the existing worker (`_carry_reviewed_candidate_past` → `merge_foreign_commit`). The second was still spelled `self._git.repo_root` when this finding was first written; it now goes through `_worker_fetch_root()` too, and the caller re-synchronises to exactly that head first so the commit is present AND pinned as a ref (git's `upload-pack` refuses an unadvertised sha). A clone that cannot be brought to it parks `observed_checkout_unusable` rather than falling back to the operator's tree.

**One deployment shape this does not cover, and nothing ships it.** The synchronisation boundary is keyed on the same field as the snapshot (`self._observed`) and NOT on the worker-repo mechanism, so a clone is always brought to the round's commit before it is observed. But an Orchestrator built with an observed clone AND the legacy linked-`worktrees` mechanism would watch the clone while its agent worked in a worktree that shares `.git` with the primary checkout — a tree that is then unobserved. `cli._build_orchestrator` always wires `WorkerRepoManager` (`cli.py:594`, unconditional) and never passes `worktrees`, so no shipped deployment builds that pair; it is recorded here rather than guarded because the guard would be a refusal for a combination nothing constructs.

**Synchronisation fails safe, every branch.** Missing → created; behind → fetched and checked out, with `HEAD` READ BACK and compared rather than predicted; ahead or diverged → refused, nothing reset; dirty (including ignored residue, which `git status` cannot see) → refused BEFORE anything is fetched, so an escape's residue is never checked out over; present but not a repository → refused, nothing deleted; any git failure or unreadable status → refused. Each refusal parks `loop_fatal` under `observed_checkout_unusable` and the write-capable agent never starts, because starting it would mean running with no escape detection at all.

**Verification check:** `rg -n "_observation_git|_worker_fetch_root" autoloop/orchestrator.py` must show the snapshots, the clean-baseline gate, every `WorkerRepoManager.create` fetch source AND the `merge_foreign_commit` source in `_carry_reviewed_candidate_past` going through them — a `self._git` reappearing at any of those sites is this finding's regression. `rg -n "list_ignored_paths" autoloop/escape_detector.py` must still show ignored paths enumerated.

**Suggested fix if the residual ever matters:** do not widen the snapshot back onto a shared tree — that is the defect, not the remedy. The next real step is the OS-level sandbox S24 already tracks, which bounds where a write can land instead of reporting where one did.

**Known gap in this pass:** `doctor.py` does not run `validate_observed_checkout`, so an unsafe `[paths].observed_checkout` is refused only when a real run builds its orchestrator, not when an operator asks `doctor` whether the deployment is sound. `autoloop/doctor.py` was outside esc-02's approved scope.
...
- **Autonomous recovery can neither publish, re-provision, nor step past a hard halt (added 2026-08-25, `halt-02`).** `[autonomy] enabled` (default **false**) lets the loop retry a transport/environment fault and then quarantine the one task at fault instead of stopping — i.e. it lets the loop act where it used to wait for a human, which is exactly the shape that needs bounding. Six properties do that. (1) **It is an ALLOWLIST, plus a redundant refusal.** Only the codes in `blockers.AUTONOMOUS_RECOVERIES` are automated, and `blockers.autonomous_recovery` refuses every member of `blockers.HARD_HALT_CODES` — `checkout_escape_detected`, `worker_isolation_violation`, `primary_checkout_dirty`, `approved_path_symlink_traversal`, `prompt_integrity_mismatch` — before it consults the table. A denylist was rejected: it would automate every future park nobody had reasoned about. (2) **It performs no new action.** The only two moves are re-entering the park's own `resume_phase` (what `run --retry` does) and re-issuing the same request id (what `run --resubmit` does). It runs no git command, spawns no process, writes no config, and touches no whitelist. (3) **`publisher_url_drift` carries a budget of ZERO, deliberately.** Its only recovery is `reprovision-publisher --confirm`, an operator confirming that a NEW push destination is correct; a loop able to take that could redirect its own publication. It is set aside without a retry and nothing is pushed — the S21-era rule that a push destination changes only by operator confirmation is untouched. (4) **It cannot forge an operator answer.** A recovered fault's record is closed by `BlockerStore.close_recovered`, which writes `archived_reason` and leaves `answer` `None`, the same discipline `archive_stale` follows so that `cli._RESOLUTION_PRECONDITIONS` cannot be laundered. (5) **Every gate fails closed** — no `BlockerStore`, no resolvable task, an unknown code, a missing/terminal `resume_phase`, a re-issue with no request, a malformed `[autonomy]` section (refused at load, never coerced: `enabled = "true"` is a **`ConfigError`**, because a truthy string must not read as consent) — each falls through to the park the loop performs today, and a corrupt blocker record RAISES rather than reading as an unspent budget. (6) **Nothing becomes invisible.** A `blockers.Blocker` is still written for every one of these faults, so `blockers`/`answer`/`health`/the heartbeat behave exactly as before; the transcript adds `autonomous_recovery` per retry. **Verification:** `pytest autoloop/tests/test_autonomous_recovery.py` (the whole file) — includes the disjointness of the two sets, the parametrized "every hard halt still parks `loop_fatal` with the flag on", the zero budget on `publisher_url_drift`, the malformed-config refusals, and the corrupt-record raise; `rg -n 'HARD_HALT_CODES|autonomous_recovery' autoloop/` should show the table in `blockers.py`, the single interception in `orchestrator._to_needs_user`, and tests — nothing in `cli.py`, `policy.py` or `tasks.py`. **Do not** add a hard-halt code to the table, do not give `publisher_url_drift` a non-zero budget, do not make the flag's gate a truthiness test, and do not add a third action to the two above.
- **An autonomous set-aside can only quarantine the round in flight, never a task a park site merely names (added 2026-08-26, `setaside-01`).** The quarantine in the bullet above is the loop taking an availability decision about a unit of work without an operator, so WHICH unit it picks is the whole of its blast radius. `orchestrator._autonomous_set_aside_task` returned any explicit `task_id` a park site passed, unconditionally, before it ever consulted `state.task_execution` — so a fault raised during t1's round could set aside an eligible registry task **t2**, which `cli._handle_parked_task` then marks `blocked_by_operator` while t1 carries on. `publisher_url_drift` is the live site with that shape (`RECOVER_UNAVAILABLE`, so it reaches the set-aside on its first occurrence, and it names `binding.task_id` — the identity captured when the packet was SENT, which an approval naming an older packet resolves for a task that is not the one executing). Four properties bound it now. (1) **The explicit id is VALIDATED, not trusted and not dropped.** It is honoured only when it equals `_active_task_id()`, compared exactly after `.strip()` and never folded; the id that reaches `park_task_id` and the blocker record is read off the round's own record, so a caller-supplied string never passes through. The parameter stays because its rationale is real: a park site is the only thing that can know a fault belongs to a task other than the executing one. (2) **There is ONE active identity, and a session that cannot name it quarantines nothing.** `_active_task_id()` reads `state.task_execution` and `state.current_task` as a single authority: the two agreeing — or exactly one of them naming anything, which is the state the pre-execution `task_base_behind_head` park is reached in — resolves the identity, and the two naming DIFFERENT tasks (a stale or transitional session: t1's reviewed candidate still mirrored awaiting push while t2 is dispatched) resolves nothing at all, so both the explicit id and the no-task-named fallback are refused. Reading them as two independent authorities, first match wins, was the first cut of this fix and left the same hazard one level down — an id matching either record was honoured, so t1 could still be quarantined for a round that was not its own. The no-task-named fallback is still ANCHORED on `state.task_execution`; the active identity can only veto it, never supply one, because sourcing a victim from `current_task` would quarantine tasks in states the loop parks `loop_fatal` for today. (3) **A mismatch preserves the SITE's own terminal.** The recovery plan is dropped, so a `loop_fatal` site still ends the loop instead of being converted into a `task_fatal` about a bystander — availability is never bought by blocking work that did nothing wrong, and every path here can only ever quarantine fewer tasks than before, never more. (4) **The refusal is loud and says which refusal fired** — `autonomous_set_aside_refused` carries the named id, the resolved active id and both records raw, under `named_task_is_not_the_active_task` or `round_identity_records_disagree`, so the guard leaves evidence rather than looking like it never fires. Nothing else moved: the allowlist, the two hard-halt locks, `publisher_url_drift`'s zero budget and `BlockerStore.close_recovered`'s inability to forge an answer are all untouched. **Verification:** `pytest autoloop/tests/test_autonomous_recovery.py -k "active or bystander or quarantine or set_aside or disagree"`, and `rg -n '_autonomous_set_aside_task' autoloop/orchestrator.py` must still show ONE definition and ONE call site, inside `_to_needs_user`. **Do not** restore an unconditional `if task_id: return task_id`, do not fall back to the site's id when `state.task_execution` is missing or unreadable (an unreadable round is "no round", never agreement), do not resolve a disagreement by preferring one record over the other, and do not widen the no-task-named fallback to `current_task`.
...
| 2026-08-24 | shelve-01 | No new finding, and none of §14's triggers is touched: no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt, no path param, no CORS/header/settings change. Recorded because `shelve` is a new OPERATOR WRITE beneath the state dir. It holds `LoopLock` across every read AND every write, exactly like `release`, so it cannot run beside a live loop; its only durable writes are `tasks.json` and `state.json`, both atomic. |
| 2026-08-24 | shelve-01 | Nothing was weakened, and the direction that could have been is the merge window: `cli._merge_window_blockers` gained NO exemption for a preserved candidate. Reopening the window by writing off a record whose worker repo and candidate commit are both still on disk would be the fail-open version of this whole feature. The command reports the cost instead — it names itself as what holds the window and lists the branches waiting behind it. |
| 2026-08-24 | shelve-01 | Two guards fail CLOSED rather than defaulting to permission. An unreadable `state.json` refuses the whole command, because whether a review packet is outstanding is exactly what cannot be established from it; an unrecognised phase refuses for the same reason. The availability hazard being refused is real: exiting mid-packet strands one whose acceptance can only ever be resolved by reconciliation, never by a resend. |
| 2026-08-24 | shelve-01 | The attempt budget cannot be refunded through this route, structurally rather than by rule: `preserve_execution` performs no filesystem write at all, so `attempt_count` and `review_round` are unreachable from it, and `MAX_TASK_ATTEMPTS` / `_park_round_cap` keep reading the counters the displaced round spent. A shelve that quietly reset them would have looked identical at the CLI and handed a task an unbounded retry loop. |
| 2026-08-24 | recut-01 | The REVIEWER now holds a destructive action (`contract.Decision.RECUT`): it can discard a task's candidate and requeue the task with no operator step. Every other discarding action here passes through a human, so this is a real widening. |
| 2026-08-24 | recut-01 | Five bounds, none of them trust: at most 2 cuts per task, never a published candidate, never one whose verdict is outstanding, nothing deleted (`retire_execution` MOVES both halves), and a `task_recut` record carrying the reviewer's reason. |
| 2026-08-24 | recut-01 | The cap had to survive the operation it bounds, and the first cut would not have: `TaskExecution.recut_count` alone reads 0 on every fresh record, because a recut ARCHIVES that record. |
| 2026-08-24 | recut-01 | So the durable count is `tasks.Task.recut_count`; `_persisted_recut_count` RAISES on an unreadable stored value (reading it as 0 hands the allowance back), and `_recut_count_for` takes the higher of the two. |
| 2026-08-24 | recut-01 | Nothing was weakened. `TaskRegistry.release` is byte-for-byte as narrow as it was, and the blocked-task admission is `recut`'s own verb, which refuses an OPERATOR hold before its status test. |
| 2026-08-24 | recut-01 | So the reviewer cannot launder a human's quarantine by naming the task. Terminal statuses are refused, and an unreadable execution record is refused rather than archived unread — it could name a published candidate. |
| 2026-08-24 | recut-01 | A loop with no worker-repository manager refuses the verb outright: `retire_execution` quarantines only `if worker_repos is not None`, so it would archive the record, report success, and leave the contaminated worktree in place. |
| 2026-08-24 | recut-01 | `contract.Directive.wanted_decision` is reviewer-authored free text on every directive, and the hard bound is that it can never be acted on: it is a `str`, `Decision` is never constructed from it, and `_dispatch` reads `decision` alone. |
| 2026-08-24 | recut-01 | Acting on it would hand the reviewer an unbounded vocabulary and let it name actions the policy engine never authorized — finding #2's circular ownership, one field over. |
| 2026-08-24 | recut-01 | It also writes to disk on reviewer input, so it is bounded as input: normalised, truncated at `MAX_WANTED_DECISION_CHARS`, folded into `(other)` past `MAX_WANTED_DECISION_KINDS` keys. It cannot grow a file one key per round. |
| 2026-08-24 | recut-01 | That tally enforces nothing, so an unreadable or unwritable one degrades rather than parking — and records `tally_reset`, so a lost history never reads as a first sighting. |
| 2026-08-24 | recut-01 | Residual named rather than claimed away, ship-01's shape one decision over: `policy._check_task_reference` has no arm for `recut`, so `authorize_directive` returns `Verdict.ok()` for one naming any task. `policy.py` was out of scope. |
| 2026-08-24 | recut-01 | `orchestrator._dispatch_recut` refuses every ineligible target itself — unknown, audit unit, terminal, operator hold, published, outstanding, over cap — and is the only gate today; the policy arm is owed as defence in depth. |
| 2026-08-24 | dash-19 | No new finding. Of §14's triggers only one is touched — untrusted text into HTML. The window's reasons and notes interpolate task ids read off `tasks.json` and `GitError` messages carrying git's stderr, and every one goes through the page's `esc()` at each interpolation, pinned by `test_a_hostile_reason_renders_as_text_rather_than_as_markup`, which drives a `<script>` id and an `onerror` note through the real render. No auth, no upload, no raw SQL, no LLM prompt, no path param, no CORS/header/settings change. |
| 2026-08-24 | dash-19 | No new WRITE path, and the read-only posture is unchanged: `merge_window` writes nothing, takes no `LoopLock`, and calls nothing that takes one — pinned by a snapshot test and by one that answers while a live lock file is held. It does add the dashboard's SECOND network read (`ls-remote`, via `_candidate_publication`), reached only for a non-terminal record that carries both a candidate and a recorded push intent, and bounded by `_bounded_git`'s 15s timeout because `GitGateway` sets none. |
| 2026-08-24 | dash-19 | Nothing was weakened, and the direction that could have been is the window itself: no exemption was added, no blocker removed, and `cli._merge_window_blockers` is CALLED rather than copied, so the page cannot come to disagree with the loop about whether a merge is safe. Every unanswerable question — unloadable config, unparseable registry, git or remote failure, timeout — renders `unknown` with a stated detail and never `open`, which is the fail-open sentence this panel exists to stop an operator reading out of a stale log line. |
| 2026-08-24 | exec-01 | No new finding. Of §14's triggers only one is adjacent — a prompt built from content an agent influenced: a failed round's summary now names the PATHS git reports in the worker repo, and an agent chooses filenames. Not a new channel. The same packet already renders `details = result.raw_text`, the agent's own unbounded text, verbatim; committed rounds already render `changed_paths` through `packet._format_changed_paths`; and nothing parses this sentence. No auth, no upload, no raw SQL, no subprocess or argv, no path param. |
| 2026-08-24 | exec-01 | Bounded anyway, because an agent-chosen string with no cap is what `_cleanup_note` already refuses to quote: `PARTIAL_WORK_MAX_PATHS` (20) and `PARTIAL_WORK_MAX_PATH_CHARS` (800) cap the list and the dropped count is always stated. It cannot reach a commit message — every branch carrying it returns before a commit exists, and the success path is untouched. `_measure_partial_work` never raises, so a report on a failure path cannot itself become a second failure. |
| 2026-08-24 | exec-01 | Nothing was weakened. This adds one READ of the worker repo through the existing `stall.WorkerTreeProbe`/`GitGateway` pair, so the policy whitelist covers it exactly as before; no new git verb, no `--numstat`, no widened whitelist. `measured=False` renders UNKNOWN rather than an empty list, which is the fail-open direction here: an unreadable repo reported as "the agent wrote nothing" is a reviewer told a round produced nothing when nobody looked. |
| 2026-08-24 | port-06 | No new finding, and no §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess, no LLM prompt, no path param, no CORS or settings change. Recorded because it changes where S32's operator write RESOLVES: `/api/priority` and its mutation ledger. Both now come from the loop's own `config.resolve_state_dir` / `workers_root_from` rather than the dashboard's second copy of those rules. |
| 2026-08-24 | port-06 | The direction is NARROWING, twice. An unresolvable state directory now REFUSES the write (`nothing was written`) where it used to write `<repo>/.autoloop/tasks.json` — inside the snapshotted checkout, where the escape detector would see an unattested `tasks.json` move and park the loop. And the ledger root no longer guesses `~/.autoloop/workers`, which filed the attestation in the operator's home directory about a checkout it was not derived from. |
| 2026-08-24 | port-06 | With no usable `workers_root` the ledger falls back INSIDE the state dir, exactly as `tasks.mutation_ledger_for` documents: no attestation is available, and the detector seeing the write is the safe direction. S32's exemption is unchanged — it still covers `tasks.json` alone and still requires a completed ledger chain. Nothing new is reachable from the page: the routes, their guards and their payloads are untouched. |
| 2026-08-25 | brw-15 | No new finding, and no §14 trigger is touched. The verified strength "Autoloop conversation rotation cannot edit tracked files or escape its budget" (2026-07-31) now has NO SUBJECT: `orchestrator._heal_config_url` was the only caller of `config_writer.update_conversation_url`, and every rotation entry point is removed. That is narrowing, not weakening — the loop no longer writes the config file at all on a transport fault. |
| 2026-08-25 | brw-15 | Two consequences for that strength's own wording. Its guards are UNTOUCHED and still enforced by their own tests: `config_writer.assert_untracked` still refuses a git-tracked config and still fails closed when git cannot be consulted, and `PolicyEngine.check_rotation_budget` is unchanged in `policy.py`. What is gone is the caller. Do not read the missing caller as licence to make `assert_untracked` advisory, or to derive `project_url` from `conversation_url` for the presence search that still reads it. |
| 2026-08-25 | brw-15 | Its verification check (`pytest autoloop/tests/test_transport_recovery.py -k "config or rotation_cap or project_url"`) still selects the config-writer tests, which are kept; the rotation-cap and project-url cases it also selected are gone with the machinery. The replacement for the removed half is `test_no_rotation_machinery_is_reachable_from_the_orchestrator` in the same file, which asserts the eight methods, four constants and four blocker codes are absent from `orchestrator.py`. |
| 2026-08-25 | brw-16 | No new finding, and no §14 trigger is touched — no auth, no upload, no raw SQL, no new subprocess, no LLM prompt built from user content, no path param, no CORS or settings change. Recorded because it makes a whole transport UNREACHABLE at runtime: with `browser_chatgpt` unregistered and `conversation._BROWSER_BACKED` empty, nothing in a live run constructs a Playwright session, drives a DOM, or launches Chrome through `browser.restart_command`. Narrowing, not weakening. |
| 2026-08-25 | brw-16 | Two things NOT to misread. `autoloop/browser/` is untouched on disk and its code is unchanged, so every guard in it stands exactly as reviewed — this removes a call site, not a control. And `register_provider(..., browser_backed=True)` still re-arms the full browser recovery path (restart command, cooldown, unattachable park), so an adapter registered later restores that surface deliberately and visibly rather than by accident. |
...
| 2026-08-25 | ceil-01 | No new finding, and no §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt built from user content, no path param, no CORS or settings change. Recorded because the REVIEWER gains two bounded authorities beside `recut`: it can EXTEND a task's attempt budget, and it can RETIRE a task into subtasks with no operator step. Both are widenings of what a non-human decides, so both are bounded by construction rather than by trust. |
| 2026-08-25 | ceil-01 | The bounds, none of them trust: at most `MAX_CEILING_EXTENSIONS` (1) grants of `CEILING_EXTENSION_ATTEMPTS` (2) per task; splitting refused past `MAX_SPLIT_DEPTH` (1); at least `MIN_CEILING_SPLIT_TASKS` (2) children, each inheriting the parent's spend so no fresh budget is issued; a published candidate never split; an unreadable execution record refused rather than acted on; and `attempt_ceiling_plan_requested` / `attempt_ceiling_extended` / `task_ceiling_split` records carrying the reviewer's own reason. |
| 2026-08-25 | ceil-01 | `MAX_TASK_ATTEMPTS` is NOT weakened and is unchanged at 5: a structural refusal still spends an attempt without spending a review round, and when both remedies are spent the task still parks `attempt_count_ceiling`. The counters that bound all this live on `tasks.Task`, not the execution record a split archives, and `_persisted_nonneg_int` RAISES on a stored value it cannot read — reading an unreadable grant count as 0 hands the whole allowance back, which is the guard switching itself off. |
| 2026-08-25 | ceil-01 | Two availability fail-opens are closed by PARKING rather than continuing. The ceiling check spends neither the attempt budget nor the denial budget, so a request that could not be RECORDED is never sent (an unrecorded question is re-asked every dispatch, forever, with `needs_attention` false); and a task that already asked parks instead of asking again. The reviewer's answer is likewise not trusted to differ — an identical plan parks, because the request itself shows it the stored plan. |
| 2026-08-25 | ceil-01 | Nothing an agent writes reaches any of this: the counters are written by the dispatch from the reviewer's directive, `Task.inherited_attempts` comes from git-backed `TaskExecution.attempt_count`, and `approved_paths` for a subtask still comes from the `plan` that created it — never from an executor's report. The parent's candidate is MOVED, not deleted, through the same `release_task_to_pending` / `retire_execution` pair `recut` and `release` use. |
| 2026-08-25 | ceil-01 | Revision round closes one more silent-widening path in the same bounded budget, and it needed no new authority. A pending classification request outlived the round it was asked about whenever an operator ran `release` or `shelve`; on the two verbs that archive the execution record the next dispatch then started at attempt 0 with the marker still set, so the first plan-carrying directive would have spent the task's single extension on a budget nothing had spent. `tasks._return_to_pending` now clears it for all three verbs it serves. |
| 2026-08-25 | ceil-01 | Verification check: `rg -n 'ceiling_plan_requested_at = ""' autoloop/tasks.py` must show `_return_to_pending` alongside `grant_attempt_extension` and `clear_ceiling_plan_request`, and `autoloop/tests/test_task_split.py::test_after_a_release_a_differing_plan_does_not_buy_an_extension` must still assert `attempt_extensions == 0`. `unblock` is deliberately NOT on that list: it ends no round, and a ceiling park tells the operator that answering it does not re-ask the reviewer. |
| 2026-08-25 | intake-02 | No new finding, and no unmitigated exposure was found. Three §14 triggers are touched and each is answered here. (1) A user-supplied FILENAME: the draft slug arrives in an HTTP body and becomes a path. `inbox.draft_path` refuses anything outside `^[a-z0-9][a-z0-9._-]{0,63}$`, so no separator, no `..`, no leading dot and no absolute form is representable, and every route resolves through that one function. Pinned by `test_a_slug_cannot_address_a_file_outside_the_intake_directory`. |
| 2026-08-25 | intake-02 | (2) An LLM PROMPT built from operator content. `interview_prompt` and `plan_prompt` carry the idea, the answers and mechanically-read repo paths to the configured conversation provider. Not a new channel and strictly smaller than one already accepted: the loop already sends whole review packets — diffs of the checkout — to that same provider. The reply is treated as INPUT throughout: only `?`-prefixed lines become questions, only a `contract.parse_response` plan becomes tasks, and nothing from a reply ever becomes evidence or an approved path. |
| 2026-08-25 | intake-02 | (3) A new dashboard WRITE surface, and it is the same class as `/api/task` rather than a widening of it. `/api/intake` and `/api/intake/edit` write one markdown file beside `workers_root` — outside the checkout, so nothing the escape detector snapshots — and `/api/intake/submit` queues through the SAME `TaskInbox.submit` `/api/task` uses, so S30's accounting is unchanged: a creation request, never a mutation of an existing task's scope. All four routes sit behind the existing `X-Autoloop` + Origin gate on 127.0.0.1. |
| 2026-08-25 | intake-02 | Nothing was weakened, and the direction that could have been is `approved_paths`. It still comes from `path_suggest` and is still authorized only by a human pressing submit; `inbox.plan_step` DISCARDS the paths a plan reply names, so a model cannot propose a task carrying its own permission slip — finding #2's circular ownership, one entry point over. Pinned by `test_planning_produces_one_level_and_takes_its_scope_from_the_repo`, which sends `/etc/passwd` and `everything/` in the reply and requires neither to survive. |
| 2026-08-25 | intake-02 | Availability, and it is where this feature could have failed OPEN rather than closed. `inbox.refuse_if_round_running` treats an unreadable or CORRUPT lock as running, deliberately disagreeing with `LoopLock.is_live` (which answers "may I take this lock", where permissive is recoverable) — a question reaching a model mid-round is `ask_user` rebuilt. And `draft_blockers` requires positive evidence, so a provider that is down cannot make an unanswered draft look finished. Verification: `rg -n 'pid == -1' autoloop/inbox.py` must show the guard. |
| 2026-08-25 | intake-02 | Revision closes a real confirmation gap on the SAME `approved_paths` direction the note above says was not weakened. The scope is authorized by the operator pressing Submit — but the dashboard posted straight to `/api/intake/submit`, which re-parses the FILE, so an operator who narrowed the mechanically suggested paths in the textarea and pressed Submit authorized the WIDER paths still on disk and was told "queued". The button now saves the box through `/api/intake/edit` first and queues nothing when that save fails. |
| 2026-08-25 | intake-02 | Severity LOW: local page, operator's own edit, and the queued request is visible as text before anything runs against it — but it is finding #2's family (a scope nobody confirmed) and silent, which is why it is recorded rather than fixed quietly. Verification: `rg -n 'iksaveFirst' autoloop/dashboard.py` must show one helper and BOTH the Ask and Submit handlers calling it, and `test_the_dashboard_submit_button_queues_nothing_when_the_save_fails` must still assert `urls == ["/api/intake/edit"]` with node present. |
| 2026-08-25 | split-04 | No new finding, and no §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt built from user content, no path param, no CORS or settings change. Recorded because a durable marker now authorizes a LATER process to archive a task's execution record and quarantine its worker repository. Nothing an agent writes reaches it: the marker is loop-written and carries only the parent id and the child ids the reviewer's own `plan` supplied. |
| 2026-08-25 | split-04 | That authority is bounded by the REGISTRY and never by the marker. `worktask.reconcile_split_acceptance` moves artefacts only when the parent's row reads `retired` with exactly the marker's children as its successors, so a stale, hand-planted or mismatched marker moves nothing and is reported. Every uncertain ending KEEPS the marker and files a `split_acceptance_unreconciled` blocker: unreadable marker, stores not wired, retirement refused. Nothing is deleted — both halves are MOVED, as before. |
| 2026-08-25 | split-04 | Verification checks: `rg -n 'recorded != wanted' autoloop/worktask.py` must show the mismatch refusal, and `rg -n 'intent_is_spent' autoloop/orchestrator.py` must show the clear gated on it. `SplitIntentStore.load` must still RAISE on a corrupt file — a marker read as absent leaves the contradictory state in place and discards the last durable record of it, which is the alarm switching itself off. |
| 2026-08-25 | split-04 | One availability fail-open was found in review and closed. `_report_split_blocker` first copied `_report_strand_blocker`'s early return once its blocker is open — safe there, because that task is IN PROGRESS, and unsafe here, because this parent is RETIRED and `cli._reconcile_retired_blockers` resolves open blockers naming a retired task. The report would then have been closed out from under the condition, with the marker left in place and nothing saying so. The transcript entry is now written every start. |
| 2026-08-25 | halt-02 | No new finding; a new Verified strength instead ("Autonomous recovery can neither publish, re-provision, nor step past a hard halt"). No §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt built from user content, no path param, no CORS or settings change. Recorded because the loop gains a new AUTOMATIC decision: with an opt-in flag it may retry a transport/environment fault and quarantine one task instead of stopping for a human. |
| 2026-08-25 | halt-02 | The direction that could have been weakened is publication, and it is not: `publisher_url_drift` is in the table with a budget of ZERO. Its only recovery is `reprovision-publisher --confirm`, an operator confirming a NEW push destination, and a loop able to take that could redirect its own publication. Nothing is pushed on that path and no snapshot is rewritten — S21's rule that a destination changes only by operator confirmation stands untouched. |
| 2026-08-25 | halt-02 | The five hard halts are refused by TWO locks, not one: the table is an allowlist, and `blockers.autonomous_recovery` refuses every `HARD_HALT_CODES` member before consulting it. A denylist was rejected — it would have automated every future park nobody had reasoned about. Verification: `python -c "from autoloop.blockers import *; assert HARD_HALT_CODES.isdisjoint(AUTONOMOUS_RECOVERIES)"`, and the parametrized hard-halt test in `test_autonomous_recovery.py`. |
| 2026-08-25 | halt-02 | Two fail-open directions closed by name. A malformed `[autonomy]` section is REFUSED at load rather than coerced — `enabled = "true"` is a truthy string and would otherwise switch autonomous mode on for a typo — and the orchestrator's own gate is `is not True`, because a directly-constructed config is validated by nothing. A CORRUPT blocker record raises rather than reading as zero recurrences, which would be a full unspent budget derived from evidence nobody could read. |
| 2026-08-25 | halt-02 | It cannot forge an operator answer either: a recovered fault's record closes through `BlockerStore.close_recovered`, which writes `archived_reason` and leaves `answer` `None` — the same discipline `archive_stale` follows so `cli._RESOLUTION_PRECONDITIONS` cannot be laundered. Verification: `rg -n '_close_without_answer' autoloop/blockers.py` must show both callers and no third, and neither may set `answer`. |
| 2026-08-25 | halt-03 | No new finding; the halt-02 Verified strength above is EXTENDED rather than duplicated, since the loop gains a second automatic decision behind the same opt-in flag: with `[autonomy] enabled`, six stale-record faults archive a record and re-dispatch instead of stopping for a human. No §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt built from user content, no path param, no CORS or settings change. |
| 2026-08-25 | halt-03 | The direction that could have been weakened is DESTRUCTION OF APPROVABLE WORK, and no new destructive path was built. The one archival routes through recut-01's `release_task_to_pending(move=registry.recut)` behind that verb's own refusals: a published candidate, a candidate whose verdict is still outstanding, a record that cannot be READ, an operator hold, and a spent `MAX_TASK_RECUTS`. `retire_execution` MOVES both halves; nothing is deleted. Verification: `rg -n 'release = release_task_to_pending\(' autoloop/orchestrator.py` must show FOUR call sites (preemption, recut, ceiling split, this) and no bespoke archive/quarantine call beside them. |
| 2026-08-25 | halt-03 | An OPERATOR HOLD is the one that would be laundering rather than churn, and it cannot be: the rebuild asks `registry.recut_obstacle` before anything moves, and that refuses `hold_origin == HOLD_ORIGIN_OPERATOR` ahead of its status test. Verification: `test_stale_record_rebuild.py::test_an_operator_hold_is_never_laundered_into_a_rebuild`, which sets the hold through `operator_block` and requires the execution record to survive. |
| 2026-08-25 | halt-03 | Two fail-open directions closed by name. `StateCorruptError` subclasses `StateError` and reaches the `state_inconsistent` handler, so a store the loop cannot READ would otherwise trigger an action that discards the round and re-dispatches; that site passes `recoverable=False`, a per-occurrence veto that can only narrow. And `_autonomous_rebuild` has NO fallback branch — an unrecognised `stale_record` rebuilds nothing rather than falling into the nearest handler. Verification: `rg -n 'recoverable=not corrupt' autoloop/orchestrator.py`. |
| 2026-08-25 | halt-03 | Publication is untouched in both directions: `publisher_url_drift` keeps its ZERO budget, the two `push_candidate_*` codes DROP an approval binding rather than resolving one, and no rebuild imports, pushes or re-provisions anything. Availability is preserved on the other side too — a queued operator `review-changeset` that is dropped has its whole record written to the transcript first, so an explicit operator request cannot evaporate unrecorded. |
| 2026-08-25 | halt-03 | REVISION, extending the note above: `changeset_binding_missing` no longer drops the operator's queued review at all — it KEEPS the entry and re-renders the packet, so the candidate stays publishable by a later approval instead of being made unpublishable for the session. The push destination is the direction that could have been weakened, and is not: the four identifiers (incl. `branch`/`dest_ref`) are read ONLY off the stored entry. |
| 2026-08-25 | halt-03 | Re-deriving them by calling `build_changeset_binding` again would read `git.current_branch()`, so a checkout that had since switched branches would rebind an operator's candidate to a destination they never named — an S21-shaped destination change with no operator confirmation. Verification: `rg -n 'current_branch' autoloop/orchestrator.py` must show NO hit inside `_rebuild_changeset_packet_at_head`, and the rebuilt packet is re-verified to carry all four stored values before it is dispatched. |
| 2026-08-25 | halt-03 | One substitution hole closed while fixing that: every rebuild now swaps the payload through `_replace_outbox`, which clears `outbox_attachment` too. `_step_ready` writes that path near the top of the step and moves it onto the request at the bottom, and every rebuild parks in between — so a displaced packet's diff file would have been attached to the NEXT request under a `report_sha256` that does not cover it, which is the one-change-presented-as-another substitution that stamp exists to prevent. Pinned by `test_the_rebuilt_packet_does_not_inherit_the_displaced_packets_attachment`. |
| 2026-08-25 | halt-03 | REVISION, correcting the note above about the two `push_candidate_*` codes: the task arm now REBUILDS a review packet rather than only dropping the binding, so state the authorization boundary exactly. It resolves NO binding and authorizes NO push. It re-presents a candidate for review, and every push-time check in `_dispatch_task_push` still runs afterwards against a binding minted by `_step_ready` from the packet's own literal identifiers. Nothing here imports, pushes or re-provisions. |
| 2026-08-25 | halt-03 | The candidate re-presented is read ONLY from the execution record on disk and rendered ONLY from the worker repository's own git objects — never from the refused approval's sha, and never from a fresh "latest" lookup. So an approval naming candidate A cannot cause candidate B to be presented as if reviewed: the rebuilt packet is the record's current candidate or the rebuild refuses. Verification: `rg -n 'build_review_packet_with_diff' autoloop/orchestrator.py` — both call sites pass a `TaskExecution` loaded from the store. |
| 2026-08-25 | halt-03 | Availability, the direction this revision is about: dropping a binding and queueing an explanatory sentence left the next request UNBOUND and the candidate unpublishable for the session — the loop's own recovery denying publication of reviewed work. Both arms now verify the rebuilt payload carries what its binder requires before dispatching, and refuse otherwise. An already-published candidate is deliberately NOT re-presented, since that would invite a second push of shipped work. |
| 2026-08-25 | halt-03 | REVISION, closing a fail-open in the DESTRUCTIVE direction that the note above did not cover: the changeset arm deleted an operator's queued review whenever its candidate "did not resolve", and no gateway, an unreadable repository, an I/O error and a name git will not parse all read that way. Destruction now requires git's own answer (`_commit_presence`: `cat-file -e` 0/1, anything else None → park). Verification: `rg -n 'object_exists' autoloop/orchestrator.py` must show the drop arm asking it, and no `except` there may turn a raise into an absence. |
| 2026-08-25 | halt-03 | REVISION 2: the SAME destructive fail-open survived on the task arm, and its blast radius is larger — a `False` there archives a live execution record, quarantines the worker, and is the ONE thing licensed to bypass `_recut_outstanding_verdict`, so a transient git failure could destroy work an approval still in flight was about to publish. Both arms now share `_commit_presence`. Verification: `rg -n '_commit_presence' autoloop/orchestrator.py` must show TWO call sites and no other route to `candidate_resolves=False`. |
| 2026-08-25 | halt-03 | Availability accounting for that fix, stated because it cuts the other way: an unanswered probe now PARKS where it used to act, so a genuinely pruned candidate in a repository git cannot open waits for a human instead of being requeued. That is the correct trade — the park is the state the loop was already in, and the alternative destroys reviewed work on evidence nobody gathered. `tree_of` failing parks for the same reason. Verification: `test_stale_record_rebuild.py` section 5b, four tests. |
| 2026-08-26 | halt-03 | REVISION 3, a CONFUSED-DEPUTY shape the two notes above did not cover: an execution record with an empty `worktree_path` made `_rebuild_task_review_at_head` build `GitGateway(Path(""), …)`, and `Path("")` is `.` — the loop process's own CWD, the primary checkout. That repository answers `False` truthfully about a commit that only existed in a worker, and `False` is the one answer licensed to archive the record, quarantine the worker and bypass `_recut_outstanding_verdict`. It refuses now, before any gateway is built. |
| 2026-08-26 | halt-03 | Same revision, the silent-guard half: an empty `candidate_sha` also archived, and that is the one shape where `_recut_outstanding_verdict` cannot object — it matches ledger entries by candidate sha, so a candidate never committed was never presented and nothing is ever outstanding for it. A guard that cannot fire is not a guard. Verification: `rg -n 'worktree_path' autoloop/orchestrator.py` — the `GitGateway(Path(execution.worktree_path)` line in that handler must be preceded by both refusals, not by a route to the archive path. |
| 2026-08-26 | shrink-01 | No new finding and no control changed — a pure extraction. The urgent-preemption entry's `file:line` list names `autoloop/orchestrator.py` for both `release_task_to_pending` and `_repair_orphaned_record`; since this change the definitions live in `autoloop/execution_records.py` and orchestrator imports them back, so read that one path as the new module. Both of that entry's greps over `orchestrator.py` still hit: the boundary/release grep finds the import plus four call sites, and the `release = release_task_to_pending(` check still finds exactly four. |
| 2026-08-25 | abort-01 | No new finding, and no unmitigated exposure was found. One §14 trigger is touched — SUBPROCESS — and it is answered here. The loop gained the ability to KILL a process group it spawned (`implement_executor._kill_group`, through `stall.ProcessGroupHandle.terminate`/`kill`, i.e. `os.killpg`). It gained no ability to START anything: the argv, the cwd and the environment of both the agent and the validation command are byte-for-byte what they were, and `killable_run` adds `start_new_session=True` and nothing else. |
| 2026-08-25 | abort-01 | The kill's reach is bounded by the same mechanism that made it necessary. Every signalled group is one this module created with `start_new_session=True`, so the group id is the spawned process's own and cannot be the loop's, a shell's, or another run's — the property `stall.ProcessGroupHandle`'s docstring already states and this reuses rather than re-derives. `os.getpgid` failing falls back to the SINGLE process, never to a wider signal, and a pid is never read from a file, an argument or anything an agent writes. |
| 2026-08-25 | abort-01 | The exposure worth naming, because it is real and pre-existing rather than new: the flag lives at `workers_root.parent/ABORT`, outside the checkout `escape_detector` snapshots, so a write-capable agent with `Write` could create it and end its own round. What that buys is nothing — the round is discarded, no candidate is committed, no push is authorized, no scope widens, the task returns to the queue unchanged — and it is LOUD: the terminal, `status`, the heartbeat and a `round_aborted` transcript entry all say the loop stopped. |
| 2026-08-25 | abort-01 | Recorded as accepted rather than filed, because it is the SAME exposure `PAUSE` has carried since it moved outside the checkout on 2026-08-02, with a strictly smaller consequence (pause stops the loop and preserves everything; this also preserves everything). Moving either flag inside the checkout is what would create a finding: the detector reports any file appearing there mid-round, so an operator stopping a live loop would park it `loop_fatal`. Verification: `rg -n 'ABORT_FILENAME|def abort_flag_file' autoloop/state.py` must show one definition, outside `state_dir`. |
| 2026-08-25 | abort-01 | Nothing was weakened, and the direction that could have been is authorization. The abort path commits nothing, pushes nothing, resolves no binding and reaches no reviewer: `_abort_round` returns from `_dispatch_task_postcommit` BEFORE `commit_and_capture`, and the ESCAPE branch above it still returns first, so a checkout-escape park can never be laundered into a clean operator stop. `verify_review`, `_dispatch_task_push` and every S21 refusal are untouched — the flag cannot reach them, because the phases that owe a packet are never killed. |
| 2026-08-25 | abort-01 | The one budget mutation is a REFUND, and it fails closed: `worktask.refund_attempt` removes only an entry that positively reads as OPEN, refuses on a counter that is not positive rather than decrementing below zero, and cannot re-open a settled entry — so a genuine task failure can never be un-charged by it. It is bounded by the operator's hand rather than by a counter, which is what the task asked for; a loop nobody is stopping cannot reach it. Verification: `rg -n 'def refund_attempt' autoloop/worktask.py` and the books-balance assertions in `test_operator_abort.py`. |
| 2026-08-25 | abort-01 | Availability, the direction this could have failed open in: an unreadable flag path reads as NOT requested, identically to `cli.pause_requested` (`Path.exists()` swallows the OSError itself). Named rather than hidden. Failing closed would be strictly worse — an unreadable path would then kill every agent the loop ran — and the consequence here is only that the loop keeps doing what it was already doing, with the operator's existing remedy (kill the process) unchanged. |
| 2026-08-26 | abort-01 | REVISION: still no new finding, and the authorization direction is now stronger rather than merely unchanged. The kill is REFUSED wherever a review packet is outstanding, decided by `state.packet_outstanding_reason`, which fails CLOSED on an unreadable session and on a phase this build does not recognise — so an unparseable `state.json` refuses the kill instead of performing one. That is the opposite default to the flag's own read, deliberately: absent evidence must not authorize a kill, while an unreadable flag must not kill everything. |
| 2026-08-26 | abort-01 | Same revision, on the one new piece of state: `implement_executor.AbortLedger` is in-memory, process-local, written only by this module's own kill sites and never from a file, an argument, an environment value or anything an agent writes. It can only make a round MORE likely to be classified as an operator abort — a classification that commits nothing, pushes nothing and charges nothing — so the worst it can do is discard a round, which is what an abort does anyway. Verification: `rg -n 'ledger.record\(' autoloop/implement_executor.py` must show only the three sites inside this module. |
| 2026-08-26 | port-05 | Tooling-only split of `autoloop/` from this repository: no security-relevant code changed, and no finding was opened, closed or edited. Recorded because several findings (S21, S22, S24, S25, S26, S28, S30, S32, S33, S35-S40) describe the loop harness rather than this application, and the harness is now outside this repository's tooling. They are deliberately NOT excised — CLAUDE.md §14 keeps findings for regression history — and they travel with the code when `autoloop/` leaves the checkout. `.autoloop/` stays gitignored; dropping that entry would expose the loop's own state dir. |
| 2026-08-26 | port-05 | REVISION: still no new finding. The added `docs` CI job is read-only — stdlib only, no install step, reads the four trackers plus `docs/audit_charters.toml`, writes nothing — so it adds no sink and no capability. Recorded for S31 / port-03: this repository can now detect its own charter file going MISSING or unparseable, which the harness deliberately treats as a supported fallback to built-in domains and therefore cannot report as a fault from its side. The file still grants nothing; read-only confinement stays argv-level. |
| 2026-08-26 | setaside-01 | An autonomous set-aside now quarantines only a task whose round is in flight. `_autonomous_set_aside_task` returned any explicit `task_id` a park site passed before consulting `state.task_execution`, so `publisher_url_drift` naming `binding.task_id` could block an eligible task t2 during t1's round. The id is validated against `state.task_execution`/`state.current_task`; a mismatch drops the plan and keeps the site's own loop-fatal terminal. Verification: `pytest autoloop/tests/test_autonomous_recovery.py -k round_in_flight`. |
| 2026-08-26 | setaside-01 | Direction check: this can only ever quarantine FEWER tasks than before — the no-task-named fallback is byte-identical and an explicit id can now only be refused, never widened. Do not restore `if task_id: return task_id`, and do not read a missing or unreadable `state.task_execution` as agreement with whatever the site named. |
| 2026-08-26 | setaside-01 | REVISION superseding the line above: the no-task-named fallback is no longer byte-identical. It is still anchored on `state.task_execution`, but the single active identity now VETOES it, so a session whose two round records name DIFFERENT tasks sets nothing aside. Direction is unchanged — strictly fewer tasks than before. Reading the two records as independent authorities, first match wins, was the rejected first cut: an id matching either one licensed a quarantine for a round that was not its own. |
| 2026-08-26 | setaside-01 | Accepted availability cost of that veto, named rather than hidden: a fault raised while the records disagree (a `login_expired` during t2's dispatch while t1's record is still mirrored awaiting push) sets no task aside, so the loop parks where it used to carry on. Traded deliberately — availability is not worth blocking work that did nothing wrong. Do not resolve a disagreement by preferring one record; `pytest autoloop/tests/test_autonomous_recovery.py -k disagree` pins it. |
| 2026-08-26 | release-02 | No new finding. Recorded because a new OPERATOR verb, `discard`, can close a blocker and retire an execution — four bounds, none of them trust. It never reaches an operator hold (`hold_origin`, refused with no override and no `--force`); it never closes a `loop_fatal` record (allowlist on `kind`, so empty or unrecognised counts as loop_fatal and REFUSES the whole command); it never un-completes finished work (`task_not_blocked`); and nothing is deleted — `retire_execution` MOVES both halves. |
| 2026-08-26 | release-02 | It cannot forge an operator answer: closure goes through `BlockerStore.archive_stale`, which writes `archived_reason` and leaves `answer` `None`, so `cli._RESOLUTION_PRECONDITIONS` cannot be laundered by it. `--reason` is required and refused blank BEFORE the lock, so an archival is never a silent delete and a blank one cannot fail after the execution is already retired. Verification: `rg -n '_close_without_answer' autoloop/blockers.py` must still show exactly two callers, neither setting `answer`. |
| 2026-08-26 | release-02 | Two fail-open directions closed by name. An UNREADABLE blocker store refuses the discard rather than reading as "nothing open names this task" — that reading would queue a task over a question nobody could read. And the post-discard merge-window line reports UNKNOWN (read as SHUT) when the predicate raises, never OPEN. Verification: `rg -n 'def _discard_blocker_refusal' autoloop/cli.py`, and the `open_blockers()` read in `_discard_locked` must stay inside its `except (StateError, OSError)`. |
| 2026-08-26 | release-02 | Ordering is the security-relevant part, and it is the INVERSE of `release`'s: retire the execution FIRST, close the blocker SECOND, save `tasks.json` LAST. A failed execution half leaves the blocker OPEN and the task NOT queued; a failed save writes every closed record back open (`_reopen_blocker`). So the state the claim forbids — a task back in the queue with an open blocker — is not reachable from any single write failure. Verification: `autoloop/tests/test_worker_publisher.py` section 15's three injected-fault tests. |
| 2026-08-26 | halt-01 | No new finding; the halt-02/halt-03 Verified strength is EXTENDED, since the same opt-in flag gains a third family: seven exhausted budgets and ceilings set the round in flight aside instead of parking or stopping. No §14 trigger is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt built from user content, no path param, no CORS or settings change. Nothing is published, imported, re-provisioned or archived on any of these paths: every entry is `RECOVER_UNAVAILABLE`, so no rebuild handler is reachable from them. |
| 2026-08-26 | halt-01 | The five hard halts stay refused by the SAME two locks, and the new `_to_fault_stop` door is held to them: it consults `_autonomy_plan`, which calls `blockers.autonomous_recovery`, which refuses `HARD_HALT_CODES` before the table. Verification: `python -c "from autoloop.blockers import *; assert HARD_HALT_CODES.isdisjoint(AUTONOMOUS_RECOVERIES)"`, plus `test_budget_set_aside.py::test_a_hard_halt_reached_through_the_fault_stop_gate_is_refused_too`. |
| 2026-08-26 | halt-01 | The fail-open this change is written around, in the AVAILABILITY direction: `_autonomous_fault_set_aside` resolves the plan and the victim itself and delegates only on a non-None answer. Handing every fault stop to `_to_needs_user` would convert a REFUSED set-aside into a `loop_fatal` park holding the session open for an answer nobody can give — rebuilding the `ask_user` stall that terminal was created to remove. Verification: `rg -n '_autonomous_fault_set_aside' autoloop/orchestrator.py` must show one caller, and the refusal path must not park. |
| 2026-08-26 | halt-01 | A BUDGET must not be able to switch itself off. `blockers.SESSION_CEILING_CODES` refuses `iteration_budget_exhausted` ahead of the table, because a set-aside deletes the session file — so automating it would hand the next iteration `iteration = 0` and make `policy.max_iterations` unenforceable while blocking the backlog. Do not move that code into `AUTONOMOUS_RECOVERIES`. Verification: `pytest autoloop/tests/test_budget_set_aside.py -k session_ceiling`. |
| 2026-08-26 | split-03 | Same widening class as recut-01's rows above: the REVIEWER now holds a SECOND action that retires a task and archives its candidate with no operator step (`contract.Decision.SPLIT`). None of §14's other triggers is touched — no auth, no upload, no raw SQL, no subprocess or argv, no LLM prompt, no path param, no CORS/header/settings change. Verification: `rg -n 'Decision.SPLIT' autoloop/orchestrator.py` must show exactly one dispatch branch, `_dispatch_split`. |
| 2026-08-26 | split-03 | Nine bounds, none of them trust, all in `orchestrator._dispatch_split` before anything moves: one level of subdivision per planned task (`MAX_SPLIT_DEPTH`, off the DURABLE `Task.split_depth`, which survives the archival a count on the execution record would not), at least `MIN_CEILING_SPLIT_TASKS` successors, no audit unit, no unknown task, no TERMINAL task, never a published candidate, never one whose verdict is outstanding, and never a task that has committed no candidate at all. |
| 2026-08-26 | split-03 | That depth is read FAIL-CLOSED rather than through `_nonneg_int`: a value that is not a non-negative int refuses the split instead of reading as 0, because 0 means "may be split" and an unreadable bound would switch the one-level rule off exactly where it does its work. The terminal check is not a duplicate of policy either — `_check_task_reference` has NO arm for `shipped_elsewhere`, which would otherwise reach `retire`'s refusal after the successors exist. Verification: `pytest autoloop/tests/test_split_decision.py -k "depth or terminal"`. |
| 2026-08-26 | split-03 | Nothing was weakened and nothing was deleted: acceptance is ceil-01's `_apply_split` unchanged — `retire_execution` MOVES both halves under `split-by-reviewer`, split-04's durable marker still bounds the crash window, and a marker that cannot be written REFUSES rather than proceeding unprotected. The registry gate is `policy._check_task_reference`, so an operator hold, a completed task and a retired one are all refused before dispatch — closing, for this verb, the residual recut-01's last row names for its own. |
| 2026-08-26 | split-03 | The fail-open this change is written around: `_successors_that_would_strand` refuses a successor depending on the parent being retired or on an already-retired task. `add_many` ACCEPTS those edges, and `retire` then re-points the parent edge at every live sibling — a cycle raised from INSIDE `retire`, after the children exist, leaving a half-applied split. Verification: `pytest autoloop/tests/test_split_decision.py -k strand`. |
| 2026-08-26 | esc-02 | New S41. The escape detector now snapshots a LOOP-OWNED clone instead of the checkout the operator also works in. What it watches is unchanged and must stay unchanged — tracked + untracked + IGNORED, because `.gitignore` is writable by the agent being watched — and `__pycache__`'s S24 exemption is untouched. What moved is WHOSE tree it is: both loop-fatal parks on 2026-08-26 were the operator's own `ruff` cache and their own Claude Code rules file. This is port-01's argument applied to the tree itself. |
| 2026-08-26 | esc-02 | The residual is named in S41 rather than argued away: a write into the operator's checkout is now unobserved. It is bounded by `orchestrator._worker_fetch_root` — a worker repository is seeded from the CLONE, so the one absolute path to a non-worker tree that a worker carries on disk (`.git/FETCH_HEAD`) names a watched tree. An agent that already knows the operator's path from elsewhere was never distinguishable from the operator there anyway. |
| 2026-08-26 | esc-02 | Every synchronisation branch fails safe and none silently observes a stale tree: missing → created; behind → fetched, checked out, HEAD READ BACK not predicted; ahead/diverged → refused, nothing reset; dirty (ignored residue included, which `git status` cannot see) → refused BEFORE any fetch, so an escape's residue is never checked out over; not a repository → refused, nothing deleted; any git failure → refused. Each parks `observed_checkout_unusable` and the agent never starts. |
| 2026-08-26 | esc-02 | Two directions deliberately NOT weakened. `validate_observed_checkout` refuses a clone nested under the checkout, its `.git`, the state dir, `workers_root` or the publisher paths — AND one CONTAINING any of them, which would put every loop write back inside the snapshot (port-01's bug on the new tree). And `primary_checkout_dirty`'s recheck (`cli._precondition_checkout_clean`) follows the tree the park was about, or it would clear on the operator's checkout being clean. |
| 2026-08-26 | esc-02 | Gap owed, stated rather than left to be found: `doctor.py` does not run `validate_observed_checkout`, so an unsafe `[paths].observed_checkout` is refused only when a real run builds its orchestrator. That file was outside esc-02's approved scope. Verification for the change itself: `rg -n "_observation_git|_worker_fetch_root" autoloop/orchestrator.py` — a bare `self._git` reappearing at a snapshot, baseline or worker-fetch site is the regression. |
| 2026-08-26 | esc-02 | S41 amended by an adversarial pass. The FETCH_HEAD bound held for worker CREATION only: `_carry_reviewed_candidate_past` still fetched the moved head from the operator's checkout, so every carried-forward task got a signpost to the one unwatched tree. It uses `_worker_fetch_root()` now, after a re-sync that pins the commit, and the verification check names that second site. Also recorded, not guarded: a clone wired alongside the legacy `worktrees` mechanism would watch the clone while its agent worked in a tree sharing `.git` with the primary checkout. `cli.py:594` never builds that pair. |
| 2026-08-26 | select-01 | No new finding, and no change to Open findings or to any Verified strength. What changed is how much validation a candidate gets before approval: per-commit test selection now narrows a round in which one changed path is unresolvable instead of running everything. The control is unweakened in the direction that matters — a path that can be attributed nothing still widens the WHOLE run and is named in the evidence — and the pre-commit run is still unnarrowed, so a narrowed post-commit re-run is still ADDED to a full-suite run of the same tree. |
| 2026-08-26 | select-01 | The one fail-open shape considered and CLOSED in code rather than argued away: `validation._files_referencing` re-reads every graph file, and a file it cannot read is seeded for EVERY unresolved path instead of being dropped. Dropping it would be a check that silently passes on unreadable input. Leaning on `build_import_graph` having marked the same file `opaque` would not have closed it either — a file that parsed during the walk and became unreadable a moment later is in neither set, so the guard would have rested on a race. |
| 2026-08-26 | advis-01 | No new finding, no Open-findings change, and no Verified strength weakened. The zero-request hand-back adds ONE bounded re-invocation of the same write-capable agent with the same `AgentSpec` shape: `WRITE_ALLOWED_TOOLS` and `IMPLEMENT_DISALLOWED_TOOLS` are byte-identical, `Bash` stays disallowed, no flag is added, no process is spawned for the agent to talk to, and `ValidationEnv` credentials still never leave the loop's process. The transport is unchanged — the same two fixed paths, the same discard-unread payload. |
| 2026-08-26 | advis-01 | Two properties held deliberately. The new prompt section is ECHO-SAFE: no line begins with `ASSUMPTION:`, `REMOVE-OUT-OF-SCOPE:`, `REVERT-OUT-OF-SCOPE:` or `DELETE-FILE:`, pinned by a test, so an agent quoting its whole brief back forges neither a disclosure nor a deletion. And `_combined_report` concatenates rather than replaces, so a hand-back cannot silently DROP a `DELETE-FILE:` the first invocation made — the authorities those lines are checked against (`tasks.deletable_paths`, `authorized_cleanup_paths`) are untouched. |
| 2026-08-26 | advis-01 | The hand-back bound is itself the control: the counter is the executor's, spent BEFORE each re-invocation and unreachable from agent output, a negative allowance reads as zero, and the round is forwarded once it is spent. An unbounded version would be a denial of the round by the loop's own machinery. Reporting is unchanged in provenance — `note()` still reads only this module's counters, never `result.raw_text`, and the UNANSWERED state is derived from filesystem observations the executor makes. |
| 2026-08-27 | advis-01 | REVISION: the round is now WITHHELD rather than forwarded once that bound is spent, which makes availability the thing to account for. It is bounded in BOTH directions — one re-invocation within the round, and `status="error"` with an empty `fault_kind` outside it, so the refusal spends the TASK's attempt budget and ends at the existing `attempt_count_ceiling` park rather than looping. Naming a `fault_kind` would have spent the fault budget and let a task refuse itself indefinitely; that is the fail-open, and it is closed by a test. |
| 2026-08-27 | advis-01 | Still no new finding and no Verified strength weakened. The withhold grants nothing: no tool, no flag, no path, no process, and it decides from executor-owned counters only — agent prose cannot buy a candidate any more than it could buy a hand-back (pinned by the echo test). Verification check: `rg -n "WITHHELD from review" autoloop/implement_executor.py` should show ONE construction site, and `rg -n "fault_kind" autoloop/implement_executor.py` should still show it set on the agent-failure branch alone. |
| 2026-08-27 | advis-01 | REVISION 2 adds `AdvisoryRendezvous.ask_outstanding()` and grants nothing either: it stats ONE fixed executor-owned path (`root / ADVISORY_REQUEST_FILE`), never follows a symlink, never reads content, and records no counter. Its only power is to SUPPRESS a hand-back, so the worst an agent buys by writing that file is the round it would have got before this task existed — the withhold's own trade, already accounted above. `_entry_present` still fails closed on `OSError`, i.e. towards asking again. |
| 2026-08-27 | witness-01 | No new finding, and nothing was weakened. One §14 trigger is adjacent — a path built from a supplied value: `merge_sweep.execution_record_ancestry` names a record file from a task id, and the dashboard's roadmap read is TOLERANT (raw `tasks.json`, no id validation), unlike the sweep's registry-issued ids. An id that is empty, holds a separator, or begins with `.` is refused before either the live read or the archive glob, so no traversal is representable. Pinned by a test that fails without the guard. |
| 2026-08-27 | witness-01 | The new read is READ-ONLY and adds no capability: the dashboard now opens execution records (live and archived) for reading and nothing on that path writes, moves or retires one — pinned by a test comparing record bytes and `git status --porcelain` before and after. No new git verb: ancestry is the existing `is_ancestor`. The S-entry grep for the sweep's own rule still holds — `rg -n '_newest_generation|_retirement_stamp' autoloop/merge_sweep.py` — because the logic moved WITHIN that module rather than out of it. |
| 2026-08-27 | witness-01 | Two fail-open directions closed by name. A record that exists and cannot be judged (torn, unorderable, or naming a sha git will not resolve) is `unverified`, never `absent` — calling it absent would report "nothing names a candidate" about a record that does, which is the shape the sweep's 2026-08-15 corrupt-record fix closed one layer down. And a reader that RAISES on the 2-second poll is caught into `unverified` rather than 500ing the page or clearing a row. Neither can produce `witnessed`. |
| 2026-08-27 | witness-01 | REVISION 2 closes a THIRD direction of the same fail-open, one layer up in the exit code: `cli._cmd_shipped_report` counted only search-state `unverified` rows, so a completed row whose EXECUTION RECORD could not be judged printed UNCHECKED and exited 0 — an alarm that could not see, reporting a clean bill of health. It now also exits 1 on any `disagreements['unverified']` row. Still no new finding, no capability granted, still read-only. Verification check: `rg -n 'disagreements.get\("unverified"\)' autoloop/cli.py` must show TWO hits — one in `_format_disagreements`, one inside `_cmd_shipped_report`'s return expression. |
| 2026-08-27 | val-04 | SUPERSEDES the last clause of the 2026-08-26 select-01 note above. The pre-commit run is narrowed too now, so a narrowed post-commit re-run is NOT added to a full-suite run of the same tree and no full-suite run is guaranteed at either phase. How much validation a candidate gets before approval therefore drops; the widening rules are unchanged, both refusal rules are honoured at both phases, and the reduction is disclosed in the evidence each run appends. No new finding, no capability granted, nothing writable that was not. |
| 2026-08-27 | val-04 | Verification check for that: `rg -n 'select_validation_commands' autoloop/implement_executor.py autoloop/orchestrator.py` must show a call in EACH file, and `rg -n 'test_selection=config.audit.test_selection' autoloop/cli.py` must show one hit — one operator setting reaching both phases. If either goes to zero, the sentence `validation.PRECOMMIT_EVIDENCE` puts in every review packet is wrong in the direction that overstates coverage. |
| 2026-08-27 | split-05 | No new finding. Recorded because a candidate is now PRESENTED to the reviewer on a path that used to park: an oversized patch becomes a stat-only packet. It is not a review bypass and cannot become one — `_current_pending_postcommit` binds NO approval while the ask is outstanding, so no candidate exists for an approval to resolve to, and `_dispatch` refuses every non-`split` reply before the push binding is even read. Both gates read the durable ledger slug `sent_for_split_review`, never the packet's text. |
| 2026-08-27 | split-05 | The text-gate was rejected deliberately and must not be reintroduced: a candidate that edits `packet.py` carries `STAT_ONLY_PACKET_BANNER` inside its own diff, so a substring search over the payload would answer "stat-only" for an ordinary FULL packet and refuse a legitimate approval. The banner is for the reviewer's eyes; the execution record decides. Verification: `rg -n 'REASON_SENT_FOR_SPLIT_REVIEW' autoloop/orchestrator.py` must show the two gates plus the one writer, and no gate may read `state.outbox`. |
| 2026-08-27 | split-05 | Only the SIZE refusal routes to the ask: `errors.DiffTooLargeError` is a `GitCommandError` SUBCLASS raised by `range_diff` and `range_diff_stat`, caught in a clause ABOVE the broad one, so a torn repo or an unresolvable sha still parks on `review_packet_build_failed` with the same message. Nothing was weakened — every existing broad handler still catches the size refusal. Verification: `pytest autoloop/tests/test_git_gateway.py -k "size_cap or not_about_size"`. |
| 2026-08-27 | split-05 | Two availability directions named. The ask costs the same one attempt the park it replaces costs (`ATTEMPT_TASK`), so it cannot be spent for free, and `MAX_SPLIT_DEPTH` (1) makes a successor's oversized candidate PARK rather than ask again — a mechanism that could ask forever is a loop with no park in it. The reply-time park CLEARS `state.task_execution`, which is what arms the gate; without that, an operator answering the blocker would be re-parked by the next directive, forever. |
| 2026-08-27 | split-05 | One read fails OPEN by choice, named rather than hidden: an execution record that cannot be READ answers "no ask outstanding" (logged `stat_only_split_review_record_unreadable`) instead of parking every later directive with a message about a candidate the loop cannot read. Safe because the approval gate does not rest on it — the stat request bound nothing when it was SENT, and `_dispatch_task_push` re-reads the record itself and refuses `push_candidate_stale` when it cannot confirm the candidate. |
