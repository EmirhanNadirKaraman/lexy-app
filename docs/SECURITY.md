...
| 2026-08-24 | release-01 | No new finding, and none of §14's triggers is touched: no auth, no upload, no raw SQL, no subprocess, no LLM prompt, no path param. Recorded because a new operator command MOVES two directory entries and CLOSES blocker records — `release-blocked` retires a quarantined task's worker repo and execution record, then archives every open `task_fatal` blocker naming it. Both writes go through the existing `worktask.retire_execution` and `BlockerStore.archive_stale`; no new primitive was added. |
| 2026-08-24 | release-01 | The one input an operator controls freely is `--reason`, and it never reaches a path. `retire_execution` interpolates its `reason` into `quarantine/<task_id>-<label>` and `executions/archive/<task_id>-<label>.json`, so the label stays a module constant plus a timestamp exactly as `release`'s is, and the free text is stored as blocker data. Pinned by a test driving `../../etc/passwd  oops` through the command and requiring neither path to carry it. |
| 2026-08-24 | release-01 | Nothing was weakened. `TaskRegistry.release`'s in-progress precondition is byte-identical, so it still cannot un-complete finished work; the new verb refuses a COMPLETED task and an OPERATOR hold (`hold_origin == HOLD_ORIGIN_OPERATOR`) with no override flag; `answer` still leaves the execution record alone; and an open `loop_fatal` record naming the task refuses the whole command, so `start`, `health` and the heartbeat cannot be quieted by discarding one task's work. |
| 2026-08-24 | release-01 | Fail-closed at every unreadable input. A blocker store that cannot be read refuses rather than reading as "nothing open" — which would requeue a task with its question still live. A failed retirement ends the command with the blocker OPEN and the task still quarantined. An archival that cannot be followed by a requeue is written back OPEN (`_requeue_after_discard`), which is `_requeue_after_close`'s existing rule applied to a multi-record close. |
