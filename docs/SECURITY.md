...
**Leftover (dead code today — prune if unused):** `core/deps.rate_limit_sentence_match` + the `SENTENCE_MATCH_*` constants in `services/rate_limiter.py` are now unreferenced. Kept short-term as ready-to-wire infra in case authenticated throttling is wanted; if that doesn't materialise in a few weeks, delete them rather than letting unused code accrete.
