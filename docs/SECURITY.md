...
- **Fail-fast validation changes what is REPORTED, never what is authorized (added 2026-08-22, `val-03`).** `validation.run_validation_commands` now stops at the first failing command and reports the rest `NOT RUN`, which touches the check a gate reads — so the boundary is stated rather than assumed. (1) **No verdict moved.** `all_passed` is False in exactly the cases it was False before: the short circuit is reached only AFTER a command has already failed, so a run that used to pass cannot now fail and a run that used to fail cannot now pass. A refused binary, a missing binary and a timeout are still failures rather than exceptions; an empty command list still reports passed. (2) **Nothing is skipped on a passing run.** The loop breaks only on a failure, so every command still runs when every command succeeds — a candidate cannot avoid a check by making an earlier one pass. (3) **The report still accounts for every configured command**, one `PASS`/`FAIL`/`NOT RUN` line each, so the summary the reviewer authorizes from cannot silently shrink; `NOT RUN` is deliberately distinct from `PASS` and from the `SKIPPED` that per-commit test selection reports. (4) **Redaction is unchanged and covers the new lines** — `redact_with(validation_env, …)` still runs LAST over the assembled summary, which is what keeps a `ValidationEnv` value out of `state.last_validation`, the transcript, blocker records and the review packet (S27). (5) **The allowlist is untouched:** `SAFE_VALIDATION_BINARIES` is unchanged, and a refused command is still refused BEFORE any subprocess spawns — the refusal now also stops the run, which can only launch fewer processes, never more. **Verification check:** `pytest autoloop/tests/test_validation_failfast.py` — includes `test_a_refused_binary_is_still_a_failure_and_launches_nothing` (asserts the runner saw nothing), `test_the_summary_is_still_redacted_end_to_end`, and `test_a_passing_multi_command_run_still_reports_each_command`; `rg -n 'redact_with' autoloop/validation.py` must still show it wrapping the returned summary. **Do not** move the short circuit before the failure is recorded, and do not drop a `NOT RUN` line to shorten the summary — an unnamed command reads as one that was never configured.
...
- **2026-08-22** — **Validation stops at the first failing command (no new findings; one Verified Strength added).** `validation.run_validation_commands` short-circuits by default and reports every command after the failure as `NOT RUN` (`val-03`, `docs/AUTOLOOP.md` §4h). Recorded here because it changes the CHECK a gate reads, and the boundary is worth stating explicitly: no verdict moved (the break happens only after a command has already failed, so `all_passed` is False in exactly the cases it was before), nothing is skipped while everything passes, the summary still names every configured command, and the `redact_with` call that keeps `ValidationEnv` values out of `state.last_validation` still runs LAST over the assembled summary, covering the lines this added (S27). `SAFE_VALIDATION_BINARIES` is untouched and a refused command still spawns nothing — it now also stops the run, which can only launch fewer processes. `AuditExecutor._run_validation`, which never routed through this function, still runs every command. No config key was added: neither call site that would read one (`cli._build_executor`, `orchestrator._run_post_commit_validation`) was in the task's approved paths, and a documented key nothing reads would be a false claim in the template. Verification check: `pytest autoloop/tests/test_validation_failfast.py` and `rg -n 'redact_with' autoloop/validation.py`.
