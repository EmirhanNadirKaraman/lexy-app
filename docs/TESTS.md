...
| `test_review_binding_carry.py` (new, 2026-08-21) | 15 | **bind-01 — a correction is still the same review.** THE CARRY: a parse error on a postcommit packet produces a correction whose `postcommit` binding is the packet's, `packet_sha256` included and NOT re-stamped to the correction's own digest; the correction names the task and the candidate PREFIX while deliberately not carrying the full sha (which would let it bind itself as a packet); `TaskExecution.presented_report_sha256` / `review_request_id` still point at the request that actually presented the work; review-mismatch and policy-denial corrections carry too, and `_handle_git_failure` deliberately does not — asserted by pre-setting a stale carry and requiring it cleared, so the test fails against a site that merely happens never to see one. THE LEDGER: a `push` stamped for an earlier packet publishes that exact candidate after `last_response` has moved to an unbound request; the ledger round-trips through `StateStore` as plain dicts and ages out at `MAX_POSTCOMMIT_PACKETS`; re-recording a request replaces its entry (the `_fall_back_to_omission` re-stamp). THE NEGATIVES: a stamp naming a SUPERSEDED packet is `push_candidate_stale` with the bare remote asserted empty, a stamp naming a request never sent is still `review_mismatch:request_id`, an unbound `push` is refused with a message that names the cited request id and the recovery directive, and `commit`/`commit_and_push` still land on `legacy_git_path_retired`. Real git; the count is hand-counted test functions. |
| `test_postcommit_review.py` (+1, −1 rewritten, 2026-08-21) | — | **bind-01.** `test_parse_error_correction_keeps_its_binding_and_the_stamped_push_publishes` is the prof-01 defect end to end through `run()`: packet → malformed reply → correction → a `push` stamped from the CORRECTION's own CONTEXT → the approved candidate on the bare remote, with the transcript asserted free of both `legacy_git_path_retired` and `push_missing_review_binding`. `test_legacy_commit_and_push_is_always_refused_even_with_a_live_candidate` reached its refusal through exactly that parse-error route, which is why it passed while the bug was live — it now answers the packet directly with a stamped `commit_and_push`, keeping the guarantee it is named for (a retired decision is refused with a live candidate on record) and dropping the dependency on the binding being lost. |
| `test_changeset_review.py` (1 renamed, 2026-08-21) | — | **bind-01.** `test_push_without_changeset_binding_still_hits_legacy_refusal` → `..._is_still_refused`: the effect it pins is unchanged (nothing published, `for-each-ref` empty), but an unbound `push` now lands on `push_missing_review_binding` rather than `legacy_git_path_retired`, which stays the refusal for `commit`/`commit_and_push`. |
| `test_prompts.py` (+1 parametrized ×3, 2026-08-21) | — | **bind-01.** `same_review_note` renders through each of the three carrying corrections; the note names the task and the 12-character candidate prefix, points at THIS request's CONTEXT, and must NOT contain the full 40-hex sha. `test_parse_error_payload_carries_code` gained the negative control — with no note asked for, nothing is added. |
| `test_review_binding_carry.py` (+4, 2026-08-21, review round 5) | 19 | **bind-01, second note.** The reviewed request stays authoritative when `last_response` carries a CHANGESET binding: `test_a_stamped_earlier_packet_beats_a_changeset_response_in_hand` publishes the postcommit candidate to its task branch and asserts the operator's commit is absent from the bare remote, `state.changeset` still set, and no `changeset_publisher_required` park (the harness has no publisher, so a routing regression cannot publish quietly). And the recovery: an unbound `push` is refused AND re-presented — executor call count unchanged, stored `candidate_sha` unchanged, the new request bound from its own text (`packet_sha256 == report_sha256`, the inverse of the carry assertion) with the execution record re-stamped, and a stamped `push` of that packet landing the same sha on the remote. Two negatives make the state LIE to prove the live checks are real: a record pointing at a worker directory that does not exist, and a spent `max_review_rounds`. |
| `test_prompts.py` (+2, 2026-08-21, review round 5) | — | **bind-01, second note.** The `postcommit_review` template gained a leading `{note}` field. One test renders it through `represented_candidate_note` (the payload starts with the refusal headline, carries the verdict text verbatim, and says no new work was produced), and its negative control renders the ordinary empty-note case and asserts the payload still starts with "The executor committed task t1" — the byte-identical-ordinary-packet claim, not an assumption. |
...
| 2026-08-21 | bind-01 | `autoloop/tests/test_review_binding_carry.py` added (15 test functions, real git) for the corrective-re-prompt binding carry, the sent-packet ledger and the reviewed-request resolution rule; the headline case runs end to end through `run()` in `test_postcommit_review.py`, because the defect was a whole round trip rather than one function's return value. |
| 2026-08-21 | bind-01 | `test_legacy_commit_and_push_is_always_refused_even_with_a_live_candidate` was rewritten rather than deleted: it reached its refusal through a parse error and an unbound `push`, i.e. through the defect itself, which is why it stayed green while approvals were being stranded. It now answers the packet directly with a stamped `commit_and_push`, keeping the guarantee it is named for. |
| 2026-08-21 | bind-01 | The git-failure test pre-sets a stale `outbox_postcommit` before calling `_handle_git_failure`, because the site is unreachable with one set today — asserting only "it is None afterwards" would pass against a site that never clears it, and the explicit clear is what makes the five-site rule structural instead of incidental. |
| 2026-08-21 | bind-01 | `test_a_push_naming_a_superseded_packet_is_refused_as_stale` is the obvious attack on the new resolution path and is asserted against the BARE REMOTE (`for-each-ref` empty), not the worker's own refs: the containment lives in `_dispatch_task_push`'s candidate check, and a test reading only the park message would pass against a push that landed first. |
...
| 2026-08-21 | bind-01 | `test_review_binding_carry.py` grew to 19 test functions in review round 5, for the two corrections the reviewer asked for: the changeset-in-hand routing case and the re-presentation recovery. |
| 2026-08-21 | bind-01 | `test_a_stamped_earlier_packet_beats_a_changeset_response_in_hand` makes the operator's commit BEFORE the postcommit packet is rendered, so the main checkout's HEAD still equals what that packet stamped. Without that ordering the HEAD-moved check refuses the push and the test passes whatever the routing does — a refusal proves nothing about which candidate would have been published. It also runs with NO publisher configured, so a regression to the changeset branch cannot quietly publish: it parks on `changeset_publisher_required`, which the test asserts is absent. |
| 2026-08-21 | bind-01 | That test asserts the changeset candidate's ABSENCE from the bare remote by name (`for-each-ref` output) and that `state.changeset` is still set — `_dispatch_changeset_push` clears it on publication, so an unchanged binding is independent evidence that path never ran. The ordinary current-changeset positive case stays where it already lives, `test_changeset_review.py::test_stamped_approval_publishes_exactly_the_bound_candidate`, rather than being rebuilt here around a second publisher harness. |
| 2026-08-21 | bind-01 | The re-presentation test asserts `postcommit.packet_sha256 == report_sha256` on the new request — the exact inverse of the carry test's assertion that a correction keeps the ORIGINAL packet's digest. That is what distinguishes a real fresh packet from a carried binding; `postcommit is not None` would pass against either. It pairs that with the execution record being re-stamped (`review_request_id` / `presented_report_sha256`), where a correction must leave both alone. |
| 2026-08-21 | bind-01 | Cost is asserted, not assumed: `WritingExecutor.calls` is unchanged across the whole recovery, the stored `candidate_sha` is unchanged, and the round ends with a stamped `push` of the new packet landing that same sha on the bare remote. |
| 2026-08-21 | bind-01 | Two negatives for the live-check rule, both driven by making the STATE lie: `test_the_re_presentation_needs_a_live_worker_not_a_state_field` rewrites the record's `worktree_path` to a directory that does not exist while `state.task_execution` still names a candidate, and `test_a_re_presentation_will_not_overrun_a_configured_review_round_cap` sets `max_review_rounds=1`. Both must refuse with the honest no-candidate wording and leave `review_round` and the executor call count untouched. |
| 2026-08-21 | bind-01 | Those two negatives assert the same refusal text, so each also asserts `push_binding_recovery` is ABSENT from the transcript, and the cap one asserts `worker_repo_is_reusable` is still True for the recorded worker — declined for the cap, not for a missing worker. Without that, a `_representable_candidate` that returned `None` unconditionally would leave both green while only the positive test failed. |
| 2026-08-21 | bind-01 | Both new tests use a long task id (`binding-task`) rather than the file's usual `t1`: `_current_pending_postcommit` binds a payload that contains all four identifiers as literal substrings, and a two-character id can occur by accident inside a diff or a stat line — loudly in the changeset test (`postcommit is None` would fail for a reason unrelated to routing) and silently in the re-presentation one. |
