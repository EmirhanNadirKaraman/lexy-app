...
| 2026-08-21 | split-01 | `autoloop/tests/test_task_split.py` added (35 test functions, real git, real worker repos): a crash is injected at each durable write boundary of split acceptance — intent, registry, archived record, quarantined worker, cleared intent — and the loop is RESTARTED over the same on-disk state, asserting recovery converges on the same four facts the uninterrupted run produces. |
| 2026-08-21 | split-01 | The negative half is the other half of the evidence: durable state that CONTRADICTS the intent (a successor id belonging to another task, a parent retired into different successors or already completed, a record or worker that vanished without being filed away, a missing retirement flag) parks loop-fatal with the intent preserved and no store rewritten. |
| 2026-08-21 | split-01 | A `Crash` there is deliberately neither `OSError` nor `StateError`: both are CAUGHT and parked, and a park is the graceful outcome — the window under test is the ungraceful one, where nothing runs afterwards. `CrashAt` is installed on the store INSTANCE after the split ask is already recorded, because the orchestrator and the test share one `StateStore` and a patch meant for the acceptance otherwise fires on the setup. |
| 2026-08-21 | split-01 | Two assertions carry the idempotence claim rather than the happy path: exactly ONE archived record and ONE quarantine directory survive any number of recovery passes (a second, freshly-minted label would leave two), and `task_store.save` is booby-trapped during a repeat reconciliation so a registry that already describes the split is proven not to be rewritten. |
| 2026-08-21 | split-01 | `test_task_split.py` grows to 42 test functions with the wrong-identity half: a record REPLACED after the intent was written is not archived, a worker repository replaced after it (same branch, different HEAD) is not quarantined, and in both cases nothing at all moves and the replacement is read back to prove it. Their mirror images are an impostor at the archive path and a different repository at the quarantine path — each parks instead of discharging the intent. |
| 2026-08-21 | split-01 | `test_bookkeeping_written_after_acceptance_still_reconciles` is the other direction, and pins WHY the binding is fields rather than a file digest: counters, the attempt ledger, assumptions and the report text are rewritten between the crash and the recovery, and recovery still completes. `intent_for` builds its provenance through the orchestrator's own capture methods, so no negative test can be testing a fixture's idea of the comparison. |
| 2026-08-21 | split-01 | `test_task_split.py` grows to 46 test functions for the working-tree binding, and the new `dirty_worker` helper is what makes them evidence: every earlier test runs against a CLEAN worker, which is the one shape the split trigger never produces, so a replacement at the same branch and HEAD would have compared equal and passed. It leaves staged, staged-then-re-edited, unstaged, deleted and untracked content behind without moving HEAD. |
| 2026-08-21 | split-01 | The four: a fingerprint that changes for each of those categories while HEAD stays put (the staged step stages bytes already on disk, so only an index-reading digest moves); a worker REBUILT at the same branch and the same HEAD refused and left untouched; a clean rebuild at the quarantine destination refused as proof; and the quarantine crash boundary re-run with a dirty worker, which fails if the digest reads a timestamp, an inode, a path or `.git/index`. |
