...
**611 tests, fully hermetic** — no network, no ChatGPT, no playwright import,
...
`test_postcommit_flow.py`, `test_postcommit_review.py` and
`test_worker_publisher.py` use real `git` against throwaway `tmp_path` repos;
...
| Autoloop | `pytest autoloop/tests` *(repo root; only when touching `autoloop/`)* | 611 passed |
