...
| `test_tasks.py` (+14, 2026-08-06) | — | **Description mutation (`TaskRegistry.set_description`).** The deliverable is not the setter, it is the shared validator, so the load-bearing test is `test_creation_and_mutation_reject_a_bad_description_identically`: for each of `""`, `"   "`, `"\n\t"` and `None` it asserts creation and mutation raise the same code AND the same message — a message comparison because a re-implemented check would almost certainly word its refusal differently, and this fails the moment mutation stops calling what creation calls. `None` is in that parametrize rather than in a test of its own on purpose: it exercises the validator's other branch (non-string, which creation previously reached as an `AttributeError` from `.strip()`), so parity is proven on both branches instead of only on blanks. Atomicity is asserted over the whole serialised graph, not one field — `json.dumps(to_dict(), sort_keys=True)` is byte-identical after every rejected call, which kills a validate-after-assign ordering, and an unknown id (`task_unknown`, the code every mutator routing through `get` raises; `set_priority`'s `unknown_task` is the outlier and stays one) creates nothing. Plus: the text is stored unnormalised (creation keeps padding, so mutation must too), no other task or field moves, the new text survives a `TaskStore` round-trip, and a blank description ALREADY on disk still loads — `from_dict` deliberately does not re-validate a stored graph, and a registry that refuses to load is unrecoverable without hand-editing JSON. |
