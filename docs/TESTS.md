...
| 2026-08-21 | dash-17 | `autoloop/tests/test_dashboard_merge_window.py` added (21 test functions, plus `make_repo`/`write_execution`/`snapshot`/`run_git`/`merge_window_js`/`run_js`/`render_window` helpers that are in neither figure). Hand-counted, no shell in the worker. Its own file rather than more rows in `test_dashboard.py` because it needs a checkout carrying a real `.autoloop/config.toml`, state file and registry — `test_dashboard.py`'s `make_repo` writes no config, so `load_config` raises and the window short-circuits to `unknown` before a single git call, and every test built on that fixture would pass without exercising the path at all. |
