...
| `test_merge_window.py` | +7 | **2026-08-21 (dash-17) — the optional `unanswered` sink, and nothing else.** The window is still fail-closed and every merge caller still passes no sink; `test_the_same_run_without_a_sink_produces_the_same_reasons` runs the same failing remote both ways and demands equal reasons AND equal notes, so a behavioural change hides nowhere. What the sink adds is the ability to tell a FINDING from a GUESS, which the reason strings cannot carry (they are prose, reworded twice). Three record: an unanswerable remote (`test_an_unanswerable_remote_is_recorded_as_unanswered_and_still_a_reason`, asserting the reason survives unchanged alongside the new entry) and the two LOCAL questions `_candidate_is_retired` asks — a checkout that cannot report its own head, and an `object_exists` probe refused by policy, each keeping the record and the window exactly as the pre-existing fail-closed tests already pin. Three do NOT, and they are what stop the sink filling on healthy runs and rendering the dashboard permanently `unknown`: a record with no push intent asks nobody anything; a reachable remote that answers "no such ref" gave a definite negative (asserted alongside `fake.lookups`, so "recorded nothing" cannot come from never asking); and a resolvable candidate is an answer. |
...
| `test_dashboard_merge_window.py` | 26 | **Added 2026-08-21 (dash-17).** The dashboard says WHY the base branch may not move, from the live check rather than from the last sweep's log line. Its own file rather than more rows in `test_dashboard.py`, because it needs a richer fixture: a checkout carrying a real `.autoloop/config.toml`, state file and registry, since without a loadable config `cli.load_config` raises and the whole window short-circuits to `unknown` before a single git call — every test built on `test_dashboard.py`'s `make_repo` would pass without exercising this path at all. The claims, in the order they would break something: **called, not copied** — patching `cli._merge_window_blockers` to return sentinels proves the payload's reasons and notes are that function's, which a structural grep cannot show, and the same test reads back the config and gateway it was handed (state dir absolute under the observed checkout, gateway rooted there, a FRESH `seen` per request); **read from the CHECKOUT, never the process cwd** — `load_config` keeps `[paths].state_dir` relative, so the test chdirs elsewhere with an `elsewhere/.autoloop` present, which is what makes the bug fatal rather than merely wrong (an empty state dir reads as "no in-flight candidate" and renders OPEN), and asserts the reason NAMES the task, since asserting `closed` alone passes on the broken version too via the absent-directory fail-closed branch; **reasons and notes stay apart** — the page's own region is lifted between `MERGE_WINDOW_START`/`END` markers and run under node against a stub document, asserting each list in the other's absence in BOTH directions (one alone passes for a panel that prints everything twice) plus per-list headings and classes, so the distinction is never colour-alone; **failure never reads as open, and a guess is never reported as a finding** — a record naming a remote that does not exist still becomes a fail-closed REASON containing "could not verify" (the CLI is unchanged and the window still shuts), and the PAGE reads `unknown` rather than `closed`, with that reason still rendered and `detail` naming the question that went unanswered; the discriminator is `cli`'s structured `unanswered` sink, pinned in BOTH directions (a reason merely WORDED like a failure, from a check that touched no sink, must read `closed`; a real unanswerable remote must record on the sink AND keep its reason), and an answered negative — a reachable remote that simply lacks the ref — stays `closed` with an empty `detail`. The sink DOMINATES the verdict, because a failed publication check whose record is then written off as a note leaves `reasons` empty while a remote sat there refusing to answer, which "closed if reasons else open" would have rendered OPEN. A missing config or a check that raises is `unknown` too, with the cause in `detail` and an empty `reasons` list that the status word keeps from reading as open; **no clock of its own** — the payload's keys are pinned to exactly `{status, reasons, notes, detail}` alongside the page's `const {served_at, progress, ...rest}` signature line, because a timestamp nested in the window would make the re-render signature differ every 2s and snap every opened disclosure shut; **read-only and lock-free** — measured against a lock a REAL other process owns, with the wall clock asserted and every file under the checkout (`.git` included) compared before and after, which is the only thing standing between the page and a 2s poll rewriting `.git/index`, since `GitGateway` does not get this module's `--no-optional-locks` injection. Plus: a hanging git call becomes a FAILED call rather than an exception (so one slow ref costs one per-record reason, not the whole panel); reason and note text carrying `<img>`/`<script>` renders escaped, as it must, since those strings hold worktree paths and raw git errors; the render function is asserted to contain no `indexOf`/`includes`/`startsWith`/`match`/`"executing"`, because classifying a reason by reading its text would be the page inventing semantics the CLI does not express — the "clears by itself" relief is static prose instead; and `MERGE_WINDOW_STATUSES` is pinned against the page's `MW` icon+word table in both directions. |
...
| 2026-08-21 | dash-17 | `autoloop/tests/test_dashboard_merge_window.py` added (21 test functions, plus `make_repo`/`write_execution`/`snapshot`/`run_git`/`merge_window_js`/`run_js`/`render_window` helpers that are in neither figure). Hand-counted, no shell in the worker. |
| 2026-08-21 | dash-17 | It is its own file rather than more rows in `test_dashboard.py` because it needs a checkout carrying a real `.autoloop/config.toml`, state file and registry — `test_dashboard.py`'s `make_repo` writes no config, so `load_config` raises and the window short-circuits to `unknown` before a single git call, and every test built on that fixture would pass without exercising the path at all. |
| 2026-08-21 | dash-17 | The cwd test is the one guarding the failure the design exists to prevent, and it needs both assertions: it chdirs elsewhere WITH an `elsewhere/.autoloop` present (an absent state dir is its own fail-closed reason, so `status == "closed"` alone passes on the broken version) and asserts the reason names the task and does not say "does not exist". |
| 2026-08-21 | dash-17 | Reasons-vs-notes is asserted in BOTH directions — the reason text absent from the notes element and the note text absent from the reasons element — because either assertion alone passes for a panel that prints everything twice; the page's own region is lifted between `MERGE_WINDOW_START`/`END` and run under node against a stub document, the `MERGE_PANEL_*` pattern reused rather than a second mechanism. |
| 2026-08-21 | dash-17 | The render function is asserted to contain no `indexOf`/`.includes(`/`startsWith`/`.match(`/`"executing"`: classifying a reason by reading its text would be the page inventing semantics the CLI does not express, and it would break the moment the wording changed. The "a phase is executing … clears by itself" relief is asserted in the static prose instead. |
| 2026-08-21 | dash-17 | The read-only/lock-free test reuses `test_dashboard.py`'s lock-holder subprocess and snapshots every file under the checkout, `.git` included. It drives `collect(repo)` rather than `merge_window(repo)` because the claim is about the ENDPOINT. |
| 2026-08-21 | dash-17 | `.git` is in that snapshot because the window's git calls go through `GitGateway`, which does NOT get `dashboard._run_status`'s `--no-optional-locks` injection — so this test is the only thing standing between the panel and a 2s poll rewriting `.git/index` in the tree the escape detector snapshots. |
| 2026-08-21 | dash-17 | The pre-existing `test_collect_writes_nothing_to_the_observed_checkout` is vacuous for this path: its fixture writes no `config.toml`, so the window short-circuits to `unknown` before a single git call — hence a second lock-free test rather than trusting that one. |
| 2026-08-21 | dash-17 | Not asserted, and checked by reading instead: `MutationLedger.__init__` (reached by `_load_tasks` via `mutation_ledger_for`) is a single assignment, with its `parent.mkdir` confined to `_append`. A write there would land BESIDE `workers_root`, outside the checkout and therefore outside anything `snapshot(repo)` can see — so the "writes nothing" claim rests on that read, and is recorded in `docs/SECURITY.md` S36 rather than left implied. |
| 2026-08-21 | dash-17 | Revision round: `test_dashboard_merge_window.py` grew to 26 test functions and `test_merge_window.py` gained 7, all hand-counted (no shell in the worker). `test_a_remote_that_cannot_be_read_is_a_reason_never_an_empty_open_window` was RENAMED to `..._reads_unknown_and_still_shows_its_reason` and its assertion inverted: a git or remote question that goes unanswered now renders `unknown`, not `closed`. |
| 2026-08-21 | dash-17 | The window itself is unchanged and still fail-closed — `test_the_same_run_without_a_sink_produces_the_same_reasons` is what pins that, comparing two runs of `cli._merge_window_blockers` over the same failing remote with and without the new sink. |
| 2026-08-21 | dash-17 | The `unanswered`-vs-blocker distinction is pinned in BOTH directions, because either test alone passes for a page that guesses: `test_the_page_never_infers_a_failure_from_the_text_of_a_reason` patches the check to return a reason reading "could not verify origin/…" while touching no sink and requires CLOSED, and `test_an_unanswerable_remote_is_recorded_as_unanswered_and_still_a_reason` drives the real path and requires the sink entry AND the unchanged reason. A page that matched reason prose would pass the second and fail the first. |
| 2026-08-21 | dash-17 | `test_an_unanswered_check_with_no_reasons_at_all_reads_unknown_not_open` is the reason `unanswered` DOMINATES rather than decorating: when publication fails, the fail-closed `why_not` flows on to `_candidate_is_retired`, and a record whose worker repo is gone becomes a NOTE — leaving `reasons` empty while a remote sat there refusing to answer, which a flat "closed if reasons else open" would have rendered OPEN. Patched rather than assembled, since the real path needs a vanished worktree, an unreachable remote and a pending registry entry to line up at once. |
| 2026-08-21 | dash-17 | Two negatives keep the sink honest, and without them a permanently-`unknown` panel would pass: `test_a_remote_that_answers_no_such_ref_records_nothing_unanswered` (a reachable remote ANSWERED, so a definite negative is a finding and the sink stays empty — asserted alongside `fake.lookups`, so "nothing recorded" cannot come from never asking) and `test_a_resolvable_candidate_is_an_answer_and_records_nothing`. `test_an_ordinary_blocker_that_asks_git_nothing_stays_closed` is the same claim at the dashboard level, via `detail == ""`. |
| 2026-08-21 | dash-17 | Both dashboard stubs of `cli._merge_window_blockers` take the new `unanswered` parameter, and `_explode`'s does not merely follow the signature: `TypeError` is inside `merge_window`'s except tuple, so a stub missing the parameter would produce `unknown` from a signature mismatch and every assertion in `test_a_check_that_raises_reads_unknown_rather_than_open` would still pass while testing nothing. A comment there says so. |
| 2026-08-21 | dash-17 | The lock-free test's closing assertion moved off the status word: its remote is deliberately unreachable, so the status is now `unknown` for a reason that has nothing to do with the lock. It asserts the REASON list instead — only a check that read the config, registry, state and execution record can produce one, and an `unknown` that gave up carries none — which keeps the discriminator the comment always claimed it had. |
