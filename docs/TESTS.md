...
`PendingRequest.prompt_sha256` verified before every resend).
**575 tests, fully hermetic** — no network, no ChatGPT, no playwright import,
...
`test_postcommit_flow.py` and `test_postcommit_review.py` use real `git`
against throwaway `tmp_path` repos; `test_lock.py` uses real separate
processes.
...
| Autoloop | `pytest autoloop/tests` *(repo root; only when touching `autoloop/`)* | 575 passed |
