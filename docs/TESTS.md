...
| `test_tasks.py` | 35 | Task registry/graph: ids, duplicates, unknown deps, cycles, batch atomicity, derived ready/blocked, `next_ready`, lifecycle guards, persistence.  **+1 and two rewrites on 2026-08-04 — `TRACKER_PATHS` widened to six.** `CLAUDE.md` and `docs/SCHEMA.md` joined, each earned by a real refusal (rt-06's stale test count, rt-02's migration-table row) rather than guessed at. The exact-set test is the intended way to notice a widening, so it was updated deliberately, and its `startswith("docs/")` half retired — CLAUDE.md lives at the repo root, and the property that matters is "a document, not code", which the directory was only ever a proxy for. The new case pins the sharpest entry: CLAUDE.md is the INSTRUCTIONS future agents read, so it asserts the non-circularity that bounds it — an unscoped task still gains nothing, and a scoped task's own paths still come from the Task, so a CLAUDE.md edit can never widen the editing task's scope. `test_trackers_do_not_authorize_code_outside_the_task_scope` kept its assertion and swapped its example: CLAUDE.md was listed there as out-of-scope and is now a tracker. |
...
| `test_conversation_retirement.py` (+14, 2026-08-15, conv-01 revision) | 44 (hand count of `def test_`; 46 collected — one case is parametrized ×3) | **New 2026-08-15 (conv-01) — retiring a conversation that has become SLOW, not one that is broken (`docs/AUTOLOOP.md` §5c-bis).** Reuses `test_transport_recovery.py`'s `RotatingFakeClient`/`build`/`pending`, so the retirement path is driven through exactly the transport model rotation is. **The signal choice is itself pinned**, because it is the decision the change turns on: the trigger is a packet COUNT, and the incident is why — during the observed degradation (two Chrome restarts in four minutes, no request sent) no round completed, so a submit-to-response latency series would have measured nothing. `conversation_is_degraded` is exercised at 79/80/200 and at the `0 = disabled` convention; a single slow round is shown structurally unable to reach the threshold (a fresh thread has counted one message). **Counting** covers the routes a send-only counter would miss — `submitting`'s pre-send reconcile finding the request already there counts one — and the idempotence that stops a `--retry` re-entering `submitting` from inflating a thread that did not grow. **The move**: an oversized thread is retired without ever attaching to it (asserted via the retarget order: straight to the project page, never the retired chat — loading a thread that size is the expensive thing being escaped), spends `retirements` and NOT `rotations`, logs `conversation_retired` carrying the packet count, the threshold and the latency series, writes `last_rotation` with `reason="conversation_degraded"` so the drift guard and `doctor` need no second record, resets the new thread's clocks to `conversation_packets == 1` (one, not zero — the move's own message is its first packet, and zero would put every later justification off by one) and an empty latency series, and survives a restart. **Every REFUSAL logs and carries on**, which is the load-bearing difference from `_attempt_rotation`'s park: no `project_url`, a spent budget, a provider without `retarget`/`current_url` (probed, so a doomed attempt never spends the budget), and a move that dies BEFORE its send (budget spent and not refunded, the request keeping its ORIGINAL prompt so a `--resubmit` into the old chat cannot send it a note saying that chat is abandoned) — each asserts the round went ahead in the existing conversation and nothing parked. The failed-move case additionally pins that the client is DROPPED: the move retargets it at the project page before submitting, and without the drop the next `attach()` would post the request where nobody is reading. **A move that already POSTED is not a refusal (+11, 2026-08-15 revision, §4b of the file).** The regression that motivated it: a persisted send followed by a later failure (the reconcile that was meant to confirm it) used to return to `_step_submitting`, which submitted the SAME request id into the old chat — two chats, one request, breaking the exactly-once property `submission_unconfirmed`/`submission_rejected` are built to hold. The pinned outcomes now follow `SendCertainty`, which the move records as it goes: `unsent` carries on, in all three shapes it comes in — the composer refusing the input; a failure on a step BEFORE the transport is handed anything (certainty from position, no flag consulted); and the boundary between them, a failure INSIDE a `submit()` that never reached its click, in a process where an EARLIER round already clicked Send. That last one is why the transport is asked through a probe armed immediately before the call (`begin_send_probe`): `send_attempted` answers "was Send ever clicked", not "did THIS call click", so a stale True would mark a provably-unsent retirement `possible` and park a merely-slow loop. Its case pins the whole chain — certainty `unsent`, the pessimistic marks restored ON DISK (read from the state file at `submitting`'s pre-send reconcile, the last moment they are observable before the ordinary send re-marks them), no `retirement_send_ambiguous`, and exactly one submission, into the old conversation. `unsent` needs a live observation and never the absence of one: no capability, an arm that raised, or a set flag all give `possible`. `persisted`/`possible` adopt the chat that still holds the request (found by the validated URL, or by CONTENT when the address bar never moved) and otherwise park `retirement_send_ambiguous` naming the last URL seen; `disproven` carries on only when a content search RAN and found the request in no chat in the project (a search that could not run is not absence — the finder raising sends the same case back to the ambiguous park), and is adopted rather than repeated when the project turns out to hold it after all. Adoption is proven, not assumed: a chat that no longer contains the request is refused, and the thread being retired is never adopted as its own replacement (epoch unchanged). The park is shown NOT to be the mechanism — a `--retry` after it hits `send_already_attempted` first, reconciles the old chat, finds nothing and parks as an ordinary `submission_ambiguous`, so no entry point resends. The two doubles that model all of this are honest about what they did: `RetireSendNeverHappens` raises before posting anything (the previous `RetireSendExplodes` claimed to die "right after posting" while raising before `super().submit`, which is exactly the case the suite needed and did not have), and `PostsThenDiesLater` persists the request in the replacement chat and then fails. One case reads the STATE FILE from inside `submit()` to prove the guarantee is on disk rather than in a live object: a crash is the same failure without the exception, and `req.send_attempted` is already marked when the transport is handed the prompt. **Deference**: a throttled loop defers rather than opening another chat on the same account, via both halves of the brw-09 seam (a `client.is_throttled()` capability and a `state.throttled_until` instant read with `getattr`), while an unparseable value fails OPEN — failing closed would make retirement unreachable until brw-09 lands. A request already sent, a chunked delivery in flight, and an uploaded diff each defer instead of degrading the review, with their own reason codes. **Budgets**: `retirements` is run-scoped and refilled by `_reset_run_scoped_budgets` while `conversation_packets` is NOT (it describes the conversation, which outlives the process; zeroing it per run would make a long-lived thread permanently unretirable), and the reset is proven to fire when ONLY retirements were spent — the guard read `if not state.rotations: return`, the same per-session trap that function exists to close, one field over. **The latency series is evidence, never a signal**: it is recorded, bounded to `MAX_ROUND_LATENCY_SAMPLES`, absent rather than fabricated for a request with no send stamp, and shown to move nothing on its own. |
