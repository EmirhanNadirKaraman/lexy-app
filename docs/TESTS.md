...
| `test_docs_merge.py` (2026-08-19, docs-01) | 12 | **Two branches recording a change note must merge without a conflict.** Real git throughout, and the fixture copies the REPO'S OWN `.gitattributes` in rather than writing `merge=union` inline — a test that writes the rule it is testing proves git works, not that this repo is configured. Two branches cut from one base each append a note to BOTH trackers and merge sequentially into the base with no conflict and both notes present; then three in sequence. The premise the shape rests on is verified rather than assumed: a 19,000-character single-line table row that both branches GROW unions into a duplicated row carrying one note each, which is why notes are one line and existing lines are never grown. Conflict handling is pinned on the other side too — the same edit/edit shape in a source file still conflicts, aborts, and leaves the base untouched, and a doc NOT named in `.gitattributes` (`docs/TODO.md`) still conflicts, so the rule is narrow. One test goes through the production `GitGateway.merge_commit` rather than a bare `git merge`, since that is the call the sweep makes. The accepted trade is a characterization test, not a silence: two branches editing the SAME note line duplicate it instead of stopping the sweep. (12 is a hand count of `def test_` in the file — no Bash in the worker — not a collected count.) |
...
Four rules. They are not style — they are what lets two parallel branches
merge (`.gitattributes` gives this file `merge=union`; the shape is pinned by
`autoloop/tests/test_docs_merge.py`, the reasoning is in `CLAUDE.md` §12):

1. **Add a line. Never grow a line.** Do not append your note into an existing
   table row or paragraph. Union merges resolve per LINE, so two branches that
   grow the same line duplicate that whole line instead of merging the two
   additions — which is how one row in this file reached 15,729 characters.
...
3. **Never edit or reorder a line someone else wrote.** A concurrent edit to
   the same line unions into a duplicate rather than stopping the sweep, so
   nothing warns you.
4. **Order is arrival order, not chronology.** A union merge concatenates one
   branch's lines then the other's. Read the `Date` and `Task` columns, never
   the position.
...
| Date | Task | Note |
|---|---|---|
| 2026-08-19 | docs-01 | `autoloop/tests/test_docs_merge.py` added (12 tests, real git): parallel branches recording change notes merge without conflict, a source-file conflict still stops the sweep, and the union-on-a-giant-line duplication is verified rather than assumed. |
