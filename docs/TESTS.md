...
| 2026-08-22 | impl-01 | Review round 2 moved the adversarial self-test's fix boundary onto `Task.approved_paths`. `test_implement_executor.py` is now 11 adversarial-section functions (12 collected — one is parametrized over implement/revise): +3 new, 1 replaced, 6 amended. Hand-counted — the worker has no shell. |
| 2026-08-22 | impl-01 | The +3: a failure outside the approved paths is report-only, a file the DESCRIPTION names is in the prompt but not in the boundary, and a task with NO approved paths gets the fail-closed report-only wording rather than a boundary that quietly vanishes. |
| 2026-08-22 | impl-01 | `test_the_addition_widens_no_path_set` is replaced by `test_the_prompt_states_the_approved_paths_and_widens_no_path_set` — the old one asserted the paths were ABSENT, which is the behaviour review round 2 overruled. It now pins set-equality on the rendered lines, `TRACKER_PATHS` absent (the raw field, not `effective_approved_paths`), a sentinel path nowhere, and the Task unmutated. |
| 2026-08-22 | impl-01 | The description-mention test asserts BOTH halves — the unapproved path IS in the prompt (via the description) and is NOT in the boundary — because an absence-only assertion passes against a string that never reaches the prompt at all. |
| 2026-08-22 | impl-01 | `make_task` gained optional `approved_paths` / `description`, defaulting to the old values. The default is the EMPTY scope, so any boundary claim tested without passing one exercises the report-only branch instead; the section comment says so. |
| 2026-08-22 | impl-01 | Validation was NOT run inside the worker (no shell), so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. |
| 2026-08-22 | impl-01 | Review round 3 adds 4 functions to `test_implement_executor.py` (6 collected — one is parametrized over three misshapen `approved_paths` shapes), taking the adversarial section to 15 functions / 18 collected. Hand-counted; the worker has no shell. |
| 2026-08-22 | impl-01 | The +4: a well-formed list renders VERBATIM (the regression guard — the new validator must be the identity for real tasks); a malformed entry cannot write its own prompt line; a refused entry is flattened and length-bounded; a misshapen field falls back to report-only. |
| 2026-08-22 | impl-01 | `test_the_new_instruction_has_its_own_ceiling` now bounds the section's FIXED PROSE (`_fixed_prose` drops the indented path lines) and asserts it is identical for 1 and 40 paths. Round 2 bounded the whole rendered section at 600 with a ONE-path list, so a thirty-path task blew past it unnoticed. |
| 2026-08-22 | impl-01 | `test_a_task_with_no_approved_paths_gets_a_report_only_boundary` also pins the "listed below" ordering on the EMPTY branch. Round 2 pinned it only on the listed branch — leaving unchecked the one branch that used to render nothing at all. |
| 2026-08-22 | impl-01 | Review round 4 SUPERSEDES the round-3 count above: the adversarial section of `test_implement_executor.py` is now 17 functions / 20 collected (was 15 / 18). +2 functions, both going through the REAL `TaskRegistry.from_dict` via the new `_registry_from_stored_row` helper; one existing function renamed, none removed. Hand-counted; the worker has no shell. |
| 2026-08-22 | impl-01 | The +2: a bare-string `approved_paths` in a stored row is already split per character before this module sees it (so `_declared_paths` does NOT catch it, and the prompt states exactly the record's own entries and invents none), and `"approved_paths": null` raises `StateCorruptError` at load and never reaches a prompt at all. |
| 2026-08-22 | impl-01 | `test_a_misshapen_approved_paths_field_falls_back_to_report_only` is renamed `test_a_task_built_in_code_with_a_misshapen_field_cannot_crash_the_builder` and no longer uses `object.__setattr__`. It built a task with `make_task` and then assigned the field around the dataclass, which bypassed the very load path its docstring cited; it now constructs the `Task` the way the reachable case actually arises. |
