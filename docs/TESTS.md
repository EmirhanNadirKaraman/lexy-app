...
| 2026-08-24 | split-02 | New `autoloop/tests/test_task_split.py`: 50 test functions / 77 collected cases (hand count), real git, real worker repos and real on-disk stores throughout — the claim is about what is on disk after a crash, so a double proves nothing. Sections: the contract key, the intent record's shape and every malformed reading, the shared retirement label, the happy path, the four write boundaries, the reconciler's fail-closed endings, and the refusals. |
| 2026-08-24 | split-02 | The four that carry the claim are `test_crash_after_the_intent_and_before_the_registry_save_reconciles` and its three siblings — one per persistent write. Each drives the REAL dispatch with one store subclassed to raise immediately AFTER its write really landed, asserts the intermediate state on disk, then throws the process away (fresh `LoopState`, plain stores, registry re-read from `tasks.json`) and requires the next start to reach one consistent state. |
| 2026-08-24 | split-02 | `_Crash` derives from `BaseException` on purpose: every failure branch in `apply_split_intent` catches a NAMED exception type, so a crash any of them could catch would silently become a test of the error path, which has the opposite requirement (an error reports and keeps the intent; a crash reports nothing). The crash is armed AFTER the parent's implement round, since a task store armed at construction would die in the round that creates the record under test. |
| 2026-08-24 | split-02 | Two boundary tests are parametrized over all four boundaries rather than written once: one finishes the split through `run()` instead of a direct call, so `_step_ready` is shown to reconcile without anybody knowing to ask, and one adds a dependent and requires the `depends_on` rewrite to survive every boundary too. `test_reconciling_twice_changes_nothing_the_second_time` pins idempotence to a FIXED POINT, not merely once. |
| 2026-08-24 | split-02 | The fail-open shapes each get a test: an unreadable intent parks instead of reading as absent, an inapplicable one parks and is KEPT, a successor id occupied by a different task is refused instead of adopted, missing artefact stores are an obstacle instead of a skip, and `NoopQuarantine` — a manager that reports a move it did not make — proves the read-back verification is what decides completion rather than "no exception escaped". |
| 2026-08-24 | split-02 | `test_contract.py` and `test_context.py` both moved their copy of the `CONTRACT_INSTRUCTIONS` ceiling, 4,550 to 4,750, for the new `split_of` key; the accounting is in `test_contract_stays_within_its_budget`'s docstring, as its own rule requires. New `test_the_contract_instruction_ceiling_is_not_slack` bounds the HEADROOM as well as the length and prints the measured value on failure, so the next executor gets a reading instead of a hand sum. |
...
| 2026-08-24 | split-02 | Revision round: 5 test functions / 8 collected cases added to `test_task_split.py` for successor-field verification (hand count), plus an `approved_paths` assertion inside `assert_split_is_consistent` so all four crash-boundary tests now carry field fidelity too. An operator widening `t1a`'s `approved_paths` — and one editing `t1b`'s `depends_on` — after a boundary-2 crash must PARK and KEEP the intent rather than adopt the edit, with neither the registry nor either artefact repaired behind them. |
| 2026-08-24 | split-02 | `ScopeDriftingTaskStore` is the registry counterpart of `NoopQuarantine`: every write succeeds, the in-memory registry is exactly right, and only the FILE carries a wider scope, so it fails unless the read-back really reads the file. Two more pin the rewrite accounting in both directions — a successor declaring the parent must NOT park at any boundary (a literal comparison would park every one), and one deleted from a half-applied registry is re-created rather than parked on. |
| 2026-08-24 | split-02 | Validation: two advisory runs were requested inside the worker and NEITHER returned — both reached `PENDING` and never produced a `RESULT` within the round, so nothing here was executed by me. The five new tests and the two `orchestrator.py` helpers are HAND-TRACED against `_retirement_rewrites`, `_substitute_dependency`, `_check_acyclic` and `TaskRegistry.from_dict`, not run. Read this as no evidence either way; the executor's own run is the verdict. |
| 2026-08-24 | split-02 | Second revision round: `test_task_split.py` gains `test_a_successor_that_stopped_being_live_after_the_crash_is_not_adopted` (parametrized over retired/completed/shipped_elsewhere plus the UNRECOGNISED `Retired` and `retiredx` that a deny-list would have adopted) and `test_a_registry_that_persists_a_status_it_was_not_given_is_never_certified`, backed by a new `StatusDriftingTaskStore`. 2 test functions / 6 collected cases (hand count). |
| 2026-08-24 | split-02 | Both use the DEFAULT successors, which carry no `depends_on` — that is the case the check exists for. The first crashes at boundary 2, hand-edits `tasks.json`, restarts, and requires a park whose question NAMES the status, the intent kept, and the registry, execution record and worker repo all untouched. The second drifts only the FILE, so it fires on the final read-back after the artefacts have moved — the clearance half of the same check. |
| 2026-08-24 | split-02 | Third revision round: 2 test functions / 2 collected cases added (hand count). `test_a_registry_that_records_a_split_whose_artifacts_never_moved_is_not_reported_done` crashes at boundary 2, DELETES the intent by hand (the remedy the park message itself offers) and re-sends the directive: the loop must park naming both residues rather than answer SPLIT ALREADY APPLIED. Its sibling truncates `executions/t1.json`, so the record is unreadable rather than absent — the fail-closed direction, where swallowing the parse error would report the split as done. |
| 2026-08-24 | split-02 | Fourth revision round: 5 test functions / 7 collected cases added to `test_task_split.py` (hand count), plus a `park_codes()` helper and the `UnresolvableWorkerPath` / `MutePathAfterQuarantine` managers. A restart whose execution store or worker manager is `None` (parametrized three ways) parks `split_residue_uninspectable` with no SPLIT ALREADY APPLIED and no denial — crashed at boundary 4, so every artefact really HAS moved and the uninspectable store is the only thing that can park it. A boundary-3 sibling does the same for a `path_for` that raises. |
| 2026-08-24 | split-02 | The same raising `path_for` is pointed at `apply_split_intent` too — through a manager that goes mute only AFTER quarantine, since `retire_execution` calls `path_for` itself — proving the intent is KEPT rather than cleared over a worker nobody could name. A positive control pins the other direction: three agreeing stores still report the split, and a mixed case pins that SEEN residue outranks unknown residue and both are named. Two existing assertions moved off the prose "was never finished" onto the park code, so the new wording is not shaped to satisfy them. |
| 2026-08-24 | split-02 | Fifth round, and the validation gap the note above records is closed: an advisory run was requested inside the worker and RETURNED — `RESULT #1 … PASSED`, with `ruff check .` PASS, `python3 -m pytest -n auto autoloop/tests` PASS and `python3 -m pytest -n auto tests/` PASS — against the tree as it stood before this round's two new tests. A second run was requested afterwards to cover those; its outcome is in this round's report, and the executor's own run is still the verdict. |
| 2026-08-24 | split-02 | 2 test functions / 6 collected cases added to `test_task_split.py` (hand count). `test_an_intent_that_would_land_outside_its_directory_is_refused` parametrizes five unusable parent ids over `SplitIntentStore.save`; its sibling is the reachable version with no crash in it — hand-edit `tasks.json` to `../escapee`, restart, split it, and require a `split_intent_unwritable` park with no intent file anywhere, the parent still pending and no successor created. |
