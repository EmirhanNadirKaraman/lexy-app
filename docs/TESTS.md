...
| 2026-08-24 | dash-15 | `test_dashboard.py` gained a dash-15 section of 15 test functions / 15 collected cases after the merge-group tests (hand count, none parametrized), plus the helpers `described_rows`, `run_git_dated`, `merge_commit_repo`, `merge_panel_payload` and `render_merge_twice`. `_MERGE_TIME_CACHE` joined the autouse cache-clearing fixture and the dash-21 measurement's own cache tuple. One existing row-shape assertion was EXTENDED, never loosened: `set(row)` still pins equality, now over ten fields rather than six. |
| 2026-08-24 | dash-15 | The discriminating ones. `test_the_panel_asks_for_merge_times_once_however_many_rows` drives twelve merged rows through `collect` and counts `--grep` invocations, so a per-row implementation passes the unit test beside it and fails here. `test_a_partial_id_never_answers_for_a_branch` offers `dash-15`'s text to `dash-1`'s branch. `test_the_description_lookup_decides_no_state_and_no_group` compares every state, detail, sha and branch with descriptions and without. |
| 2026-08-24 | dash-15 | The page-side three RUN the lifted `MERGE_PANEL` region under node twice with a toggle between: an opened description survives the poll (the negative — closed on the first render — is what makes it mean anything), a hostile description reaches the DOM escaped and 6,000 characters whole, and an unresolved row renders its cells with no empty disclosure. `test_the_description_disclosures_are_bound_over_both_containers` is the fail-open pin — the stub has no DOM, so a binder scoped to `#merged` alone passes every node test while forgetting the merged group. |
| 2026-08-24 | dash-15 | Validation WAS run inside the worker through impl-02's advisory channel. Run 2 of 3 PASSED both configured commands — `python3 -m pytest -n auto -p no:cacheprovider autoloop/tests -q` and `ruff check .` — covering the implementation and all 15 new tests. Run 3 was spent on the final tree, these notes included. Run 1 is not evidence of anything: it was taken against a tree that could not import, and the defect was found by re-reading the file rather than from its answer. |
