...
| 2026-08-24 | release-01 | `autoloop/tests/test_worker_publisher.py` gained a section 6 for `release-blocked`: 15 test functions, 15 collected (hand count), plus two fixtures, one gateway stub and two helpers in neither figure. It lives beside the worker-repo primitives because the quarantine half of the retirement is `WorkerRepoManager.quarantine`, driven against real directories rather than a stub. No existing test is modified, which is what makes "`release` and `answer` are untouched" checkable from the diff alone. |
| 2026-08-24 | release-01 | The discriminating one is `test_release_blocked_frees_a_merge_window_a_bare_unblock_would_hold_shut`. It asserts the COUNTERFACTUAL first — requeueing the task alone removes the terminal-state exemption its `blocked` status was getting, so the live record starts naming it in `_merge_window_blockers` — and only then runs the real command and requires no reason at all. Without that first arm the closing `reasons == []` would pass just as well against a task that was never requeued. |
| 2026-08-24 | release-01 | The fail-closed negatives: a quarantine failure leaves the blocker OPEN and the task blocked (the record is still archived — the loud residue, since a surviving worker announces itself at the next dispatch), a `loop_fatal` record refuses before anything moves, an empty `--reason` refuses before the lock, and a COMPLETED task and an OPERATOR hold each refuse with both execution records still live. `test_release_and_answer_behave_exactly_as_they_did` is the unchanged-neighbours pin. |
| 2026-08-24 | release-01 | `test_release_blocked_never_uses_the_operator_reason_as_a_path_label` drives `../../etc/passwd  oops` through the command and requires the quarantine directory and the archive filename to carry neither the traversal nor a space, while the words themselves survive on the blocker's `archived_reason`. `..._tolerates_a_quarantine_with_nothing_left_to_retire` pins the absent case as a no-op that still SAYS what was missing, rather than silence a reader could take either way. |
| 2026-08-24 | release-01 | Four more came out of the adversarial pass rather than the plan, one per remaining failure point. A corrupt blocker record RAISES and must refuse, never read as "nothing open" (the fail-open shape). An archival that fails leaves the task quarantined. A requeue that cannot be completed REOPENS the record, asserted on `resolved_at` and `archived_reason` both being back. And a colliding label — time frozen, the same task retired twice inside one second — is retried under `<reason>-retry`, with both attempts' evidence intact on disk. |
| 2026-08-24 | release-01 | Revision round, +1: `test_a_repaired_record_path_names_this_retirement_not_an_older_retry` (section 6 now 16 functions). Three retirements of one task, the third failing its worker half once, so the reported `record_path` is repaired by reading the archive — where TWO label namespaces now live. It fails against a whole-name `sorted(...)[-1]`, which puts every `-retry-` file after every dated one, so the command named an older attempt's archive in the blocker's reason. Asserted on the printed path AND on `archived_reason`. |
