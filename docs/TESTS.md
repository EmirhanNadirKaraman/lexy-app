...
**584 tests (pre-blockers baseline — historical, not current; see the measured total above), fully hermetic** — no network, no ChatGPT, no playwright import,
...
| `test_smoke.py` | 16 | smoke-browser through the real CLI with a fake registered provider: full contract path (request id, stamped CONTEXT, parser, transcript), isolated smoke state, clean `stopped` terminal, FAIL path, executor provably never invoked, **a parked smoke session is archived not resumed**, and **no git write is reachable even from a commit-approval reply** (guard gateway). **Single-round-trip proofs:** the effective policy is one iteration / zero parse retries / zero denial retries / one failure, a malformed reply sends exactly one request and fails, exactly one submission and one response are transcribed, an audit executor or agent runner can never be constructed, and each run mints a fresh request id. |
| `test_smoke.py` (REWRITTEN 2026-08-25, brw-16) | 16 | **`smoke-browser` is retired, and this file pins that it refuses PLAINLY** — a stronger claim than "exits non-zero", and the reason the round-trip coverage above is gone rather than repointed: the command smoked the BROWSER transport, and none is registered. 13 functions, 16 collected (one parametrized over four argv shapes); hand-counted, no shell in the worker. Most of them assert ABSENCES, because the failure to catch is the command quietly becoming a generic smoke of `conversation.provider` — a different command wearing this one's name, reporting PASS about a transport nobody asked it for. So: `create_conversation`, `Orchestrator`, `PolicyEngine` and `GitGateway` are all boom-patched on `cli` and the refusal still prints; `LoopLock` is boom-patched too, and separately a foreign-host LOCK (live by definition, since `is_live` fails closed on a pid it cannot verify) is planted and the answer is still the refusal rather than `error: another autoloop process holds …`; a pre-existing `.autoloop/smoke/state.json` comes back byte-identical with no `state.json.bak-*` and no new sibling, since the old command archived on every run and a retired one that still did would destroy the last real smoke session's evidence; and the state dir is asserted not to EXIST afterwards, which covers the transcript, diagnostics and lock in one. Three cases prove no config is read — missing path, malformed TOML, and a `[conversation]` still naming the retired provider — each landing on the identical message rather than a `ConfigError` (`cli.main` maps that to exit 1, so 2-with-the-message is the observable difference). `test_it_never_exits_zero` is the fail-open guard: a wrapper, a cron job or the `doctor` + `smoke-browser` upgrade runbook would read 0 as evidence the transport was proven. Two more pin that the command is DISABLED rather than deleted — the subparser still parses to `_cmd_smoke_browser` with an accepted-and-inert `--provider`, and `--help` says RETIRED — because deleting it exits 2 as well, from argparse, on stderr, saying "invalid choice". The file no longer imports `autoloop.browser.chatgpt` (it used to, for `SubmitResult`), so the test tree has one fewer edge into the package this task disconnected. |
| `test_audit_findings.py` | 22 | Strict agent-output contract: field-by-field validation, per-item rejection with reasons, duplicate ids, non-JSON output, wrong top-level shape. |
| `test_audit_reconcile.py` | 8 | Bucket classification, cross-agent dedupe, speculation + style hard-rejected from promotion. **Dedupe semantics changed 2026-08-01** — it merges rather than keeping the higher-quality instance; preservation is pinned in `test_audit_compaction.py`. |
...
| `test_implement_executor.py` | 50 | `ImplementExecutor` with fake/stubbed agents: write-capable argv (Edit/Write allowed, Bash/Task disallowed), no `--model` flag (automatic selection), subagent `cwd` is the task's own worker repo not the main checkout, `changed_paths` derived from the worker repo's real `git status` and NOT from the agent's own claim (a fake agent claims a file it never touched — ignored), a filename with both a space AND a tab round-trips (`-uall`/`-z` NUL-safety), agent failure / no-files-changed / validation failure each `status="error"` without raising, success is `status="ok"` with `changed_paths`/`validation` populated, and nothing is written outside the worker repo (main checkout + `.autoloop/` marker both provably untouched); plus the audit/`None`-task defense-in-depth refusals and the `worker_repo_root_for`/`policy` constructor pairing contract. |
...
| `test_profile.py` (2026-08-20, prof-01) | 48 | **Measured durations, and the read-only `profile` command over them.** Two halves, kept apart because they fail differently. **The write half (`transcript.Stopwatch`)** is driven by an injected clock, so "a completed operation records how long it took" is an EQUALITY against a controlled elapsed time (`FakeClock(1000.0, 1002.5)` → exactly `2.5`), never a tolerance. Its four inert cases are the ones that matter: a clock that raises on construction, a clock that raises only on `stop()`, a non-finite reading, and a BACKWARDS reading each record NOTHING — the payload comes back byte-identical, with the key absent rather than negative — and the failure is LATCHED, so a second `stop()` whose reading would have been plausible still reports unknown (first stop wins for failures too, or a latched "unknown" would turn into a number on a retry). `stamp` is asserted not to mutate its argument, since the caller's dict is the event payload. **The read half** is pure functions over record lists. Statistics are pinned on a hand-computed set (count/median/mean/nearest-rank p90/total), an empty set is `None` and never zero, and `measured_duration` refuses a string, a bool, a negative, a non-dict `data` and a missing key — coercing any of them would make a fabricated number indistinguishable from a real one. The reader is tolerant per LINE against a file whose tail is a partial write plus a non-object and a typeless object, keeping the good records in front of them. **Provenance is the point of the remaining cases.** A MEASURED transcript reports per-stage stats and `executed` pairs with its `directive`; a LEGACY transcript (no duration anywhere, and `executed` carrying no `request_id` — the pre-prof-01 shape) still reports, labelled gap-derived, and a scan of the rendered text asserts no measured statistic appears anywhere in it. A MIXED transcript is the discriminating one: the measured request contributes 9.0s of real transport work, the unmeasured one degrades to its 400s window, and BOTH windows stay in the gap set — the two sources are independent sample sets over the same history, not competing estimates, so nothing is dropped to keep them apart (`test_a_measured_record_still_contributes_its_window_to_the_gap_set`, `test_a_measured_stage_is_never_folded_into_its_own_gap_number`: 480s measured inside a 499s window, on the same request, as two numbers). What separation IS asserted is a property of the rendered text — every statistics line carries exactly ONE of the two source labels, since a line carrying both would be the "same column" failure the task forbids — plus the one piece of arithmetic that would be wrong being printed rather than prevented (`test_the_report_says_not_to_sum_the_two_rows`). `review_wait` is asserted to have no measured form at all and to SAY so. Incomplete and hostile inputs contribute nothing rather than a zero: a start with no end, an end with no start, a second end for one request (first window wins), an unparseable `ts`, a record with no `request_id`, a backwards pair, and an empty transcript. **Eight CLI cases** run `cli._cmd_profile` against a real config file, plus one that `build_parser().parse_args(["profile"])` really resolves to it (`test_merge_sweep.py`'s wiring pattern — a command function nothing dispatches to is a command nobody can run): it prints every stage AND all five statistic labels, it says so when there is no transcript, it labels an all-legacy transcript, and — the read-only guarantee — it runs under a LIVE `LoopLock` with the state directory's bytes snapshotted before and after and asserted identical. Three cover `--transcript FILE`: an archived legacy file is profiled while the configured one is untouched, a missing override reports the overridden path rather than the configured one, and a file that is not a transcript at all yields "no usable records" with none of its content on stdout. That last one and the marked-packet case are the S36 controls: a transcript carrying a marked packet body and a marked reviewer reply must produce neither on stdout. (48 is a hand count of `def test_` — no Bash in the worker — not a collected figure.) |
...
> which `tests/test_document_package.py` (A2) requires for its bare
> `from services import …` imports; the console script omits cwd and that one
> file fails collection. Re-measured 2026-08-25: bare `pytest` collects
> **1,196 + 1 collection error**, that file holds **124** tests, and
> 1,196 + 124 = **1,320**, the full `python3 -m pytest` figure. (It was
> 1151 + 107 = 1258 when this note was written.)
...
That ledger stops at 894 and was not extended after 2026-07-29. Measured
2026-08-25 the backend suite **collects 1,320 tests** (`python3 -m pytest
--collect-only -q` from `lexy-app/backend`) — 426 beyond the last ledger row,
landed by work that did not update this table. Treat the rows above as history
and 1,320 as the current figure. (For the bare-`pytest` trap: that entrypoint
collects 1,196 plus one collection error — see `docs/COMMON_ERRORS.md` §2.)
...
| 2026-08-22 | impl-01 | +8 test functions in `test_implement_executor.py` (9 collected — the implement/revise case is parametrized) plus a `capture_prompt` helper, for `_ADVERSARIAL_SELF_TEST` in `_agent_prompt`: the instruction reaches both decisions, is bounded to the claim and forbids going fixing, a task with no decomposition still gets a well-formed prompt, prior feedback still lands, the cleanup section survives beside it in the documented order, and the tool tuples and ground rules are unchanged. |
| 2026-08-22 | impl-01 | Two of those pin what the addition must NOT do, which is the larger risk in adding an instruction. Tool names are asserted absent from the CONSTANT, not the assembled prompt — the ground rules legitimately say "no Bash access", so a prompt-level search matches that sentence and proves nothing. And the enumeration is asserted to reach `outcome.details` while `outcome.assumptions` stays exactly the one real `ASSUMPTION:` line: an adversarial-case list is line-per-item prose that could collide with `_ASSUMPTION_RE`, which would promote merely-considered cases into the durable record a reviewer reads most closely. |
| 2026-08-22 | impl-01 | `_agent_prompt` carries NO pinned budget and does not inherit `CONTRACT_INSTRUCTIONS`' 3,700 — that ceiling is justified in its own docstring as a per-turn tax on text re-sent every turn, while this prompt is built once per round for a fresh `claude -p`. Nothing was breached; the new constant gets a self-imposed 1,800 ceiling anyway, mirroring the contract's per-clause ceilings, because it is the part most likely to attract elaboration. |
...
| 2026-08-20 | dash-16 | +11 test functions in `test_dashboard.py` for the dependency graph's two display filters, plus three helpers (`dep_view`, `filter_rows`, `assert_layout_is_sound`) that are in neither figure. Hand-counted, no shell in the worker. |
| 2026-08-20 | dash-16 | What the 11 filter tests cover, toggles: each one removes exactly its own state's nodes and every edge incident to them, asserted for each toggle ALONE — a render reading one checkbox for both filters passes any test that only tries both off. |
| 2026-08-20 | dash-16 | What the 11 filter tests cover, layout and counts: the filtered graph is re-layered with no empty column and no gap in a column, and `shown + hidden + omitted == total` holds under all four combinations, with dash-14's `omitted + shown == total` still exact in the default view. |
| 2026-08-20 | dash-16 | What the 11 filter tests cover, survivors: a live task left with no drawn relation is kept and named, a partly hidden cycle is still reported over the survivors, and a phantom `unknown` id survives its only edge being hidden. |
| 2026-08-20 | dash-16 | What the 11 filter tests cover, display only: the registry's states and the `next_ready()` pick are unchanged by drawing the file. |
| 2026-08-20 | dash-16 | `filter_rows` carries `d-done-alone` — completed AND in no dependency relation — purely to make the hidden/omitted disjointness say something: it belongs to `omitted` in every view, so an implementation that swept "every completed task" into `hidden` breaks the identity in exactly one view and passes the other three. |
| 2026-08-20 | dash-16 | The cycle fixture holds two shapes at once because they fail in opposite directions: a three-node cycle whose middle member is completed (hiding it must NOT hide the surviving two-node cycle) and a two-node cycle that exists only through a completed node (hiding it takes the cycle with its edges, and reporting one anyway would describe a relation the drawing does not show). Termination is asserted by the test returning at all, as dash-14's cyclic test is. |
| 2026-08-20 | dash-16 | Two of those 11 RUN under node (the other nine are pure-payload or template-string tests): `renderDeps` against a stub document carrying the two checkboxes, one position at a time, asserting the page's own sentence (`4 drawn + 1 hidden + 2 in no relation = 7`) rather than payload fields — hiding has to be visible; and the refresh gate, where a filter must survive a tick that redraws (`.checked` untouched, hidden nodes still hidden) while a FOCUSED checkbox must not hold the redraw, paired with a focused node that still must. |
| 2026-08-20 | dash-16 | The "boxes are where the operator left them" assertion is what pins persistence, and it works only because the harness sets `.checked` itself and asserts the value back after four renders: `renderDeps` reads the boxes and never writes them, so there is no state to reconcile and nothing for a poll to reset. |
| 2026-08-20 | roadmap-01 | `autoloop/tests/test_roadmap_ancestry.py` added: 23 test functions, 33 collected with the parametrized whole-token set, every repository built with real git. A mocked git could not carry the file's central case — a commit that exists on a ref and is not an ancestor of the base head — because it would be defining the answer it is asked to prove. |
| 2026-08-20 | roadmap-01 | The `unknown` test asserts a NEGATIVE as well as a positive (`state != "not-in-base"`), since the whole failure being guarded against is a report that reads absence of a subject mention as evidence of absent work. |
| 2026-08-20 | roadmap-01 | The boundary rule is pinned in both directions: `pkt-03` is asserted to miss `pkt-030`, `x-pkt-03`, `pkt-03.5` and `pkt-03-followup`, AND `pkt-030` is asserted to still report shipped for its own commit — a tightening that broke real ids would otherwise pass the misses alone. |
| 2026-08-20 | roadmap-01 | The indeterminate cases use an INJECTED ancestor function (`shipped_states` takes one) rather than a contrived repository, because "git could not resolve this commit" is a state a healthy fixture cannot reach on demand — and it is the state the report is least allowed to round down to a negative. |
| 2026-08-20 | roadmap-01 | `test_the_search_returns_none_rather_than_empty_on_an_unreadable_repo` guards the seam that makes the distinction possible at all: `dashboard._run` collapses "git failed" and "git printed nothing" to the same `""`, so the search uses `_run_checked` — with `_run`, an unreadable repository would report every completed task as `unknown`. |
| 2026-08-20 | roadmap-01 | Read-only is asserted by mtime snapshot plus a clean `status --porcelain` after the report runs, not by inspection: this is meant to run beside a live loop, and a dirty checkout is what makes the escape detector park the next write-capable round. |
| 2026-08-20 | prof-01 | `autoloop/tests/test_profile.py` added (43 test functions), plus +4 in `test_orchestrator.py` and +2 in `test_implement_executor.py`. Every duration assertion is an EQUALITY against an injected clock, never a tolerance on a real one: a timing test that sleeps proves the machine was busy, not that the right interval was measured. |
| 2026-08-20 | prof-01 | The orchestrator regressions for `executed` use `build_postcommit` (worker repos present), not `build`, because production always takes `_execute_with_escape_detection` while `build`-based tests take the other arm of the same `if` — a stopwatch instrumenting only the `else` passes the whole `build` suite and records nothing in a real run. The trap is written up in `docs/COMMON_ERRORS.md`. |
| 2026-08-20 | prof-01 | The mixed-transcript test asserts a PER-LINE property of the rendered text — every statistics line carries exactly one of `measured`/`gap-derived` — rather than checking the two totals differ, because separate numbers in one column is precisely the failure that looks correct in a summary and is wrong in a report. |
| 2026-08-20 | prof-01 | `test_a_broken_timing_path_leaves_the_executor_outcome_unchanged` runs the REAL `ImplementExecutor` twice, once inside a stopwatch whose clock raises on every read, and compares the two `ExecutionOutcome`s — a stubbed executor would prove the stopwatch is inert around a stub, which is not the claim. |
| 2026-08-21 | prof-01 | The boundary regressions (+3 in `test_orchestrator.py`, so prof-01's total there is 7) work by making the loop CONSUME CLOCK READINGS after each operation returns: `BurningWatch` burns four at `stamp()` and `test_work_after_the_boundary_cannot_inflate_any_measured_duration` asserts all three durations are still one step, where an unfrozen watch would report five. Nothing else can tell a frozen window from an open one — with no post-boundary work in the way, both stop at the same reading and every assertion passes on the wrong code. |
| 2026-08-21 | prof-01 | Two boundary tests rather than one, because they fail differently: the burn test proves the freeze empirically at all three sites through the real phase machine, and `test_no_emit_site_stamps_a_watch_that_is_still_running` encodes the RULE (a stamped watch was already stopped) so a measured stage added later is covered before anyone writes a clock test for it. `test_real_post_send_bookkeeping_cannot_inflate_the_submit_duration` is the same claim at a production seam — `_client_send_outcome`, patched to cost ten readings — rather than at an injected one. |
| 2026-08-21 | prof-01 | `test_the_render_layer_receives_no_record_and_so_cannot_print_one` walks every value reachable from the `TranscriptProfile` and requires each to be a count, a flag, a float or a static stage label, because S36's claim is that the packet bodies are NOT IN REACH of the renderer — an output assertion ("the secret did not appear") passes just as happily on a renderer that holds them and happens not to print them, which is exactly the state the finding described before this. |
| 2026-08-21 | prof-01 | `test_a_stage_with_no_measured_samples_never_calls_the_transcript_legacy` needs a fixture that is modern AND has an empty measured stage — `ONE_TIMING_FAILURE`, a round whose `executed` carries no duration while the two events around it do. Neither `MIXED` nor `LEGACY_ROUND` can express it: in the first every stage is measured, in the second none are, and the false claim only appears in between. |
| 2026-08-21 | blk-01 | `test_blockers.py` gained a section 15 (+11 test functions, plus three helpers `_open_blocker` / `_transcript_entries` / `_split_brain` that are in no figure) for the invariant "a task is `blocked` only while at least one OPEN blocker names it": the last blocker's answer requeues the task, one of two leaves it blocked, an open `loop_fatal` record still counts, a registry already in the split state is repaired by `start`'s preflight and by the top of a real `_run_continuous` iteration, an operator hold survives every one of those, and the transition is asserted in the TRANSCRIPT because `unblock()` clears the reason it carried. |
| 2026-08-21 | blk-01 | Four of those eleven are the negative half and are the reason the rest mean anything: every blocker record is compared byte-for-byte (`asdict`, open and closed alike) across a sweep, since "do not resolve blockers as a side effect" is otherwise a claim nothing checks; an operator hold survives every automatic path; and the two `archive-blocker` refusals (a live `LoopLock` held in-process, and an unreadable lock file that reads as stale) assert on the BLOCKER — `resolved_at is None` — not just on the task, because "the task is still blocked" is equally true of the broken outcome where the archival landed and the requeue did not. |
...
| 2026-08-21 | blk-01 | Review round 3 added 6 more test functions to `test_blockers.py` (plus two helpers `_boom_reconcile` / `_boom_task_save` that are in no figure) for FAILING CLOSED: `answer` and `archive-blocker` each get a reconciliation failure (`KeyError` / `TaskGraphError`, injected after the lock is taken and after the record is closed) and a real `TaskStore.save` failure, and each asserts all four halves — non-zero exit, the record restored byte-for-byte via `asdict`, the task still `BLOCKED_BY_OPERATOR`, and NO `task_auto_unblocked` transcript entry — plus that the success line ("resolved." / "archived at") was never printed. |
| 2026-08-21 | blk-01 | Two of those six are the boundary rather than the rule: a restore that itself cannot be written must SAY the record is closed and the task was not requeued instead of raising (the injected `BlockerStore.save` fails only on a record with `resolved_at is None`, so the close under test still happens), and `start`'s preflight must stay TOLERANT of the same unreadable graph — exit 2 with `UNREADABLE` reported, not a traceback, because a startup sweep has no blocker of its own to put back. |
| 2026-08-21 | blk-01 | The two reconciliation-failure tests end by undoing the injection and re-running the command, so the reopened record is proven still answerable/archivable — a restore that left it unusable would be the same split brain one file over. `test_an_operator_hold_is_never_swept_back_into_the_queue` had its transcript assertion narrowed from "no entries at all" to "no entry names `held`" in the same round: `answer` now releases through the shared reconciliation, so the answered task legitimately gets one. |
| 2026-08-21 | blk-01 | `test_a_reopen_that_itself_fails_says_the_record_is_still_closed` asserts a NEGATIVE as well as a positive — `"NOT resolved" not in out` — and reads the record back to show the answer really is on disk. An earlier draft of that test asserted both "could NOT be reopened" and "NOT resolved", which pinned a contradiction: the branch where the restore fails is the one branch where the close stands, and an operator told the answer was not recorded would answer a blocker that `resolve` then refuses a second time. |
| 2026-08-21 | ctx-01 | `autoloop/tests/test_context.py` gained 15 functions for the per-task in-flight rows and the merge-window line: every rendered field, the summary counts held byte-identical, an unreadable record listed as unknown rather than dropped, no rows with nothing in flight, and a five-positional-argument `build_context` call rendering the pre-change block. |
| 2026-08-21 | ctx-01 | The merge-window half is pinned at the SEAM rather than by re-deriving an answer: a recorder monkeypatched over `cli._merge_window_blockers` asserts the config and the caller's gateway are what it receives, and separate tests drive the real predicate for the open case and for a real candidate record holding it shut. A reimplementation would pass a behavioural test and fail this one, which is the point. |
| 2026-08-21 | ctx-01 | Two tests own bounds rather than behaviour, because the block is re-sent every round: the addition measures 437 characters for the two-in-flight shape (capped at 600), and the rendered rows are asserted to contain no imperative while `CONTRACT_INSTRUCTIONS` gains no text and stays under 3,700 — the same ceiling `test_contract.py` pins from its own side. |
| 2026-08-21 | ctx-01 | Validation was NOT run inside the worker for this task: it has no shell, so `ruff check .`, a bare root `pytest` and `cd lexy-app/backend && python3 -m pytest -n auto` are the three commands owed to the orchestrator. Test counts here are hand-counted, and the 437-character figure is a hand sum of line lengths that the 600 cap deliberately leaves room around. |
| 2026-08-21 | ctx-01 | `autoloop/tests/test_orchestrator.py` gained `test_prompt_names_the_in_flight_task_and_what_holds_the_merge` — the PRODUCTION wiring, which no `test_context.py` test reaches: `_step_ready` is the only place that hands `build_context` both the execution store and the config, so dropping either argument leaves all 15 renderer tests green while the loop keeps sending the block that caused the misschedule. |
| 2026-08-21 | ctx-01 | That test's `build()` helper gained an optional `execution_store` parameter (default `None`, so every other caller is unchanged), and it creates the worker repo so `cli._candidate_is_retired` answers from the filesystem rather than reaching a `FakeGit` that has no `read_commit`. |
| 2026-08-21 | ctx-01 | These six ctx-01 note lines sit at the END of the ledger because `note_merge.resolve_note_append` requires each side's section to START with the base's section byte for byte: an earlier head merge had left them above lines mainline already carried, which is an insertion rather than an append and refuses every later parallel merge of this file. |
| 2026-08-21 | merge-04 | `test_merge_window.py` grew 32 → 43 for the narrowed hazard test: a base equal to the head still blocks and the reason says so, a base git confirms is a PROPER ancestor becomes a note naming task/candidate/base, and four fail-closed cases block with their own distinguishable reasons — a `merge-base` git refuses to answer, a base placed OUTSIDE the head's history, a record naming no base, and a head the checkout will not name. |
| 2026-08-21 | merge-04 | Three of those 11 are the "unchanged" pins rather than the new behaviour: an already-behind candidate that is PUBLISHED still gets the publication note, one whose record was RELEASED still gets the retirement note, and one already-behind record beside a sibling bound to the head leaves the window SHUT — the mutation that runs the ancestry check before the two older exemptions fails the first two, and the one that treats any already-behind record as opening the window fails the third. |
| 2026-08-21 | merge-04 | `_FakeCheckout` gained `is_descendant`/`ancestors`/`descendant_error`/`refs` and `_FakeRemote` gained `head_sha`/`is_descendant`, both raising `GitCommandError` on a sha the fake does not hold — as real git does. That is deliberate rather than convenient: the existing fail-closed cases now stay green because git cannot place their unresolvable bases, not because an `AttributeError` escaped, and only `(GitError, OSError)` is caught in production so a gateway method that stopped existing would crash instead of silently disabling the exemption. |
| 2026-08-21 | merge-04 | `test_merge_sweep.py` grew 62 → 64, both real git: a candidate bound to a base the head has already moved past lets the WHOLE backlog through in one pass (three branches merged and pushed, `SWEPT`, no deferral, and the gate's note still names the record), and a candidate bound to the head as it stands AFTER a move still defers the sweep — so the narrowing is not a one-time amnesty that trades a permanently shut window for a permanently open one. |
| 2026-08-21 | merge-04 | Review round 2: `test_a_candidate_at_the_head_STILL_shuts_the_gate_after_the_head_moves` was failing on its own fixture, not on the behaviour — `Backlog.commit_on_base` commits without pushing, so `b.origin_base()` was still the init commit while `before` was the moved local head, and `b.origin_base() == before` could never hold. The moved head is now pushed so both ends start equal. |
| 2026-08-21 | merge-04 | Review round 2: that same test now carries a SECOND outstanding branch, because the gate is evaluated once against the pre-sweep head — an at-head candidate has to keep BOTH branches off the base, since letting the first land would move the head past that record's base and hand the second the new already-behind note, the exemption granting itself one merge at a time. |
| 2026-08-21 | merge-04 | Review round 2: what that test asserts — local and remote base byte-for-byte at their pre-sweep shas, neither candidate an ancestor of the head, nothing reached the merger, and the at-head record present as a blocking reason and in no window note. |
| 2026-08-21 | blk-02 | `test_blockers.py` gained a blk-02 section (+18 test functions, plus one helper `_severity_pair` that is in no figure) for primary-blocker selection: loop_fatal outranks task_fatal however the directory lists them, no number of task_fatal records outranks one loop_fatal, newest wins within a kind, identical timestamps fall back to the blocker id and repeat calls agree, an unparseable stamp sorts oldest, an unrecognised kind ranks with loop_fatal, resolved and archived records are never selected, nothing open yields `None`, and a corrupt record still raises rather than being ranked around. Hand-counted, no shell in the worker. |
| 2026-08-21 | blk-02 | Every multi-blocker fixture is built so filename order AND recency point at the WRONG record — `blk-t-a-001` (task_fatal, newer) sorts before `blk-t-b-001` (loop_fatal, older) — because the incident's own ids do not discriminate: `(` sorts before `b`, so `blk-(loop)-038.json` was listed first and a test built from that pair passes with `open_blockers()[0]` left in place. `_severity_pair` asserts the glob order it depends on, so the fixture fails loudly if that ever stops being true; the real incident pair is kept as a second test. |
| 2026-08-21 | blk-02 | Four tests pin the no-change-when-one-is-open bound BYTE-EXACTLY rather than by substring — `health.check`'s whole `detail` and `summary` strings, `_report_blockers_and_phase`'s first four lines, and `_print_blocker_summary`'s output asserted to contain no "primary" at all — because a "+N more open" suffix leaking into the common case would trade one wrong impression for another and every substring assertion would stay green. |
| 2026-08-21 | blk-02 | Integration tests cover the three surfaces separately from the store, so deleting the central selector or reintroducing a caller-local sort cannot leave the suite green: `test_health.py` (+2) drives the real `health.check` over the incident fixture, and `test_blockers.py`'s operator-facing group drives `_report_blockers_and_phase`, `_cmd_blockers` and `_print_blocker_summary` — the last of which takes a LIST, which is where a second `sorted()` would creep back in. `test_health.py`'s `_blocker` helper gained keyword-only `task_id`/`kind`/`question`/`when` with its old defaults, so its existing call sites are unchanged. |
| 2026-08-21 | blk-02 | `test_heartbeat.py` gained `test_the_beat_reports_a_count_and_never_picks_one_blocker`, which pins a NON-change: the heartbeat publishes a count plus the session's own question with two blockers open, so it has nothing to order. It is what makes naming one there a deliberate decision later rather than an accidental second opinion. |
| 2026-08-21 | blk-02 | Validation was NOT run inside the worker for this task: it has no shell, so `ruff check .`, a bare root `pytest` and `cd lexy-app/backend && python3 -m pytest -n auto` are the three commands owed to the orchestrator. Test counts here are hand-counted. |
| 2026-08-21 | blk-02 | Review round 2: the four reported failures (`test_a_held_candidate_renders_the_reason_it_holds_the_merge`, `test_the_addition_stays_inside_its_per_round_budget`, `test_no_scheduling_advice_moved_into_the_context_block`, `test_prompt_names_the_in_flight_task_and_what_holds_the_merge`) are NOT caused by any blk-02 change — none of this task's code is reachable from `context.py`, `orchestrator.py` or `cli._merge_window_blockers`. |
| 2026-08-21 | blk-02 | Review round 2: those four are an inherited mainline conflict — merge-04's `cli._candidate_base_ancestry` calls `gateway.is_descendant` (`cli.py:3402`) catching only `(GitError, OSError)`, while ctx-01's `FakeGit` doubles in `test_context.py` and `test_orchestrator.py` implement `head_sha` and no ancestry method. |
| 2026-08-21 | blk-02 | Review round 2: the `AttributeError` that follows escaped into `context._merge_window`'s broad catch, so every one of those tests read `merge_window: unknown — AttributeError…` instead of `shut — …`. |
| 2026-08-21 | blk-02 | Review round 2: fixed in the fixtures, not the assertions — `detailed_store` gained a keyword-only `base` defaulting to its old `0 * 40` (so every other caller is byte-unchanged) and the three tests that run the REAL predicate bind their record to `FakeGit.head`. |
| 2026-08-21 | blk-02 | Review round 2: that is what "holds the window shut" has meant since merge-04 — `BASE_AT_HEAD` is decided by string equality and returns before any `is_descendant` call, so no fake git answer is invented. The held-candidate record's candidate became `c * 40` so its candidate and base are distinguishable shas, and that test now asserts both. |
| 2026-08-21 | blk-02 | Review round 2: the two `FakeGit`s still lack `is_descendant`, so a future test needing `BASE_BEHIND`/`BASE_UNVERIFIED` will hit the same trap — out of scope here, and named rather than papered over. |
| 2026-08-21 | blk-02 | Review round 2: `test_the_addition_stays_inside_its_per_round_budget` keeps its `0 < added <= 600` assertion untouched; only its documented measurement moved, 437 → ≈528 characters (93- and 97-character rows plus a 338-character merge-window line, up from 245), because each reason now carries merge-04's ancestry clause. Hand-counted, as the original 437 was. |
| 2026-08-22 | codex-01 | `autoloop/tests/test_codex_app_server.py` added: 48 test functions, 56 collected with the three parametrized sets, no codex binary anywhere. Hand-counted — the worker has no shell. `test_codex_provider.py` is UNCHANGED at 33: the subprocess adapter is not modified by this task, and a green run of that file is part of what "this adds a provider and deletes none" means. |
| 2026-08-22 | codex-01 | The fake app-server is strict on purpose: `turn/start` naming a thread it never opened is an `AssertionError`. Without it the shared-context test passes for a transport that opens a thread per turn, which is exactly the design being ruled out — an assertion the transport must EARN rather than one the fake grants. |
| 2026-08-22 | codex-01 | The reviewer double answers a verdict question by naming every part id the THREAD has seen, never the ids in the message it was handed. That is what makes the delivery test evidence rather than a round trip: a reply listing all five parts can only come from earlier turns in one shared context. |
| 2026-08-22 | codex-01 | `test_prose_that_mentions_a_usage_limit_is_not_exhaustion` is the load-bearing half of the structured-error claim; the five positive cases would pass against a substring matcher too. |
| 2026-08-22 | codex-01 | `test_a_turn_that_fails_after_appending_leaves_the_prompt_on_the_thread` DEMONSTRATES the reason `idempotent_submit` is False rather than asserting the attribute twice: it reads the appended prompt back off the server while this process holds no reply, which is the double-post a retry would cause. |
| 2026-08-22 | codex-01 | The protocol pin parses the real `ClientRequest`/`ServerNotification`/`ServerRequest` unions out of `docs/codex-app-server-protocol.generated.ts` rather than hard-coding a copy, so regenerating that file against a newer codex-cli is what breaks it. Its twin asserts the `v2/` param bodies are still ABSENT from that file, which is the evidence for keeping those field spellings tolerant instead of pinned. |
| 2026-08-22 | codex-01 | Two claims are deliberately untested because they are not made: no test asserts a thread survives a process restart, and none asserts a sandbox preset is enforced. Both are codex-03, and a test here would be the overclaim rounds 2-5 were refused for. |
| 2026-08-22 | codex-01 | Validation was NOT run inside the worker for this task: it has no shell, so `ruff check .`, a bare root `pytest` and `cd lexy-app/backend && python3 -m pytest -n auto` are the three commands owed to the orchestrator. The backend suite is untouched by this change (nothing outside `autoloop/` and `docs/` was edited). |
| 2026-08-22 | codex-01 | The pin's NEGATIVE half now asks the reference's declared `"method"` literals, not its raw text. Round 1 of this file searched the whole file and could never pass: the reference opens with a header comment naming the four invented methods while explaining they do not exist, which a substring search cannot tell apart from a declaration. |
| 2026-08-22 | codex-01 | `wire.py` no longer spells those four names at all, so a grep of `autoloop/codex/` for an invented method comes back empty; naming them is the test's job, where the assertion is that the reference declares no such method. |
| 2026-08-22 | codex-01 | `test_answer_refuses_unknown_and_already_resolved` (test_blockers.py) was fixed, not written, by this task: an INHERITED environmental dependency, unrelated to the codex provider. Its blocker code maps to `_precondition_browser`, which runs a real doctor sweep — an HTTP probe of `browser.cdp_url` plus a playwright import — so it passed only while the operator's dedicated Chrome answered. |
| 2026-08-22 | codex-01 | That precondition is now neutralised by `monkeypatch.setitem` on the `_RESOLUTION_PRECONDITIONS` entry rather than by swapping the blocker to a precondition-free code, so the scenario stays the one the test describes. `DoctorProbes.probe_cdp` binds its default at class-creation time, so patching the module global — the pattern two neighbouring tests use — does not reach it. |
| 2026-08-22 | codex-01 | The same test now writes its own `tasks.json` under `tmp_path`, because with the precondition passing `_cmd_answer` reaches `_load_tasks`, which otherwise falls back to reading the repository's real `autoloop/seed_tasks.json`; and it asserts the first answer returns 0, so a re-refused precondition fails at the line that explains it rather than as a missing StateError two lines later. |
| 2026-08-22 | preempt-01 | `autoloop/tests/test_urgent_preemption.py` added: 43 test functions, 48 collected with the six-way parametrized unsafe-phase set, plus four helpers (`build_loop`, `in_flight`, `_audit_round`, `_task`) that are in neither figure. Hand-counted — the worker has no shell. No existing test file is modified, which is part of what "review, approval and publication are untouched" means here. |
| 2026-08-22 | preempt-01 | The headline test reproduces the 2026-08-21 failure exactly rather than asserting the fix abstractly: both tasks at P0, `next_ready()` asserted to pick `brw-13` BEFORE and AFTER a P0 re-prioritisation of `codex-01` (the id tiebreak), and only then does the pin move it. A test that started from unequal priorities would pass against the design that already existed. |
| 2026-08-22 | preempt-01 | The safe boundary is parametrized over all six non-`ready` phases plus `ready`-with-a-pending-request, and each asserts the FULL non-effect — status still in_progress on disk, worker repo still present, execution record still loadable, phase not stopped — because "returned False" alone is equally true of a preemption that ran and failed. Its paired positive (`test_the_wait_ends_at_the_boundary_rather_than_being_forgotten`) is what stops a `_preempt_for_urgent` that always declines from passing the whole group. |
| 2026-08-22 | preempt-01 | Two structural tests carry claims a behavioural one cannot: `_self_upgrade_due` and `_preempt_for_urgent` must both call `_at_round_boundary` (a second copy of the predicate would pass every behavioural test and drift later), and `_preempt_for_urgent`'s source must contain `release_task_to_pending(` and NOT `registry.release(` — a re-implemented release is the 2026-08-21 defect, and it is invisible to an outcome assertion that only checks the status moved. |
| 2026-08-22 | preempt-01 | "The gate is untouched" is tested as an ABSENCE in the preemption's own source (`verify_review`, `report_sha256`, `push_exact`, `_dispatch_task_push`, `mark_completed`, `reviewed_commit`) plus the pause flag never existing on disk after two overlapping requests — the deadlock claim is about a lock never taken, so the honest form of it is that no such file is written and no such symbol is reachable. |
| 2026-08-22 | preempt-01 | `test_an_idle_loop_is_not_preempted_in_a_circle` exists because the obvious implementation loops forever: a pin observed with nothing in flight would end the session, the next selection would start a fresh one, and it would reach the same boundary with the same pin. It asserts the non-action, which is why `_displaced_work_exists` has a test at all. |
| 2026-08-22 | preempt-01 | `test_the_session_a_preemption_starts_does_not_preempt_itself_in_a_lap` drives the SECOND lap, not the first: preempt, then dispatch an audit into the session that preemption started (which opens on the audit kickoff), and assert nothing is displaced. Written after review caught that the first cut displaced audit rounds while the dispatch gate deliberately let them through — one round burned and one quarantined worker per lap, ending only when the reviewer happened to pick `implement`. A test of the audit case alone passes against that bug. |
| 2026-08-22 | preempt-01 | Validation was NOT run inside the worker for this task: it has no shell, so `ruff check .`, a bare root `pytest` and `cd lexy-app/backend && python3 -m pytest -n auto` are the three commands owed to the orchestrator. The backend suite is untouched (nothing outside `autoloop/`, `docs/` was edited). |
| 2026-08-22 | preempt-01 | Second round. `test_urgent_preemption.py` grew a section 2b and a release-failure group: 14 new test functions (57 functions, 62 collected with the unchanged six-way parametrized unsafe-phase set, hand-counted — the worker has no shell), plus one new helper `_next_session` and one new stub class `FlakyWorkerRepos`. One existing test was re-worded rather than re-scoped — `test_the_session_a_preemption_starts_does_not_preempt_itself_in_a_lap`'s docstring — because its body still holds while its stated REASON no longer did; a green test documenting the opposite of the code is worse than no test. |
| 2026-08-22 | preempt-01 | `test_nothing_is_dispatched_between_the_preemption_and_the_urgent_task` is the end-to-end regression the review asked for, and its shape is the claim: `_dispatch_task_postcommit` is replaced by a recorder, so the assertion is on the ORDERED LIST of units that reached the executor. It is empty after the preemption, still empty after an `audit` and an unrelated `implement` are both attempted, and then exactly `["codex-01"]`. An assertion that only checked the urgent task ran would pass against an audit having run first. |
| 2026-08-22 | preempt-01 | `test_the_urgent_kickoff_asks_for_a_directive_policy_will_authorize` runs `PolicyEngine.authorize_directive` on the bare `implement` the kickoff solicits, both with and without a stored decomposition, and pins the payload sentence to each answer. Without it the kickoff could look correct while inviting a directive policy denies — audit churn traded for denial churn, which ends a run rather than costing it a round. |
| 2026-08-22 | preempt-01 | `FlakyWorkerRepos` raises `GitCommandError` from `quarantine`, which is what a colliding label actually raises and is NOT an `OSError` — the shape that used to escape the preemption's own `except (TaskGraphError, StateError, OSError)` and reach the loop's git handler instead of the record. `failures=1` proves the retry under `<reason>-retry` recovers; `failures=2` proves the partial is reported as a partial, with the status still moved. |
| 2026-08-22 | preempt-01 | The repair has a test on BOTH sides of its gate, which is the whole point of the gate: `test_a_resumable_residue_is_left_paired_so_the_next_dispatch_continues_it` monkeypatches `worker_repo_is_reusable` to True and asserts the RESTORED record is the original one (its `candidate_sha`, not a fresh stub — a placeholder would satisfy "a record exists" while losing the pointer to the commit being kept); `test_a_worker_the_next_dispatch_cannot_resume_is_left_split_and_named` runs the REAL probe against the plain directory the fixture builds and asserts the record stays archived, so the merge window is not shut for a dispatch that would refuse anyway. |
| 2026-08-22 | preempt-01 | Not covered, and stated rather than implied: no test drives a real dispatch after a partial release, because that needs a git-backed worker and this file deliberately runs without one. The resumable branch is pinned at the boundary it can be — the restored record plus the probe the dispatch path itself calls — not end to end. Nothing tests the consequence either: a restored record carries `review_round` and `attempt_count` into the resumed dispatch, which is intended (it is the same unit of work) but is a behaviour no assertion holds. |
| 2026-08-22 | preempt-01 | Correction to the earlier lap-test note above, which says the session a preemption starts "opens on the audit kickoff": it no longer does. The test body is unchanged and still passes — it calls `_preempt_for_urgent` directly — but the parenthetical is stale, and `test_a_preempted_session_opens_on_the_urgent_task_not_on_the_audit` is now what pins the real behaviour. |
| 2026-08-22 | preempt-01 | Third round. `test_urgent_preemption.py`: 59 functions, 64 collected — 57/62 plus two. Hand-counted, the worker has no shell. |
| 2026-08-22 | preempt-01 | One of those two is parametrized over `contract.RETIRED_DECISIONS`, so it contributes ONE case only while that set holds one member (`ask_user`); the collected figure moves with it. |
| 2026-08-22 | preempt-01 | The +2 are for the kickoff template's new home in `cli.py`: `test_the_urgent_kickoff_is_as_strict_as_every_other_template` and `test_the_urgent_kickoff_offers_no_retired_decision`. |
| 2026-08-22 | preempt-01 | Those two re-run, against `cli.URGENT_KICKOFF`, exactly the two checks `test_prompts.py` runs across `TEMPLATES` — otherwise moving a template out of that dict would silently move it out of its coverage, a worse trade than the scope violation it fixes. |
| 2026-08-22 | preempt-01 | The named regression was `test_recovery_commands.py::test_the_record_is_retired_before_the_worker`, and the diagnosis is one line: `release_task_to_pending` swallowed `GitError` and returned a partial `Release`, so `_cmd_release` printed and exited 1 where that test asserts `pytest.raises(GitCommandError)` — DID NOT RAISE. Fixed by making the report the OPT-IN (`tolerate_retirement_failure`, passed only by `_preempt_for_urgent`) rather than the new default. That file is untouched. |
| 2026-08-22 | preempt-01 | `test_release_command_reports_a_retirement_it_could_not_finish` is accordingly renamed to `test_release_command_still_raises_but_the_status_move_is_already_durable` and re-scoped to the corrected semantics: the error propagates and the record is archived (which the recovery-commands test already pins), while what is pinned HERE is what only the shared path made visible — the status is durably `pending` on the way out of that raise, the `<reason>-retry` attempt runs on the CLI path too, and no record is restored beside the survivor, since `_repair_orphaned_record` must not shut the repository-wide merge window behind a traceback. |
| 2026-08-22 | preempt-01 | Validation was NOT run inside the worker again this round — it has no shell — so `ruff check .`, `python3 -m pytest -n auto autoloop/tests -q -p no:cacheprovider` and `python3 -m pytest -n auto tests/ -q -p no:cacheprovider` are owed to the orchestrator, and the claim that the regression is fixed is a reading of the two files, not an observed green run. |
| 2026-08-22 | preempt-01 | Fourth round, fix-only. The ruff finding is F811 in `test_urgent_preemption.py`: a second `from autoloop.contract import RETIRED_DECISIONS` below the import block that already names it. Removed; counts stand at 59 functions, 64 collected, since a duplicate import defines no test. |
| 2026-08-22 | preempt-01 | The third-round 767-character note above is split into four short lines, substance unchanged and nothing deleted. Only lines this task wrote were touched; the other preempt-01 notes are left as they are. |
| 2026-08-22 | preempt-01 | Correction to the round-three validation note above: the three commands are `ruff check .`, a bare root `pytest` (whose `testpaths` covers `tests` and `autoloop/tests`) and `cd lexy-app/backend && python3 -m pytest -n auto`. That note narrowed the middle one into two per-tree invocations and dropped the backend suite; the round-one note has it right. Still not run in the worker — no shell. |
| 2026-08-22 | val-03 | `autoloop/tests/test_validation_failfast.py` added: 19 test functions, 20 collected with the two-way parametrized timeout/missing-binary set, plus two helpers (`runner_failing`, `named`) that are in neither figure. Hand-counted — the worker has no shell. |
| 2026-08-22 | val-03 | The two halves are asserted together on purpose: that the later commands never reach the runner (the saving) AND that the summary still names them `NOT RUN` (the evidence) — a test of either alone passes against a broken runner. |
| 2026-08-22 | val-03 | Five more pin what must NOT change: a one-command passing run's summary asserted BYTE-EXACTLY (`"ruff check .: PASS"`, so an unconditional ordering note could not slip onto every round's `state.last_validation`), a failure in the LAST command adding no note, an empty list still passing, a refused binary still launching nothing, and the summary naming neither `[audit]` nor `validation_run` (the first draft told the reviewer to set a key this task decided not to ship). |
| 2026-08-22 | val-03 | `test_neither_production_call_site_overrides_the_default` is structural — it reads `implement_executor.py` and `orchestrator.py` for the string `fail_fast`, because "production gets the default" is a claim about callers passing NOTHING. |
| 2026-08-22 | val-03 | One existing test changed: `test_validation_parallelism.py::test_a_parallel_run_still_reports_pass_fail_per_command` now passes `fail_fast=False` — its subject is parallelism, and its third and fourth commands would legitimately be `NOT RUN` under the new default — which also gives the full-run mode an exercise from a test that predates it. |
| 2026-08-22 | val-03 | Validation was NOT run inside the worker (no shell), so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched. |
| 2026-08-22 | val-03 | The before/after timing figure the brief asked for could not be measured here either — `~/.autoloop/analysis/timing.py` is outside the worker repo. What IS defensible is the arithmetic: a round refused on a docs or lint violation paid ruff + both full pytest suites + the serial `isolated` re-run against a 1282s median round, and now pays the first failing command alone. |
| 2026-08-22 | val-03 | Review round 2 re-cut every docstring and note in this change for LENGTH; review round 3 split this file's two remaining overlong val-03 notes and `docs/SUMMARY.md`'s 872-character one into short lines. No test removed, no assertion weakened, counts above unchanged. |
| 2026-08-22 | val-03 | Review round 4: `test_the_report_says_it_stopped_and_how_to_run_everything` now asserts the note reports the CONFIGURED order preserved and ADVISES cheapest-first, instead of asserting the old "cheapest first" claim. No test added or removed; the shipped-order tests (which are template-scoped and verifiable) are untouched. |
| 2026-08-22 | impl-01 | Review round 5 takes the adversarial section of `test_implement_executor.py` back to 8 functions / 9 collected (the implement/revise case is parametrized), matching the earlier impl-01 count above. The nine tests rounds 2–4 added for the rendered approved-path list — its entry validation, its injection case, its misshapen and `from_dict` shapes — are removed with the machinery they covered, along with the `_fixed_prose`, `_sole_line_containing` and `_registry_from_stored_row` helpers. Hand-counted; the worker has no shell. |
| 2026-08-22 | impl-01 | What survives pins the claim itself: the instruction reaches implement AND revise, is bounded and forbids going fixing, names no list or document that could dangle, leaves the tool tuples / ground rules / cleanup section / prior feedback intact, works with no decomposition, keeps its 1,800-char ceiling, and carries an `ADVERSARIAL CASES:` enumeration into `outcome.details` without polluting `outcome.assumptions`. |
| 2026-08-22 | impl-01 | `test_the_prompt_reads_no_scope_field_and_renders_no_path_list` replaces round 2's rendering test and asserts EQUALITY of two prompts differing only in `approved_paths`: an `in`/`not in` check passes against a builder that reads the field and renders it somewhere the test did not think to look. `make_task` is back to its original one-argument form. |
| 2026-08-22 | impl-01 | Validation was again NOT run inside the worker (no shell): `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-22 | impl-01 | Review round 6 takes the adversarial section of `test_implement_executor.py` to 13 functions / 15 collected (two are parametrized over implement and revise). `test_the_prompt_reads_no_scope_field_and_renders_no_path_list` is removed — its property (the prompt is independent of `approved_paths`) is what the reviewer instructed be reversed. Hand-counted; the worker has no shell. |
| 2026-08-22 | impl-01 | The load-bearing addition is `test_the_rendered_scope_list_is_exactly_the_effective_approved_paths`: it PARSES the path lines back out of the assembled prompt and asserts equality with a fresh `effective_approved_paths(task.approved_paths, TRACKER_PATHS)` call, for both an implement and a revise round. Comparing the renderer against itself on both sides would pass against any rendering bug, and a membership check would pass against a list carrying something extra. |
| 2026-08-22 | impl-01 | Five more pin the surface: the prompt and the gate read the same tracker source (`Orchestrator._tracker_paths()` == `TRACKER_PATHS`), two tasks get their own lists and neither carries the other's paths, an unscoped task gets the fail-closed `APPROVED SCOPE: none` branch, a hostile entry is harvested by neither `_extract_assumptions` nor `_extract_cleanup_requests`, and a 60-path scope is rendered whole. |
| 2026-08-22 | impl-01 | `test_a_hostile_scope_entry_cannot_open_a_line_or_forge_an_instruction` exercises BOTH echo shapes, because they need different controls: an entry containing a newline (closed by escaping) and one that merely BEGINS with `ASSUMPTION:`/`REMOVE-OUT-OF-SCOPE:` (closed by the `- ` bullet, which both regexes refuse). The first cut of this test placed the anchor mid-line only, so it passed while the property did not hold — the leading-anchor case is the one worth keeping. |
| 2026-08-22 | impl-01 | Updated rather than added: the grants-nothing test now loops the disallowed tools over `_SCOPE_HEADING`, `_SCOPE_TRAILER` and `_SCOPE_NONE` as well as the instruction; the ceiling test bounds the fixed scope prose (<= 1,200) beside the instruction's 1,800 while deliberately leaving the rendered list uncapped; the cleanup test pins that the residue list and the scope list stay separate. |
| 2026-08-22 | brief-01 | `autoloop/tests/test_docs_merge.py` no longer defines its own 700; it imports `note_merge.MAX_NOTE_LINE_CHARS` and gained `test_the_implementing_agents_brief_states_the_limit_this_file_enforces`, which asserts the executor brief states that same number with the AT MOST wording, the boundary case, the whole-line measure and the append-a-second-line remedy. Enforcement and brief now read one name in one module, so a re-hardcoded copy has to break one of those two tests to exist. |
| 2026-08-22 | brief-01 | `autoloop/tests/test_implement_executor.py` gained 6 functions / 7 collected (the implement-and-revise case is parametrized) for the MECHANICAL AUTHORING RULES section, plus one `_rules_section` helper in neither figure. Hand-counted; the worker has no shell. |
| 2026-08-22 | brief-01 | The load-bearing one is `test_the_stated_limit_follows_the_constant_the_validator_reads`: it monkeypatches `note_merge.MAX_NOTE_LINE_CHARS` to 123 and asserts the assembled prompt says 123 and no longer says 700. That is the only assertion able to tell a shared constant from a hard-coded copy which happens to agree today, and it is why the renderer reads the module at call time rather than importing the name. |
| 2026-08-22 | brief-01 | The rest pin the surface: the rules reach a real `execute()` round on implement AND revise without displacing the feedback, the named files come from `NOTE_TRACKERS` sorted, an EMPTY tracker set still renders a rule with a subject instead of an empty parenthesis, an unscoped task gets them after the fail-closed scope branch, and the section grants no disallowed tool, is harvested by neither extractor and stays under 1,200 characters. |
| 2026-08-22 | brief-01 | The brief test also pins the SIBLING mechanical rule of the same section — call the marker CHANGE-NOTES in prose, never write the comment out again — because `notes_section` in the same file requires it exactly once per tracker and a duplicate disables auto-resolution silently. Two mechanical rules from one test file are stated; the rendered section is still under its 1,200-character ceiling. |
...
| 2026-08-23 | notes-03 | `test_docs_merge.py` now covers FOUR trackers: 34 test functions, 49 collected cases (hand count; was 30 functions). Three are new — a source conflict alongside four otherwise-resolvable trackers, a tracker missing its marker, and a note written the old way — and five are parametrised over every path in `TRACKERS`, so the next tracker granted is covered without editing them. |
| 2026-08-23 | notes-03 | The shipped-file shape check is now the pure `section_problems(name, text)`, which lets `test_a_note_written_the_old_way_fails_the_shape_check` require a doctored copy to be REJECTED rather than only requiring the real files to pass. Renames: `test_both_trackers_end_with_an_append_only_change_note_section` → `test_every_shipped_tracker_ends_with_an_append_only_section`, and the scope test → `test_the_resolver_is_scoped_to_exactly_the_declared_trackers`. |
| 2026-08-23 | notes-03 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | dash-18 | `test_dashboard.py` gained 19 tests for the merge panel's third evidence source, in one section after the merge-group tests (hand count). Thirteen are pure `merge_states` / `naming_ancestor` cases — ancestor decides, `detail` distinguishes all three sources, no match, non-ancestor, `unknown` verdict, `None`/`[]`/`()` search, empty id, whole-token `pkt-03` vs `pkt-030`, ref and record precedence, first-match order, ancestry asked only about matches. The other six drive real repositories through `collect`. |
| 2026-08-23 | port-01 | New `autoloop/tests/test_state_dir_location.py`: 10 tests (hand count; the worker has no shell), every one driven through the real `load_config` on a config written at `<checkout>/.autoloop/config.toml` — a hand-built `AutoloopConfig` proves nothing about a DEFAULT. Pins the move beside `workers_root`, an explicit value honoured verbatim, the `workers_root`-name collision refusal, the one legacy read and its new-wins precedence, and that a pre-existing hand-built config is unchanged. |
| 2026-08-23 | port-01 | The two load-bearing ones: `test_every_path_the_loop_writes_resolves_under_the_state_dir` reflects over `AutoloopConfig`'s Path properties (minus four external by design) plus `LOCK` and the three publisher paths, so a path property added later that forgets the rule cannot slip past a hand-listed set; and `test_writing_the_state_dir_is_not_something_the_escape_detector_sees` drives the REAL detector and asserts the old in-checkout location still trips it, so a green run cannot mean the test lost its grip. |
| 2026-08-23 | port-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | loop-03 | `test_dashboard.py` gained 35 test functions / 40 collected cases for the dashboard's self-replacement, in one section at the end (hand count; the worker has no shell), plus two autouse fixtures. `no_process_replacement` mirrors `test_self_upgrade.py`'s and is what makes "not exec'ed" a real assertion; `_clean_upgrade_state` resets the in-flight count, the armed flag and the per-sha attempt memo, without which test order becomes load-bearing. |
| 2026-08-23 | loop-03 | The load-bearing three. `test_a_replacement_never_interrupts_a_response_in_flight` holds one request inside `collect` while a second runs to completion and asserts no exec happened; `test_nothing_from_the_record_reaches_the_relaunch_command` drives `--evil-task`, `--evil-path` and a shell metacharacter through a real attempt; `test_a_stale_dashboard_ends_up_serving_the_new_code` runs a real child on a COPY of this package, edits that copy, arms the signal and waits for the served page to change. |
| 2026-08-23 | loop-03 | Three existing assertions in `test_the_summary_renders_at_the_top_of_the_page` were updated, not weakened: `renderStats` now takes the process's own currency as a second argument, so the two "could not be read" panels can say which fault the operator has. `test_self_upgrade.py` is deliberately untouched — "the loop still neither finds nor signals the dashboard" is proven by those tests passing unchanged. |
| 2026-08-23 | loop-03 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | dash-18 | The load-bearing negatives: `test_a_match_git_could_not_resolve_is_not_read_as_integration` (the check must be `== "yes"`, never `!= "no"`), `test_a_failed_search_and_an_empty_one_can_never_move_a_row`, and `test_an_unreachable_origin_stays_unknown_even_with_the_commit_in_the_branch`. Plus a read-only snapshot test around the new `git log --all`, a cache test pinning one walk per two polls, an assertion that the walk is skipped outright when the remote is unreadable, and `_SUBJECT_CACHE` added to the autouse cache-clearing fixture. |
| 2026-08-23 | quota-01 | `test_codex_provider.py` gained 35 test functions (hand count; the worker has no shell). Three existing tests changed with the signature, one was renamed, and the exhaustion-wording parametrization split in two, because `"rate limit exceeded"` and `"429 Too Many Requests"` classify as transient now and not as a spent allowance. |
| 2026-08-23 | quota-01 | The transcript tests read `config.transcript_file` off disk rather than counting calls on a double. That is the whole point: `codex_invocation_failed` was written on every failure for 24 days and reached the transcript zero times, so a mock-call assertion would have been green throughout. `test_classification_cannot_be_called_without_the_prompt_it_sent` is the other structural pin — `inspect.signature` must report no default for `prompt`. |
| 2026-08-23 | quota-01 | The three loop-level tests drive a REAL `CodexConversation` in the orchestrator's client seat. A double that raises `QuotaExhaustedError` on demand can show what the orchestrator does with one; it cannot show that an echoed review packet stops producing one, which is the claim. |
| 2026-08-23 | notes-04 | New `autoloop/tests/test_base_refresh_notes.py`: 23 test functions, 29 collected cases (hand count; the worker has no shell), real git and real worker repos built the way production builds them. Pins that a head conflicting only in change-note sections merges INTO the task branch with both sides' notes intact, while tracker prose, an edited note line, a source file, an unlisted doc and a tracker with no marker each still park with the existing message and the existing `task_base_behind_head` code. |
| 2026-08-23 | retire-01 | `test_tasks.py` gained 23 test functions / 23 collected cases in two new sections after the written-once group (hand count; the worker has no shell). They pin the refusal naming EVERY direct dependent, the transitive count beside it (printed even when nothing is behind them), completed and retired dependents excluded and not descended through, a quarantined one still counted, a live successor re-pointing every dependent, and a task with no dependents retiring exactly as before. |
| 2026-08-23 | retire-01 | The load-bearing negatives, all of them fail-open shapes: `test_the_strand_check_survives_a_dependency_naming_a_task_that_is_gone` (the graph `state_of` raises `KeyError` on — a crash or a swallowed exception here would pass the very input it must refuse), `test_asking_about_an_unknown_id_is_refused_rather_than_answered_empty`, `test_a_rewrite_that_would_build_a_cycle_leaves_nothing_applied`, and `test_the_rewrite_flag_on_an_already_retired_task_is_refused_not_ignored`. |
| 2026-08-23 | retire-01 | The atomicity test is the cycle one: `_check_acyclic` runs on the whole candidate AFTER every dependent has been planned, so it fails at the last possible moment and then asserts the retirement, both dependents' `depends_on` and the successor record are all unchanged. `test_an_in_progress_dependent_refuses_the_whole_operation` pins that NEITHER route forces a rewrite under a running dispatch. `test_the_rewrite_and_the_retirement_persist_as_one_save` covers the store round trip. |
| 2026-08-23 | retire-01 | Two existing tests changed rather than being deleted. `test_a_task_depending_on_a_retired_one_stays_blocked` asserted the behaviour this task removes; it is now `…_is_blocked_with_no_way_out`, building the shape through `from_dict` (how a pre-retire-01 `tasks.json` holds it) and pinning WHY the refusal exists — `unblock` and `release` both refuse it and only `set_depends_on` gets out. `test_policy.py`'s retired-denial test passes `rewrite_dependents=True`, since its `blocked1` depends on the id it retires. |
| 2026-08-23 | retire-01 | `test_recovery_commands.py` gained 4 CLI tests in a new section after the retire group, and `_retire_args` a `rewrite_dependents` parameter. They pin exit 1 with every dependent named and the transitive count printed, both flag names offered, nothing written on refusal, `ingest-01 now depends on roadmap-02` echoed from the registry rather than assumed, the drop-the-edge path, and `dependents: none` on an ordinary retirement so silence never reads as an unrun check. |
| 2026-08-23 | retire-01 | Correction to the `test_tasks.py` row above, in its task-retire-01 paragraph. Two claims there are superseded: `retire` no longer "refuses only `completed`", and "a task DEPENDING on a retired one stays BLOCKED … a real consequence rather than an oversight" is the belief this task overturned — it is still what `state_of` does, but a NEW retirement may no longer create it. The test named there was renamed and rewritten accordingly. The row is append-only and unedited; read it with this line. |
| 2026-08-23 | retire-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | notes-04 | The load-bearing one is `test_a_refreshed_task_branch_still_merges_back_out_through_auto_merge`: it refreshes a base, lets mainline record one more note, then merges the task branch back out through the REAL `AutoMerger._resolve_note_conflicts` and requires all three notes to survive. Only that round trip catches a refresh that ordered the notes so the branch can never merge again; `test_the_wrong_ordering_really_would_break_the_next_merge` is its unit counter-case. |
| 2026-08-23 | notes-04 | `test_docs_merge.py` and `test_auto_merge.py` are UNCHANGED, deliberately — the task-to-mainline direction had to keep passing untouched, and `auto_merge._resolve_note_conflicts` delegates without altering one refusal string, the commit-message suffix or the `auto_merge_notes_resolved` payload. Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | quota-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | dash-18 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | quota-01 | `test_codex_provider.py` gained 19 more test functions, an `ECHO_TRANSFORMS` table of 15 reshapings and a `WIDENED` pattern list; 63 further collected cases (hand count; the worker has no shell). Three parametrizations run every reshaping — at the classifier, against the widened list, and end-to-end through the orchestrator — so the transforms the review named (newlines, repeated whitespace, wrapping, punctuation) are each a case rather than a claim. One existing signature test grew a fourth function. |
| 2026-08-23 | quota-01 | `WIDENED` is deliberately the worst list an operator could plausibly write: both built-in lists plus the bare words the 2026-08-22 stopgap removed plus `"limit"`, which occurs in the packet and so must be suppressed for all 15 reshapings. `test_the_bound_that_carries_the_property_is_suppression` then SHOWS which bound does the work — a hard-wrapped echo leaves `'quota` on a line the content floor keeps, where only suppression can refuse it. |
| 2026-08-23 | quota-01 | Two are about the TESTS, not the code, and are the ones to keep. `test_every_echo_transform_actually_changes_something` fails any case that silently reduces to identity — green while covering nothing is the fail-open shape of a suite. `test_the_non_breaking_space_case_is_really_one` asserts the U+00A0 that is invisible in the source; a reader cannot tell it from the ordinary space beside it, and neither can a reviewer. |
| 2026-08-23 | quota-01 | `test_anything_that_can_match_is_first_suppressible` is the property the guard rests on, tested over both default lists and every reshaping: `_fold(needle) in _fold(text)` implies `_squeeze(needle) in _squeeze(text)`. `test_the_line_bound_is_stricter_than_the_property_above_covers` is the bridge — a folded line is a contiguous run of the folded whole, so the property covers the stricter per-line matcher too rather than a matcher the code no longer uses. |
| 2026-08-23 | quota-01 | Two tests for the marker no side ever held. `test_two_echoed_lines_cannot_synthesise_a_marker_across_the_join` asserts the join really does manufacture `quota exceeded` and the prompt really does not hold it, under both normalizations, before asserting the verdict. `test_a_marker_split_across_two_lines_of_codex_output_is_not_a_match` isolates the line bound from the prompt guard — neither line is attributable — and pins that the same words on ONE line still classify. |
| 2026-08-23 | quota-01 | The half a false-park fix quietly breaks is pinned twice: `test_a_genuine_exhaustion_survives_a_marker_laden_echo` and its loop-level twin. Both put a packet that QUOTES exhaustion wordings in flight and require codex's own message to still park, with the record naming both the deciding marker and the refused one. A clean-prompt-only version of that test would pass while the guard suppressed everything. |
| 2026-08-23 | quota-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | quota-01 | Correction, not a new claim: `test_the_bound_that_carries_the_property_is_suppression` asserted `suppressed == ("quota",)` while `classify` runs the spent list AND the transient one, and `MARKER_PROMPT` also quotes `429` and `rate limit`. It now asserts membership under production defaults and exactness only with `rate_limit_patterns=()`. The TEST was wrong about the semantics; `quota.py` is unchanged, because weakening the guard to satisfy an assertion is the wrong repair. |
| 2026-08-23 | quota-01 | `test_codex_app_server.py` gained 13 test functions and a `TRANSIENT_ERRORS` table of 6 shapes (hand count; the worker has no shell) for the transient-vs-spent split. `test_exhaustion_is_read_off_a_named_field` lost its three throttle cases to that table, which asserts at the predicate AND at the transport seam that a 429 raises a `BrowserError` and never `QuotaExhaustedError`. `test_a_failure_digest_is_bounded_and_carries_no_packet` grew the new `classification` key. |
| 2026-08-23 | quota-01 | Four are the discriminating ones. `test_spent_wins_when_one_error_carries_both` pins the precedence on a `usage_limit_reached` type WITH a 429 status. `test_the_two_vocabularies_do_not_overlap` stops the check order deciding whether a loop parks. The last two are the config mistake in both directions: an explicit `quota_error_codes` entry is obeyed even for a throttle (a false park), while a spent marker misfiled as transient still parks (never a missed one). |
| 2026-08-23 | quota-01 | Validation for THIS round was not run in the worker either, same reason. Every assertion added or changed here was hand-traced against the code instead — the round it corrects died on one untraced tuple assertion, which is exactly the failure mode a hand trace is for. |
| 2026-08-23 | retire-01 | Revision: 5 more `test_tasks.py` functions plus a `corrupt_self_edge` helper. Four split the subject's self-edge from what the cycle check must still refuse — two pin that such an edge no longer vetoes a retirement REACHING `_check_acyclic` (via a live successor, and via the rewrite flag), one pins a self-edge on an untouched task still refusing with nothing applied, one pins `old-01 -> dep-01 -> old-01` dissolving through the rewrite rather than the carve-out. |
| 2026-08-23 | retire-01 | The fifth is the one that matters most: `test_a_self_edge_on_a_stored_graph_is_refused_at_load_not_at_retirement` pins all three routes in refusing a self-edge, `from_dict` included — it runs `_check_acyclic` over the whole stored graph. The round before this shipped one test built on the opposite premise, and it raised at `from_dict` before asserting anything — a failure, not coverage. Every case is now corrupted IN MEMORY through the helper, which says in its docstring why that is the only way. |
| 2026-08-23 | ship-01 | New `autoloop/tests/test_shipped_elsewhere.py`: 57 test functions, 80 collected cases (hand count; the worker has no shell), in ten sections mirroring the claim — dependents satisfied, evidence validated, re-recording, the mutators that must refuse, the merge sweep, the inbox, the continuous re-check, the dashboard, the CLI, and the eight measured records end to end. Ancestry questions use real git repositories; a mocked `merge-base` cannot show a record going stale when the base moves under it. |
| 2026-08-23 | ship-01 | The load-bearing pair is in the sweep section. `test_the_sweep_does_not_enumerate_a_shipped_elsewhere_task` runs with a `_RefusingMerger` that raises on ANY attempt, so "it was not asked for a branch" is an assertion rather than an absence, and requires `NOTHING_TO_DO` with `is_clear` true. Its twin adds a genuinely completed task whose record names no candidate plus a mergeable one, and requires HELD with the mergeable branch left pending — the unresolved rule, unweakened. |
| 2026-08-23 | ship-01 | The fail-open tests are the point of the re-check section. `test_an_indeterminate_ancestry_check_is_unverified_never_verified` pins both directions of the `unknown` verdict: never rounded UP to verified (the alarm switching itself off when it cannot see) and never DOWN to a disagreement (every shallow clone reporting the roadmap as broken). `test_record_shipped_refuses_when_git_cannot_answer` is the CLI half of the same rule. |
| 2026-08-23 | ship-01 | `test_no_mutator_can_silently_overwrite_the_record` is parametrized over eleven registry entry points (`mark_completed`, `mark_in_progress`, `block`, `operator_block`, `retire`, `set_decomposition`, the three setters, `request_urgent`, `release`) and asserts the shas survive each refusal. Each of those ends in a bare status assignment or a field rewrite, so a status added without an arm loses the record while leaving its evidence on the row saying nothing. |
| 2026-08-23 | ship-01 | Two existing `test_dashboard.py` tests were updated, not weakened, because a seventh `TaskState` reaches them by construction: the exact-equality counts and the copied `summary()` line in `test_the_counts_are_what_state_of_reports_and_nothing_else` now carry `shipped_elsewhere`. `test_every_task_state_is_claimed_by_exactly_one_bucket` and the DMARK/DFILL/DORDER pin needed no edit and are what forced `STAT_BUCKETS` and the page's icon tables to move with the state. |
| 2026-08-23 | ship-01 | `test_all_eight_measured_records_can_be_expressed` is deliberately ASYMMETRIC and that is the assertion: five become verified records that satisfy inbox-03 / inbox-04 and leave the queue, three stay `completed` and come back as disagreements — one proven, two unproven. A test that recorded all eight the same way would be testing the auto-conversion this task exists to refuse. |
| 2026-08-23 | ship-01 | Two tests came out of the adversarial pass rather than the plan, and they pin the refusal of a LOOP-raised quarantine: one shows the orphaned-blocker case refusing with `autoloop answer` named, the other (`test_a_crafted_hold_reason_cannot_launder_a_loop_quarantine`) parks with a reason that BEGINS with `OPERATOR_HOLD_PREFIX` and requires it still to refuse — provenance is the stored field, never the free text the loop also writes. |
| 2026-08-23 | ship-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | ship-01 | Revision round. The two sweep tests in `test_shipped_elsewhere.py` were asserting against a sweep that never ran: `PolicyConfig()` defaults `auto_merge_enabled` off and the flag check is the first thing `sweep()` does, so both got `disabled`. The fixture now sets it True. Worth knowing generally — an assertion like `unresolved == []` would have PASSED on that same short-circuit, so a sweep test has to pin the outcome too. New §2 entry in `docs/COMMON_ERRORS.md`. |
| 2026-08-23 | ship-01 | Revision round. `test_task_inbox.py::test_no_payload_field_falls_outside_both_per_kind_sets` is the drift guard that caught the new `shipped_elsewhere` payload field belonging to neither per-kind set; it is now in `inbox.MUTATION_ONLY_FIELDS`. Nothing validates against that union — `_check_creation` / `_check_mutation` hold the real rules — so this widens no request's shape and only earns the field a "mutation-only" hint on the wrong kind. |
| 2026-08-23 | ship-01 | Revision round. `test_dashboard.py::test_an_unreachable_origin_stays_unknown_even_with_the_commit_in_the_branch` (dash-18) counts `commit_subjects` calls and requires ZERO when the remote is unreadable. Adding the disagreements panel to `collect` made a second consumer run that walk; `collect` now decides it once and hands the result down. The test was not touched — it is the bound, and it caught exactly what it was written to catch. |
| 2026-08-23 | stop-01 | New `autoloop/tests/test_stop_livelock.py`: 32 test functions, 38 collected (one parametrized over six malformed ledgers, one over two empty-reason shapes; hand count, the worker has no shell), in seven sections mirroring the claim — the park itself, one stop unchanged, what "the same situation" means, reset on progress, `answer` clearing it, the adversarial pass, and the ledger. No existing test file is modified. |
| 2026-08-23 | stop-01 | Every scenario drives REAL rounds through a `Loop` helper that rebuilds state, registry and Orchestrator per session, exactly as `cli._run_continuous` does. A test that compared two hand-built fingerprints would assert on its own input; `test_the_fingerprint_is_identical_across_three_real_rounds` reads back what the orchestrator actually computed, because a digest churning per round would disable the park with nothing saying so. |
| 2026-08-23 | stop-01 | That test runs WITH a live `TaskExecution` record carrying a candidate, which is the incident's own shape — an empty store would only prove the digest holds still when there is nothing to churn, and the whole `asdict` of that record is digested by raw bytes. Its empty-store half is kept beside it, and `test_the_registry_term_is_a_fixed_point_across_a_load_and_dump` pins the other input: `from_dict` COERCES, so a round trip that were not a fixed point would churn the term silently. |
| 2026-08-23 | stop-01 | The load-bearing negatives are the fail-open shapes. `test_an_unusable_ledger_parks_rather_than_disabling_the_check` and its directory-shaped twin cover both the read and the write side; `test_a_park_without_a_blocker_store_still_parks` pins that an unwired store costs the RECORD and never the park. `test_stops_about_genuinely_different_situations_never_park` and its same-wording twin pin both directions of "text is not what makes a situation the same". |
| 2026-08-23 | stop-01 | `test_the_task_identity_term_actually_carries_the_current_task` exists because the first cut of the fingerprint read `current_task["id"]` — the `Task` dataclass's field name, never the key either writer uses — so that term silently contributed nothing while looking exactly like a working one. It asserts three distinct digests for no task, t1 and t2, and that an id-less dict reads as "no task" rather than raising. |
| 2026-08-23 | stop-01 | `test_needs_attention_is_true_after_the_park` asserts `health.check(...).needs_attention` rather than only that a blocker file exists — the claim's own wording, and the signal that stayed green through the incident. `test_answer_clears_the_park_through_the_ordinary_command` runs the real `cli._cmd_answer`; the session stays parked afterwards, which is how every loop_fatal park recovers and is asserted rather than glossed. |
| 2026-08-23 | stop-01 | `test_a_wrongly_typed_ledger_is_refused_rather_than_used` is parametrized over six shapes no writer produces, chosen for six different ways of going wrong: `"3"` raises `TypeError` from a `>= 3` comparison inside `_dispatch`, where nothing in `run`'s type-keyed handler chain catches it; `True` is an `int` in Python and answers False forever; `0` and `-1000` postpone the park silently. Its integration twin proves the round PARKS rather than dying. |
| 2026-08-23 | stop-01 | Validation was NOT run inside the worker — it has no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-23 | stop-01 | Revision round. `test_a_stop_with_an_empty_reason_still_parks_and_says_so` was scripting `stop_block("")`, a shape the contract refuses (`_require_str`), so the round spent a parse retry and then asked for a scripted response that did not exist. It now drives three unchanged empty-reason stops through `_dispatch` — the entry `test_blockers.py` already uses — and still parks on the third, quoting the `(no reason recorded)` fallback. Neither the fallback nor any stop behaviour changed. |
| 2026-08-23 | stop-01 | Revision round. That test is now parametrized over `""` and `"   \n  "`, because the two fail differently: the blank one is TRUTHY, so a bare `or` fallback would print the whitespace and produce the same unreadable question while looking handled. `_park_stop_livelock` strips before falling back, and the `blank` case is what pins it. |
| 2026-08-23 | stop-01 | New `test_the_contract_never_delivers_an_empty_stop_reason` pins WHY that one cannot be a full round: an empty `reason` is `missing_field:reason`, never a `Directive`. The fallback therefore guards a directive handed straight to `_dispatch`, and nothing else can reach it — `StopRepetitionStore.observe` rewrites `last_reason` on both branches, so a stale empty value on disk cannot survive into a park. If the contract ever accepts a terse stop, that test fails and the other becomes a real round. |
| 2026-08-23 | stop-01 | Also new: `test_the_park_ends_continuous_mode_instead_of_opening_another_session` runs the real `cli._run_continuous` over the state the park actually wrote and requires exit 2 with the parked session still on disk. A blocker record alone does not prove the claim's second half — the loop must stop OPENING SESSIONS — and `_run_continuous` is the function whose reaction to a `stop` was the bug, so it is driven rather than reasoned about. |
| 2026-08-23 | impl-02 | New `autoloop/tests/test_agent_self_validation.py`: 30 test functions, 31 collected (one parametrized over the passing and failing validation paths; hand count, the worker has no shell), in seven sections mirroring the claim — no argument exists, it is the round's OWN validation, the cap, the honest non-results, the executor still deciding, the reporting, and the published descriptor. No existing test file is modified. |
| 2026-08-23 | impl-02 | The load-bearing three. `test_an_agents_claim_that_it_ran_the_suite_is_not_evidence_that_it_did` feeds a report boasting five green runs and requires the summary to say NOT OFFERED and 0 while the boast still reaches `details` intact. `test_the_agent_facing_call_takes_no_arguments_at_all` asserts `inspect.signature(...).parameters == ['self']` — the property is the ABSENCE of a parameter, not a validated one. `test_the_advisory_run_and_the_executors_own_run_launch_the_same_thing` compares recorded argv and cwd from a real round. |
| 2026-08-23 | impl-02 | The fail-open negatives: an empty command list is NOT RUN and never a pass (`run_validation_commands` returns True for one, which is right for the executor's accounting and catastrophic as an answer to the agent), a missing working directory likewise, a launcher that raises is a FAILURE with `last_run_ok` False, and a zero or negative cap refuses every request. `test_the_last_run_flag_follows_the_real_result_not_the_first_one` runs green-then-red AND red-then-green, since a first-wins bug passes either half alone. |
| 2026-08-23 | impl-02 | Three came out of the adversarial pass rather than the plan. `test_a_second_request_arriving_mid_run_is_already_counted` re-enters `run()` from inside the launcher to pin that the cap is committed BEFORE the run — a counter bumped afterwards would admit any number of overlapping runs. The other two pin that a malformed `Task.validation` says NOT RUN instead of raising, since `_advisory_for` now runs on the agent-failure path, which returned before that field was ever read. |
...
| 2026-08-23 | impl-02 | Revision round: `test_agent_self_validation.py` gains sections 8 and 9 (19 further tests; hand count, no shell here). Section 8 is what proves the claim and did not exist before — `RendezvousAgentRunner` uses NOTHING but Write (to ask) and Read (to collect), the two tools a real implement subagent holds, and whole `execute()` rounds are driven through it. It never imports the executor's internals, so if it can get a result, so can a `claude -p` agent. |
| 2026-08-23 | impl-02 | The three end-to-end ones that carry the claim: an agent really runs the configured suite and the recorded argv/cwd are the executor's; it sees a FAILURE, fixes it and confirms green inside one round (the loop this task exists to close); and the cap refuses its second ask with NOT RUN while the executor still runs its own. Two more require `changed_paths` to carry no trace of the channel, on the success path AND on the agent-failure path, which reads the tree earlier. |
| 2026-08-23 | impl-02 | Section 9 is the adversarial pass on the rendezvous itself: a hostile JSON payload in the request file changes nothing about what launches; a DIRECTORY at the request path is swept (Write creates parent dirs); a symlinked request costs the link and never its target; stale files from a killed round are swept before the agent starts; `stop()` sweeps when `start()` never ran; a request landing after the round ends is not served. |
| 2026-08-23 | impl-02 | `test_the_second_answer_cannot_be_read_as_the_first_ones_leftovers` pins the ordinal stamp — without it the second ask's answer is the first's, since the agent cannot delete the result file. `test_the_pending_marker_never_reads_as_a_finished_result` reads the file from inside the launcher, mid-run, to prove PENDING is distinguishable by the agent's stated test. Two existing tests were updated, both because the channel is now really offered. |
| 2026-08-23 | impl-02 | Revision round: validation was again NOT run inside the worker — no shell — so `ruff check .`, a bare root `pytest` and the backend suite are owed to the orchestrator; the backend suite is untouched. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way. |
| 2026-08-24 | scope-05 | New `autoloop/tests/test_scope_revert.py`: 31 test functions, 44 collected with the four parametrized sets (hand count), in six sections mirroring the claim — the claim itself, the decided REMOVE/REVERT overlap, what stays refused, fail-closed inputs, no new agent capability, and the restore at unit level. Real git and a real `ImplementExecutor` driven through a real `Orchestrator`, like `test_scope_cleanup.py`, because the claim spans the prompt, the restore and the record. No existing test file is modified. |
| 2026-08-24 | scope-05 | The DISCRIMINATING check is `test_the_reverted_path_leaves_the_reviewed_range_entirely`, not the byte comparison beside it: it asserts the path IS in `git diff --name-only base candidate` after round 1 and is NOT after the repair, which is what the feature is for. A content-only assertion passes while a stale mode bit keeps the path in the range forever — hence `test_the_executable_bit_is_restored_with_the_bytes`, which also requires `git status --porcelain` to come back empty. |
| 2026-08-24 | scope-05 | The fail-open negatives are the point. `test_an_unreadable_base_tree_reverts_nothing_and_never_deletes` uses a well-formed sha this repository does not hold, so the capability IS offered and the restore then cannot resolve — the one case where falling through to "absent at the base" would convert every request into a deletion. `test_an_unusable_base_sha_offers_nothing_and_reverts_nothing` is parametrized over empty, raising, non-hex and flag-shaped values, and requires the prompt not to mention the form at all. |
| 2026-08-24 | scope-05 | Two more carry rules that are easy to get backwards. `test_a_revert_survives_a_round_whose_validation_then_fails` runs green-then-red and requires the record to hold the revert with nothing committed — recording only on the success path would silently lose every repair a failing round made, since the worker tree is not rewound. `test_the_revert_record_survives_the_orchestrators_own_save` is the union-on-save pin; without it the executor's mid-dispatch write is clobbered every time and the record reads empty rather than erroring. |
| 2026-08-24 | scope-05 | The refusal set mirrors `test_scope_cleanup.py`'s deliberately: a path no round ever wrote (tracked at the base, outside `approved_paths`), a path INSIDE `approved_paths` (never recorded, so never revertable), an ordinary edit to a recorded path (still unauthorized), an echo of the instruction's placeholder, and the four markup-prefixed shapes. Unit level adds a symlink at the target, a directory at the target, four escaping paths, four non-regular-file base entries, and a missing parent directory. |
| 2026-08-24 | scope-05 | Validation WAS run inside the worker this round, through impl-02's advisory channel: two runs, both PASS (`python3 -m pytest -n auto autoloop/tests -q` and `ruff check .`), the second covering every code change — only Markdown prose and three Python docstrings changed after it. `test_scope_cleanup.py` is deliberately UNCHANGED: scope-04's behaviour had to keep passing untouched, and it does. The backend and root-pipeline suites are untouched by this change and still owed to the orchestrator. |
| 2026-08-24 | dash-21 | `autoloop/tests/test_dashboard.py` gained a dash-21 section: 12 test functions, 14 collected with the three-way parametrized failure table (hand count). The server half runs real threads against real `collect_shared` and, once, against a real `ThreadingHTTPServer` so `do_GET` calling the unguarded `collect` would fail somewhere. The page half lifts the poll loop verbatim out of `PAGE` between its `PURE_POLL` markers and RUNS it under node with a stub `fetch`, rather than grepping the source. |
| 2026-08-24 | dash-21 | The determinism seam is `joins_recorded`, which swaps `_Sweep.done` for an Event recording every caller that enters `wait()` — which is exactly what "joined the sweep already running" means. Without it a test can only assert that one sweep happened, and that is ALSO true when the later callers arrived after it finished, so it would pass against the unguarded code. Every concurrency test here holds the leader until the joins are recorded instead of sleeping and hoping the scheduler cooperated. |
| 2026-08-24 | dash-21 | The fail-open cases are the point. `test_a_sweep_that_fails_releases_every_waiter` is parametrized over a plain error, the measured 2.45s `git log --all` timeout and `KeyboardInterrupt`: an `except Exception` in the leader would not catch the third, `done` would never be set, and every joined tab would wait forever on a thread that had already gone. Its page-side twin is `test_a_throw_before_the_request_is_even_made_does_not_latch_the_guard`. |
| 2026-08-24 | dash-21 | Two tests exist only to refuse a cache: `test_a_caller_arriving_after_the_sweep_ended_runs_a_fresh_one` (three sequential callers get sweeps 1, 2, 3 — no reuse) and `test_the_sweep_registry_is_not_a_cache_and_must_never_become_one`, a drift guard asserting `collect_shared`'s source names no clock, TTL or expiry. One existing test was updated by this change: `test_a_replacement_never_interrupts_a_response_in_flight` now uses `/` for its second request, and says in its docstring why. |
| 2026-08-24 | dash-21 | `test_the_pile_up_before_and_after_measured_in_one_repository` IS the measurement, not a proxy for one: one checkout, one process, eight concurrent callers, both arms. Observed 2026-08-24 — BEFORE 8 sweeps / 141 subprocess launches / 3.06s, AFTER 1 sweep / 20 launches / 1.96s. It asserts the two counted columns and only REPORTS the wall clock, because eight sweeps on eight threads release the GIL inside their subprocesses and that figure partly measures the host's core count. |
| 2026-08-24 | dash-21 | Validation WAS run inside the worker, three advisory runs. Runs 1 and 2 were deliberately failed by one temporary assertion carrying the measurement out (an agent has no shell), and every other test passed in both; run 3 is the clean one. `node=1` was carried out in the same way and matters: `run_js` SKIPS when node is absent, so without it the four page-side tests would have been silently uncovered rather than green. The backend suite is untouched by this change. |
| 2026-08-24 | scope-05 | Revision round adds a seventh section to `test_scope_revert.py`, the PRODUCTION wiring — the half the first round had none of, and the half without which every test above passes while no live round can revert anything. Three build a real `cli._build_orchestrator` (deliberately NOT `null_executor=True`, which returns a `NullExecutor` before `ImplementExecutor` exists): the authority is a `RecordedRevertAuthority`, it reads and writes the ORCHESTRATOR's own execution record, and the wired executor renders the revert form for a recorded path. |
| 2026-08-24 | scope-05 | Store IDENTITY is asserted functionally, not by `isinstance` alone: an authority bound to a second `TaskExecutionStore` type-checks, reads no base sha for any real task and records repairs where nobody looks. Two more pin what going live newly makes reachable — `test_a_corrupt_execution_record_offers_no_revert_and_never_raises` (the `StateCorruptError` that was theoretical while nothing in production called `base_sha`) and a first dispatch with no record at all. Both answer "" and neither raises. |
| 2026-08-24 | contract-01 | `test_contract.py` gained 8 test functions / 16 collected cases for the `notes` length and line-break rule, in one section before the one-step-decomposition test, plus a `_flat` helper and a `_raw_newline_in_notes` builder in neither figure. Nothing existing was deleted, renamed or reordered: `test_notes_accepted` and `test_notes_must_be_string` still pin the old parse behaviour untouched, which is what makes "the parser did not change" checkable from the diff alone. |
| 2026-08-24 | contract-01 | The load-bearing one is `test_contract_forbids_a_literal_line_break_inside_a_string_value`, which asserts the escape with a RAW string literal. Written as an ordinary quoted escape it would pass vacuously against a text full of real newlines — certifying the exact bug it looks like it catches, since `_RESPONSE_FORMAT` is a NON-raw triple-quoted string, so a source that wrote the escape unescaped there would ship a real line break inside the very sentence forbidding them. |
| 2026-08-24 | contract-01 | Two tests PROVE the parser is unchanged rather than asserting it. `test_a_raw_newline_inside_a_string_value_is_still_rejected` is parametrized over both envelopes — the reported failures were `line 6 column 2073`, a pretty-printed object, i.e. the rendered/bare path — and requires `invalid_json` plus `control character` in the message; its payload is hand-built, because `json.dumps` escapes the newline and cannot produce the defect. `test_the_notes_rules_are_guidance_the_parser_does_not_enforce` parses a 5,000-character notes AND an escaped-newline one. |
| 2026-08-24 | contract-01 | The other three pin the rule's content against a vacuous pass: the length bound as the whole clause and never as the digits (`"200" in text` would match any unrelated number), the escape's presence tied to `commit.message` still being documented as the FULL commit message, and the new prose naming no retired decision. `test_a_directive_omitting_notes_parses_exactly_as_it_did_before` is parametrized over every active decision, so optionality is pinned per decision rather than for `stop` alone. |
| 2026-08-24 | contract-01 | `test_contract_stays_within_its_budget` is UNCHANGED except for an appended docstring paragraph — the 3,700 ceiling did not move, and `test_context.py`'s independent `<= 3700` needed no edit. That paragraph carries the first MEASURED figure in a docstring whose every earlier count says "hand count, no shell": 3,636 characters, 64 of headroom left, read off a real run through impl-02's advisory channel. The hand sum (3,489 + 147) agreed exactly, which is worth knowing since nothing had ever re-verified one of those counts. |
| 2026-08-24 | contract-01 | How that figure was obtained is reusable: a THROWAWAY `assert len(...) == 0` was added, one advisory run turned it into `assert 3636 == 0`, and the probe was then deleted before the confirming run. pytest prints the real value on a failed equality, so an executor with the advisory channel but no shell can read any single number out of the suite this way. Note the run STOPS at the first failing command (§4h), so the probe run left `ruff check .` NOT RUN and a second, clean run was needed. |
| 2026-08-24 | contract-01 | Validation WAS run inside the worker through impl-02's advisory channel. Run 2 of 3 PASSED on the final tree: `python3 -m pytest -n auto autoloop/tests -q` PASS and `ruff check .` PASS, with only this report written afterwards. Run 1 is not evidence of anything but the probe — it failed on that deliberate assertion and so left ruff NOT RUN. The configured pytest command is scoped to `autoloop/tests`, so the root pipeline suite and the backend suite were not exercised; both are untouched by this change and are still owed to the orchestrator. |
| 2026-08-24 | recov-01 | New `autoloop/tests/test_transport_fault_recovery.py`: 28 test functions, 28 collected (hand count; the worker has no shell). Its own table row above says what each group pins. The one methodological point worth repeating here: every "no browser was launched" test configures a REAL `restart_command`, because with the default empty tuple `_browser_restart_outcome` returns `RESTART_DISABLED` and never reaches `subprocess.run` — the assertion would then pass for the wrong reason and prove nothing about routing. |
| 2026-08-24 | recov-01 | `test_rounds_and_restart.py` and `test_transport_recovery.py` are UNTOUCHED, deliberately. "The browser provider's restart, cooldown and fault-budget behaviour is unchanged" is a claim only their passing can carry, and an edit to either would have been the signal that the design broke the bound. Both build with the default `ConversationConfig()`, i.e. `browser_chatgpt`, so they route through the new `_route_transport_fault` and land in the same handlers as before. |
| 2026-08-24 | recov-01 | The load-bearing negatives, all fail-open shapes: a `reconcile` that RAISES declines the replay rather than reading "could not ask" as "absent"; a `submit`-side `ResponseTimeoutError` (the CLI's own timeout raises the SAME type) is not a replay signal; an unknown provider name is not browser-backed; and `test_the_replay_budget_is_bounded_and_ends_in_a_park` drives a deliberately self-contradictory transport that confirms every send and answers none — the only shape that could re-invoke a reviewer forever. |
| 2026-08-24 | recov-01 | Round 1 of the advisory validation caught a real design fault, not a typo: `test_codex_app_server.py::test_the_orchestrator_policy_and_state_modules_stay_provider_agnostic` failed because the per-transport remedy table had been written into `orchestrator.py`. It moved to `conversation.transport_remedy`, which is the registry and may know a provider by name. The seam held; the first draft breached it. Worth knowing before adding the next remedy. |
| 2026-08-24 | recov-01 | Validation WAS run inside the worker through impl-02's advisory channel. Run 2 of 3 PASSED all three configured commands: `ruff check .` PASS, `python3 -m pytest -n auto autoloop/tests -q` PASS, `python3 -m pytest -n auto tests/ -q` PASS. The backend suite (`lexy-app/backend`) is not among the configured commands and is untouched by this change. |
| 2026-08-24 | recov-01 | NOT covered by that passing run, and stated rather than implied: the LAST FIVE tests, the third `verdict_text` branch in `_handle_rate_limited`, and the transport refusal at the top of `_attempt_rotation` were all written after run 2's snapshot, with the 3-run budget spent (run 3 was already in flight over the older tree). They are hand-traced against the existing rotation and rate-limit tests, not executed here. The executor's own run is the verdict; read these five as unproven until it passes. |
| 2026-08-24 | recov-01 | Two of the five cover the rate-limit park (a codex run must not be told to `curl .../json/list`; the browser's two branches stay byte-identical). Three cover what the adversarial pass found LAST and worst: `_step_submission_rejected` reaches `_attempt_rotation` after two disproven sends WITHOUT passing any fault handler, and `codex_cli` returns REJECTED on every non-zero exit — so two ordinary codex failures parked on "set browser.project_url". |
| 2026-08-24 | recov-01 | That third door had a second exit worth its own test: an operator who moved from the browser to codex and left `[browser]` populated PASSED both preconditions, spent `state.rotations`, and only then failed inside `_rotate_conversation` for the missing `retarget`. `test_a_stale_project_url_cannot_spend_a_rotation_on_a_codex_run` pins that a leftover config value can no longer cost a budget, and its browser twin pins `rotation_unavailable` unchanged. |
| 2026-08-24 | recov-01 | Revision round. What the earlier "run 2 PASSED" note could not cover: a prose comment in `orchestrator.py` still spelled a provider name, and `test_the_remedies_live_with_the_registry_not_in_the_orchestrator` reads that file as TEXT, so the shipped tree failed its own new test. Reworded, no behaviour touched. Rerun from a stable snapshot with no concurrent edits — ruff PASS, `autoloop/tests` PASS, `tests/` PASS, including the two `test_task_inbox.py` tests the previous round recorded failing. |
| 2026-08-24 | strand-01 | New `autoloop/tests/test_strand_recovery.py`: 35 test functions (hand count plus a real collection run), in seven sections mirroring the claim — the requeue arm, the narrow safe set, what is NOT a strand, the accounting and the bound, the reporting, `health`'s detection half, and the predicate at registry level. No existing test file is modified, which is what makes "no behaviour anyone else relies on changed" checkable from the diff alone. |
| 2026-08-24 | strand-01 | The pool assertion is `next_ready().id`, never `status == "pending"`: `state_of` answers BLOCKED for a pending task whose dependency is incomplete, so those are different questions and only one of them is the claim. It is asserted DURABLE too, by re-loading `tasks.json` — the registry object the orchestrator holds is not the file the next process reads, and a missing save would bring the strand straight back. |
| 2026-08-24 | strand-01 | The bound is a test, not a paragraph: `test_an_outage_that_faults_every_round_still_ends_at_the_ceiling` drives dispatch/fault/move-on/sweep through the real `_dispatch_executor` `MAX_TASK_FAULT_ATTEMPTS` times, requeueing each lap until the allowance is spent — the last lap files the blocker instead, and the next dispatch parks on `fault_attempt_ceiling` before the executor runs. Its companion asserts the requeued `TaskExecution` is equal to what it was, which is why that ceiling arrives at all rather than never. |
| 2026-08-24 | strand-01 | The fail-open negatives are the ones worth reading: a `release` that raises is REPORTED rather than skipped (a silent `continue` rebuilds the third state), an unreadable execution record is a blocker rather than "no strand", and an unreadable task file makes `health` say it could not check instead of "nothing is stranded". The vacuity guards run the other way — the current task, a `sent_for_review` round (including the `fault|<origin>>sent_for_review` redo shape), a published candidate and a task-budget failure must all stay untouched. |
| 2026-08-24 | strand-01 | Validation WAS run inside the worker through impl-02's advisory channel. Run 2 of 3 PASSED all three configured commands: `ruff check .`, `python3 -m pytest -n auto autoloop/tests -q`, `python3 -m pytest -n auto tests/ -q`. Run 1 found two real defects in the new test file, both mine and both fixed: a wrong expectation that the last lap of the bound test would still requeue, and a case-sensitive substring. The backend suite is not among the configured commands and is untouched by this change. |
| 2026-08-24 | strand-01 | Run 3 of 3 then PASSED the same three commands over the FINAL tree, covering the two edits that landed after run 2's snapshot: `health._strand_survey` widened its exception net to `TaskGraphError`/`KeyError` (matching `cli._cmd_start`'s preflight over the same call), and `test_nothing_is_written_when_nothing_moved` was strengthened to drive an unsafe strand so the loop body really runs — without that it returned two lines in and passed vacuously. Only this report was written afterwards. |
| 2026-08-24 | strand-01 | Revision round: `test_strand_recovery.py` gained section 3b for the BOUND on the current-task exemption. The pair the review asked for is the heart of it — a lone `in_progress` faulted task whose stale state still names it, past the round ceiling, is requeued and logged with `stale_current: true`; the same setup INSIDE the ceiling is untouched, no blocker and no requeue entry. Ages are built by ageing the stamp rather than by injecting a clock, so the parse under test is the production one. |
| 2026-08-24 | strand-01 | The rest of 3b pins what the bound must not do: an unsafe shape past the ceiling is still blocked rather than requeued, the requeue happens once (the released task leaves `in_progress_tasks()` — asserted, not assumed), and a loop that NEVER moves on still walks into `fault_attempt_ceiling` rather than cycling one task through a dead API. Plus the ceiling's arithmetic and its fallbacks: a missing, non-numeric or non-positive `agent_ceiling_seconds` falls back to the shipped default, never to zero. |
| 2026-08-24 | strand-01 | `test_health_stays_quiet_for_the_task_the_loop_is_working` now writes the dispatch stamp the loop really writes (`Wiring.working_on`), since that stamp is what separates a live round from a strand. New unit tests cover every unusable stamp — absent, another dispatch's, unparseable, naive (read as UTC), near-future (skew, reads 0), far-future (unknown) — and one new health case: a round that outlived the ceiling is named on the verdict, which is the detection half of the same bound. |
| 2026-08-24 | strand-01 | Revision round validation, through impl-02's advisory channel inside the worker. Run 1 PASSED all three configured commands (`ruff check .`, `python3 -m pytest -n auto autoloop/tests -q`, `python3 -m pytest -n auto tests/ -q`) over the code and the 12 new tests. Run 2 PASSED the same three over the tree including these change notes and a defensive `isinstance` guard in `current_round_age_seconds` (a hand-edited state file whose `current_task` is not a dict must read as unknown, never raise into the sweep). Nothing but a docstring followed run 2. |
| 2026-08-24 | bind-02 | New `autoloop/tests/test_postcommit_binding_carry.py`: 23 test functions in five sections mirroring the claim — the parse-error carry, the OTHER corrective re-prompts, the sent-packet ledger, what is still refused, and the fail-closed edges. Real git against `tmp_path`, with this file's own copies of the small helpers per the `test_postcommit_primitives.py` convention. Its `deliver()` copies exactly the fields `_step_awaiting` copies, so a binding dropped when the response is persisted (the A2 shape) would still show up. |
| 2026-08-24 | bind-02 | Section 2 covers FIVE corrective sites, not the brief's four: `_handle_git_failure` was found by walking every place that replaces `last_response` with a payload asking for another decision, and it carries for the same reason the other four do. Its neighbour pins the BOUNDARY as a test — `test_an_executor_round_that_reported_back_does_NOT_carry` — because a failed round's `implementation_review` report is a round that RAN, not a re-prompt about an unchanged packet. |
| 2026-08-24 | bind-02 | One existing test WAS changed and it is the important one. `test_postcommit_review.py::test_legacy_commit_and_push_is_always_refused_even_with_a_live_candidate` drove its refusal with a `push` answering a parse-error correction — the exact sequence that is now the fix, so it pinned the bug. It now drives `commit_and_push`, which is retired by DECISION and cannot be rescued by any binding, and its block comment records why. Nothing else in that file moved. |
| 2026-08-24 | bind-02 | Added beside it: `test_a_push_answering_a_parse_error_correction_publishes` runs the whole 2026-08-20 incident through a real `orch.run()` — `_step_submitting`, `_step_awaiting`, `_step_executing` — rather than a hand-built `LastResponse`, and requires the candidate on the remote plus no `legacy_git_path_retired` in any prompt. It reuses the now-otherwise-unused `push_approval_from_last_submitted` helper that the amended test above gave up. |
| 2026-08-24 | bind-02 | The discriminating negatives, all of which pass vacuously if written the obvious way: naming a sent packet with a WRONG `report_sha256` must be `review_integrity` and must publish nothing; naming an id the loop never bound must be refused, not guessed; a malformed carry must read as absent AND log `postcommit_carry_unusable`, never raise into `_step_ready`; a carry whose candidate a revise round has advanced must not re-present the old one; and a second push naming an already-published packet must refuse. |
| 2026-08-24 | bind-02 | Revision round: two tests for the seam BETWEEN the carry and the ledger. `test_a_mis_stamped_push_naming_a_sent_packet_is_repairable_by_re_stamping` is the one that fails without the fix — `last_response` has moved on, the push names the earlier packet with one wrong `report_sha256`, and the corrected push names the CORRECTION, which is what `review_mismatch_payload` asks for. Written the obvious way, naming the packet again, it would have passed pre-fix through the ledger and proved nothing. |
| 2026-08-24 | bind-02 | Its sibling `test_a_policy_denied_push_naming_a_sent_packet_keeps_the_resolved_binding` covers the other handler and pins that carrying a resolved binding widens nothing: the corrected `push` is authorized AGAIN against the carried binding's own `task_branch`, so a protected branch still refuses it and nothing reaches the remote. `test_naming_a_sent_packet_still_demands_that_packet_s_exact_stamps` gained one assertion, so the newly-bound correction is visible at the test that proves the refusal. |
| 2026-08-24 | bind-02 | Revision-round validation, through the worker's advisory channel. Run 2 of 3 PASSED all three configured commands: `ruff check .`, `python3 -m pytest -n auto autoloop/tests -q`, `python3 -m pytest -n auto tests/ -q`. Run 1's three failures were the `inspect.getsource` drift artifact §2 of COMMON_ERRORS describes — same two test files, same wrong-function assertion text — caused by editing `orchestrator.py` while that run was in flight. Both named subjects were checked directly and were intact. Run 3 confirms the tree including these notes. |
| 2026-08-24 | shelve-01 | `test_worker_publisher.py` gained a section 14 of 22 test functions / 26 collected cases (the packet-phase refusal is parametrized over five phases; hand count). Placed HERE rather than in `test_recovery_commands.py` because the claim is only checkable against a REAL worker repository — the resume decision shells out to `git rev-parse --show-toplevel` and `git branch --show-current` — and this module is where real throwaway worker repos are built. |
| 2026-08-24 | shelve-01 | `test_shelve_reaches_an_in_progress_task_whose_dependency_is_incomplete` pins the ONE place `shelve` is wider than `release`, in both directions in one test: `release` still refuses that row (it asks `state_of`, which reports BLOCKED first) and `shelve` moves it, and the row then lands `pending` while `state_of` still says BLOCKED — so the widening is reach, not a grant. Same stored-status reading `in_progress_tasks` and `_displaced_work_exists` already take. |
| 2026-08-24 | shelve-01 | The load-bearing pair is discriminating rather than merely true. `test_the_next_dispatch_resumes_the_shelved_round_rather_than_starting_over` asserts the two facts `_dispatch_task_postcommit` actually branches on — a LIVE record, and the REAL `worker_repo_is_reusable` returning True — and `test_release_still_archives_all_three_and_leaves_nothing_to_resume` runs the IDENTICAL fixture through `release` and fails every one of them. |
| 2026-08-24 | shelve-01 | The fail-open negatives, which is where this class of round dies: an unreadable `state.json` must REFUSE (answering "no packet outstanding" by default is the guard silently passing), an unrecognised phase must refuse rather than fall through the `in PACKET_OUTSTANDING_PHASES` test, a `_merge_window_blockers` that raises must print UNKNOWN and never OPEN, and unreadable deferral records must print UNKNOWN rather than "nothing is waiting". |
| 2026-08-24 | shelve-01 | `test_the_shelve_stop_kind_is_not_mistaken_for_a_fault_or_a_preemption` exists because every reader of `LoopState.stop_kind` gates on the POSITIVE value it wants, so a new value is not inert by assumption — `_is_fault_stop` True would STOP the continuous loop, the opposite of what shelving buys. The detach test asserts the literal `Phase(state.phase) in TERMINAL_PHASES` condition `_run_continuous` branches on, not a paraphrase of it. |
| 2026-08-24 | shelve-01 | Validation through the worker's advisory channel: runs 1 and 2 of 3 both PASSED both configured commands (`python3 -m pytest -n auto autoloop/tests -q`, `ruff check .`). Run 2 covered the whole implementation, the full test section and these notes. Run 3 was spent on the final tree, whose only later changes were operator-facing report STRINGS plus the one test assertion paired with them. The executor's own run is the verdict. |
| 2026-08-24 | dash-15 | `test_dashboard.py` gained a dash-15 section of 11 test functions / 11 collected cases after the merge-group tests (hand count, none parametrized), plus the helpers `described_rows`, `rows_by_id`, `merge_panel_payload` and `render_merge_twice`. One existing row-shape assertion was EXTENDED, never loosened: `set(row)` still pins equality, now over eight fields rather than six. |
| 2026-08-24 | dash-15 | The discriminating ones. `test_a_partial_id_never_answers_for_a_branch` offers `dash-15`'s text to `dash-1`'s branch and demands `""`. `test_the_description_lookup_decides_no_state_and_no_group` compares every state, detail, sha and branch with descriptions and without. `test_adding_descriptions_reorders_nothing_inside_a_group` pins dash-13's row order through `collect` against the flat list, so a re-sort smuggled into `merge_groups` fails. |
| 2026-08-24 | dash-15 | Three of the page-side four RUN the lifted `MERGE_PANEL` region under node twice with a `msToggle` between: an opened description survives the poll (the negative — closed on the first render — is what makes it mean anything), a hostile description reaches the DOM escaped and 6,000 characters whole, and an unresolved row renders its cells with no empty disclosure and a colspan matching its five headers. |
| 2026-08-24 | dash-15 | The fourth closes the gap those three leave: they call `msToggle` by hand, so a binder that read the wrong attribute or never fired would keep them green. `test_the_binder_is_what_actually_records_an_operators_click` lifts `bindMergeDescriptions` too, never calls `msToggle`, and runs render → bind → dispatched `toggle` → re-render → dispatched `toggle` again. |
| 2026-08-24 | dash-15 | That test's stub derives its elements from the HTML each render EMITTED and rebuilds them when that HTML changes, so each dispatch lands on the element THAT render bound — asserted, the two are different objects — and an unrendered disclosure binds nothing. `..._bound_over_both_containers` stays as the source-level pin that both containers are reached. |
| 2026-08-24 | dash-15 | Validation WAS run inside the worker through impl-02's advisory channel. All three runs PASSED both configured commands — `python3 -m pytest -n auto -p no:cacheprovider autoloop/tests -q` and `ruff check .`. Run 1 covered the implementation and 10 of the 11 tests, run 2 added these notes, run 3 the final tree with the 11th (the binder test, written after run 2 was already in flight). The only edit later than run 3 is this line and its SUMMARY.md twin. The executor's own run is the verdict. |
| 2026-08-24 | recut-01 | New `autoloop/tests/test_recut.py`: 62 test functions, 105 collected cases (hand count), real git and a real `WorkerRepoManager` throughout, since the recut's whole claim is about what is on disk afterwards. |
| 2026-08-24 | recut-01 | Five sections mirror the claim: contract parsing, the wanted verb's non-dispatchability, the tally, the recut's happy path and its bounds, and the registry verb underneath it. |
| 2026-08-24 | recut-01 | The load-bearing one is `test_a_wanted_verb_never_becomes_the_verb_that_is_acted_on`: EVERY `Decision` value plus junk goes into `wanted_decision` on a hand-built `stop`, through the real `_dispatch`. |
| 2026-08-24 | recut-01 | It asserts the stop happened, no executor ran, no worker exists and nothing was recut. Hand-built rather than parsed, so even a value the parser refuses (the empty string) still reaches the dispatcher. |
| 2026-08-24 | recut-01 | `test_a_wanted_recut_on_a_stop_discards_nothing` is the sharpest instance: a live candidate, `wanted_decision: "recut"`, and the candidate and worker still there afterwards. "The field is recorded" would not test the bound. |
| 2026-08-24 | recut-01 | `test_the_next_dispatch_is_cut_from_the_current_base_and_inherits_nothing` moves the base between cuts, then asserts the new `task_base_sha` is the NEW head, `review_round`/`attempt_count` restart, and the range is one commit. |
| 2026-08-24 | recut-01 | It writes `docs/A.md` in round 1 and `docs/B.md` in round 2, so `commit_range_paths` holding only the second makes "inherited nothing" observable rather than asserted. |
| 2026-08-24 | recut-01 | The bound tests are negatives sharing `assert_nothing_discarded`: published, outstanding verdict, in-flight request, over cap, operator hold, completed, unknown, audit unit, missing/unreadable record, no worker manager, failed retirement. |
| 2026-08-24 | recut-01 | `test_recutting_the_candidate_this_reply_is_about_is_allowed` is the positive control that guard needs — without it the check could refuse every recut that matters and the suite would still be green. |
| 2026-08-24 | recut-01 | `test_a_record_that_lost_its_count_cannot_lower_the_cap` and its mirror do the same job for the two-copy `max` rule: each covers a loss the other copy survives. |
| 2026-08-24 | recut-01 | `test_contract.py` gained one `_COMPLETE_PAYLOADS` row for `recut`, required by `test_complete_payloads_cover_every_active_decision` — the test that stops a new decision going unexercised by the question and decomposition rules. |
| 2026-08-24 | recut-01 | The CONTRACT_INSTRUCTIONS ceiling MOVED for the first time since 2026-08-15, 3,700 -> 4,550, in `test_contract_stays_within_its_budget` and in `test_context.py`, which asserts the same number independently. |
| 2026-08-24 | recut-01 | Both were failing at a measured 4,988. The additions were compressed by a hand-summed 510 — rationale into source comments and §9g — BEFORE the number moved, which is why the raise is 850 and not 1,350. |
| 2026-08-24 | recut-01 | Validation ran in the worker. Run 2 of 3 PASSED both configured commands (`pytest -n auto autoloop/tests`, `ruff check .`) over the whole implementation and test file. |
| 2026-08-24 | recut-01 | Run 3 had ONE pytest failure, `test_docs_merge.py::test_every_change_note_line_is_short_enough_to_merge_by_line` — a 941-char note in this family, not code. Every other test passed, including all of `test_recut.py`. |
| 2026-08-24 | recut-01 | These shortened notes are that fix. Run 3 left `ruff check .` NOT RUN (§4h), so ruff is confirmed only as of run 2; the Python added after it is one test function and two comments. The executor's own run is the verdict. |
| 2026-08-24 | recut-01 | Worth knowing: a hand count of note length is unreliable at this style. Mine came out ~1.8x short — 502 estimated against 913 actual — because backticked identifiers cost far more than ordinary words. Count words and budget ~11 chars each. |
| 2026-08-24 | recut-01 | Second revision round: advisory run 1 of 3 PASSED BOTH configured commands, in order, on the repaired tree — `python3 -m pytest -n auto autoloop/tests -q -p no:cacheprovider` PASS, then `ruff check .` PASS. Nothing was left NOT RUN this time, so ruff is confirmed against the same tree as pytest. |
| 2026-08-24 | recut-01 | Correction to the run-3 lines above: the 941-char line that failed `test_every_change_note_line_is_short_enough_to_merge_by_line` was NOT a recut-01 note. It was two 2026-08-23 stop-01 rows in `docs/COMMON_ERRORS.md` joined by a deleted newline, so shortening this task's own notes fixed nothing. The same seam sat in all four trackers, and the test reports only the FIRST violation, so one repair per run would have cost four runs. All four newlines are restored; no note text was rewritten. |
| 2026-08-24 | dash-19 | New `autoloop/tests/test_merge_window_panel.py`: 36 test functions / 38 collected cases (one is parametrized over three unanswered-check shapes; hand count) in five sections — the payload, the failure paths, read-only/lock-free, the page, and the wiring. Local copies of `run_js` and the repo helpers rather than an import from `test_dashboard.py`: these modules are not a package, so a cross-file import would depend on collection order putting that directory on `sys.path` first. No existing test changed. |
| 2026-08-24 | dash-19 | Both halves, because either alone is an echo. The SEAM is pinned with a recorder over `cli._merge_window_blockers` asserting what it receives (a config whose `state_dir` is `dashboard._state_dir(repo)`, a gateway rooted at the checkout, a fresh `seen`) and that the payload is its return verbatim — a reimplementation passes every behavioural test and fails this one. The REAL predicate is then driven against real git checkouts for the open case, a candidate bound to the live HEAD, and both note shapes. |
| 2026-08-24 | dash-19 | The fail-open negatives, which is where this class of round dies: a predicate that RAISES must be `unknown` and never `open`, a missing config and an unparseable `tasks.json` likewise, a state dir that does not exist must be REPORTED rather than read as an empty in-flight list, and a `subprocess.TimeoutExpired` must reach the guard as `unknown`. On the page, three shapes of unanswered check (stated unknown, uninterpretable state, no window at all) must all render UNKNOWN with counts reading `unknown` rather than `0`. |
| 2026-08-24 | dash-19 | `test_the_stamp_moves_on_every_poll_even_when_the_window_does_not_change` is the one that pins the claim rather than the plumbing: it runs the lifted `MERGE_WINDOW` region under node over two payloads differing only in `served_at`, and requires the stamp to move while the lists do not rebuild. Its sibling requires a CHANGED window to redraw, so a signature that never invalidated fails. Distinctness of notes from reasons is asserted in both directions; a one-way check passes for a page that renders the notes twice. |
| 2026-08-24 | dash-20 | `test_dashboard.py` gained 12 test functions in one section at the end (hand count) for the dashboard's ONE state-directory resolver and its unit panel. Five drive `collect()` against an explicitly configured `[paths].state_dir` while a COMPLETE decoy state dir — state.json, tasks.json, two execution records, a blocker, a transcript, an audit run, a pending_upgrade marker and a LIVE LOCK, every value carrying the marker `decoy` — sits at `<repo>/.autoloop`, where the pre-fix code read from. |
| 2026-08-24 | dash-20 | The load-bearing assertion is `DECOY not in state_derived(payload)`: one blanket check covering every reader in the payload at once, so a SECOND resolver spelled `Path(repo, ".autoloop")` or `repo.joinpath(...)` is caught too. A source grep for the literal was rejected as an echo — it would specify the string it then goes and finds. `agents` and `inbox` are excluded because neither resolves from the state dir; the decoy's live LOCK makes liveness a discriminator, not just another string. |
| 2026-08-24 | dash-20 | The other cases: an absolute state_dir, a RELATIVE one resolved against the checkout (the shipped config ships one, so an absolute-only fix would leave those deployments broken), a configured dir that does not exist yet, and the unconfigured default unchanged. The unit-panel tests pin a first round's real `review_round` 0 rendering as "0" and never as an em dash, "no candidate yet" not sharing the idle marker, an idle loop borrowing nothing from a published record, a `task_execution` with no `task_id` contributing no figure even in the RAW fields, and every `UNIT_STATES` member reachable. |
| 2026-08-24 | dash-20 | `test_a_config_that_will_not_parse_falls_back_to_the_checkout` PINS rather than endorses the residual hole: `_config_toml` swallows malformed TOML into an empty dict, so on a post-port-01 deployment a typo puts the whole page back on `<repo>/.autoloop`. Tolerable only because the fallback is TOTAL — every panel moves together, so the page is plainly stale rather than half-live — and because port-06 owns the unconfigured default. Surfacing it would need a payload field somebody has to decide on. |
| 2026-08-24 | exec-01 | New `autoloop/tests/test_partial_work_report.py`: 17 test functions, 17 collected cases (hand count, none parametrized), in five sections mirroring the claim — a failed-validation round, an agent-error round, the branches deliberately left alone, the fail-open cases, and confinement. Real git repositories and whole `ImplementExecutor.execute()` rounds throughout; fixtures and fake agents follow `test_implement_executor`. No `claude` CLI and no real validation binary runs. |
| 2026-08-24 | exec-01 | The load-bearing one is `test_the_numbers_contradict_the_agents_own_claim`: the stand-in agent writes ONE file and reports "900 lines across 40 files, mostly in services/invented.py". The round must report 1 file and 1 line, must not contain that path or "40 file", and the agent's text must still be present in `outcome.details`. A report built from `result.raw_text` — the echo this task exists to avoid — fails it; every purely behavioural test would still pass. |
| 2026-08-24 | exec-01 | The fail-open negatives, which is where this class of round dies. `measured=False` with no paths must say UNKNOWN and must contain neither "NONE" nor "0 file" — an empty list rendered as zero is the guard silently passing on missing input. Paths must survive when only the COUNTS failed (two git reads, either can fail alone). A measured zero must NOT read as UNKNOWN. And the bounded list must name what it dropped: silent truncation reads exactly like complete coverage. |
| 2026-08-24 | exec-01 | Two tests pin the measurement WINDOW rather than its output. `test_validations_own_residue_is_not_counted_as_the_agents_work` gives the round a command runner that really writes `.ruff_cache/CACHEDB` into the worker repo and then fails: the summary must not mention it and must agree with `changed_paths`, so a count taken after validation fails. `test_reporting_writes_nothing_outside_the_worker_repo` snapshots the main checkout across a whole failed round. |
| 2026-08-24 | exec-01 | Three negatives pin the decisions NOT to report: the success path carries no partial-work section at all (it is the branch whose summary becomes the commit message), the changed-nothing branch makes one claim rather than two, and a stall keeps `summary.count("file(s) changed") == 1` while gaining paths. `test_stall_detector.py`'s two executor tests were left unmodified on purpose — they are the pre-existing pins for the agent-failure branch and had to keep passing untouched. |
| 2026-08-24 | exec-01 | Validation WAS run inside the worker through impl-02's advisory channel; see the run lines the report carries. The backend suite is not among the configured commands and is untouched by this change. Per `docs/AUTOLOOP.md` §4h a run stops at the first failing command, so read any `NOT RUN` line as no evidence either way, and the executor's own run after this agent returns is the verdict. |
| 2026-08-24 | port-06 | `test_state_dir_location.py` gained 7 test functions / 9 collected cases (hand count) for the ONE state-dir resolver: absent, absolute and relative `[paths].state_dir`, each resolved by BOTH `load_config` and `dashboard._state_dir` from one config file and compared to EACH OTHER, never to a path the test spelled. The relative case runs under `monkeypatch.chdir(checkout)`, which executes the "the loop's cwd is the checkout" premise instead of restating it in prose. |
| 2026-08-24 | port-06 | The discriminating one is `test_a_priority_edit_from_the_page_lands_where_the_loop_reads`, parametrized over all three shapes: the dashboard's own `_task_store` applies the edit, and the read-back goes through `TaskStore(load_config(cfg).tasks_file)`. Reading it back through `dashboard._tasks_file` proves only that the page agrees with itself — which is exactly what the broken version did, and why an operator could not tell. |
| 2026-08-24 | port-06 | `test_the_resolver_is_the_only_state_dir_rule_the_dashboard_has` replaces `dashboard.resolve_state_dir` and requires the page's answer to move with it, so a second implementation that agrees today still fails. The refusals are pinned in both readers together: neither key present, no config file, and blank / whitespace / non-string values — `Path("")` is `.`, a guess wearing a value's clothes. |
| 2026-08-24 | port-06 | `test_dashboard.py`: `make_repo` now writes `state_dir = ".autoloop"` into the config directory it already made, so every test writing state there STATES where it lives instead of relying on the fallback this task deleted. Two tests were rewritten rather than weakened — the unconfigured case now reads the loop's own default, and the unparseable-config case (whose docstring named port-06 as its owner) now asserts the error. |
| 2026-08-24 | port-06 | Three new `test_dashboard.py` tests, plus two rewritten under new names (the unconfigured case and the unparseable-config case, whose docstring named port-06 as its owner). New: a checkout with no config says so, names the missing file and shows no SEED roadmap under a banner claiming nothing was read; the payload keeps EVERY key it has on a healthy checkout, so the banner cannot be lost to a page that throws; and `/api/priority` refuses through the REAL `_task_store` — not `serving_with_store`, which patches the very function that must refuse. |
| 2026-08-24 | port-06 | One existing end-to-end test was fixed rather than weakened: `test_a_stale_dashboard_ends_up_serving_the_new_code` builds its checkout WITHOUT a config and writes `pending_upgrade.json` at the old default, so under the new rule the child could not locate its own marker and never re-exec'd. It now writes `state_dir = ".autoloop"` — what a real deployment states — and the claim it pins is unchanged. Advisory run 1 caught it; it was the only failure. |
| 2026-08-25 | brw-15 | `test_transport_recovery.py` lost every assertion that a rotation HAPPENED: the nine `_url_in_project` cases, the bind-after-reconcile / failed-attempt / cap / config-heal / address-priming tests, and the three-strike silence rotation. Each was replaced by the park or the negative that is now true, not merely dropped — e.g. the third start timeout asserts the silence check is never asked for and nothing is sent. NO test file was deleted or emptied; `RotatingFakeClient` keeps its browser-shaped affordances. |
| 2026-08-25 | brw-15 | New `test_no_rotation_machinery_is_reachable_from_the_orchestrator` is the claim as a test: the eight removed methods and four constants are absent from the module, and its SOURCE no longer contains the four retired blocker codes, `check_rotation_budget` or `rotations +=`. The source half matters because a helper re-added under a different name would still have to spend the budget or emit one of those codes to do anything. |
| 2026-08-25 | brw-15 | Also updated: `test_transport_fault_recovery.py` (three tests now expect `send_rejected_twice`, and the browser/codex split they pin is now about the REMEDY quoted, not about which recovery each transport gets), `test_conversation_retirement.py` §4 (three tests assert the park while keeping the no-restart / no-budget rules the 2026-08-17 incident is about), `test_chunked_packet_delivery.py` (two rotation tests became one: a wedged chat parks and the delivered parts stay put). |
| 2026-08-25 | brw-15 | Validation through the worker's advisory channel. Run 3 of 3 PASSED all three configured commands over the final tree: `ruff check .`, `python3 -m pytest -n 4 autoloop/tests -q`, `python3 -m pytest -n 4 tests/ -q`. Only this note line was written after that snapshot. Run 1 caught one unused import; run 2 caught one real defect and four artifacts of editing `orchestrator.py` while it was in flight — the `inspect.getsource` drift COMMON_ERRORS §2 documents, which is why run 3 was taken over a frozen tree. |
| 2026-08-25 | brw-15 | The real defect run 2 caught is worth knowing: the module docstring explaining WHY rotation was removed named the default provider, and `test_the_remedies_live_with_the_registry_not_in_the_orchestrator` reads `orchestrator.py` as TEXT and forbids every registered provider name in it. Prose breaches that seam exactly as code does. Reworded to point at `conversation._BROWSER_BACKED` instead — the one place allowed to know a provider by name. |
| 2026-08-25 | brw-16 | `test_conversation.py` rewritten around the new claim. `test_browser_chatgpt_is_registered` inverts into `test_no_browser_provider_is_registered` (an exact `available_providers()` equality) plus `test_nothing_registered_is_browser_backed`. REMOVED: `test_browser_chatgpt_satisfies_interface`, the structural probe of `BrowserChatGPT` — it now certifies a class nothing can select, and keeping it would hold an `autoloop/browser/` import in this file for no claim. |
| 2026-08-25 | brw-16 | Same file gained `test_a_new_adapter_can_still_declare_itself_browser_backed`, which is the BOUND on the whole task: `register_provider(..., browser_backed=True)` still registers, still reports browser-backed, still builds, and silence still means not-a-browser. Without it every other assertion here is satisfied by deleting the mechanism, which is exactly what conv-05 needs kept. It restores the registry it found. |
| 2026-08-25 | brw-16 | New `autoloop/tests/test_browser_provider_removed.py`: 12 test functions (hand count) in four sections — no `[browser]` section, a leftover one, a `[conversation]` naming the retired provider, and the conftest edge. The leftover-section tests write EVERY key `BrowserConfig` accepts, and two negatives require an unknown key and a malformed `restart_command` to be refused still: ignored must not become unvalidated. |
| 2026-08-25 | brw-16 | The one that pins the saving is `test_the_tests_conftest_does_not_import_autoloop`, asserted on conftest.py's SOURCE with comments and the docstring stripped. A module-object check would pass against a lazy import inside a fixture, and the point is the static graph `validation.select_validation_commands` reads — a string monkeypatch target would keep the runtime dependency while hiding the edge. |
| 2026-08-25 | brw-16 | `autoloop/tests/conftest.py` lost its only fixture, autouse `_no_live_cdp_probe`. The probe it stubbed is reachable only for a browser-backed provider and none is registered, so the two modules that still register one do the stubbing themselves: `_browser_orch` (`test_rounds_and_restart.py`) and `build` (`test_transport_fault_recovery.py`) each set `orch._attachable_page_targets = lambda: None` on the instance. Same UNMEASURABLE value, same behaviour, no live socket. |
| 2026-08-25 | brw-16 | Both of those modules gained an autouse fixture registering a browser-backed adapter of their own (`fake_browser_for_restart_tests` / `fake_browser_for_fault_tests`) and now select it as `conversation.provider`. They used to inherit `browser_chatgpt` from `ConversationConfig()`'s default; without the replacement every rate-limit test would silently take the "this run drives no browser" branch and pass for reasons unrelated to what it says it pins. Each fixture restores the registry. |
| 2026-08-25 | brw-16 | Smaller updates. `test_transport_fault_recovery.py`'s three registry assertions now name the fake and additionally require the retired name NOT to be browser-backed. `test_codex_app_server.py` drops `browser_chatgpt` from its selectable-providers set and from the fallback test. `test_codex_provider.py` renames its failover label to `FALLBACK_PROVIDER` (injected `provider_factory`, so the registry is never consulted). `test_smoke.py` is rewritten end to end around the retired command — see the later note. |
| 2026-08-25 | brw-16 | Found by advisory run 1, not by reading: `test_orchestrator.py` and `test_transport_recovery.py` also inherited `browser_chatgpt` from `ConversationConfig()`'s default, so four transport-fault tests flipped from `failed` to `needs_user` when the default moved. Both builders now take a `provider=` defaulting to a registered browser-backed fake of the module's own, which keeps every test pinning what it says it pins. `test_conversation_retirement.py` imports that fixture BY NAME — it uses `build` from there. |
| 2026-08-25 | brw-16 | `test_doctor.py`: `doctor.py` keys `conversation_url`, `conversation_active`, `project_url` and the `primary_live` skip on the LITERAL retired provider name, and `doctor.py` was outside brw-16's approved paths. Rather than delete that coverage, `url_check` and the CDP-unreachable test now name the retired provider explicitly (`BROWSER_PROVIDER_CHECKS`) to reach the branches, and `test_all_green` no longer expects `conversation_url` at all. Removing those dead checks is a follow-up. |
| 2026-08-25 | brw-16 | Two tests came out of the adversarial pass rather than the plan. `test_a_default_run_never_probes_the_cdp_endpoint` makes `_attachable_page_targets` RAISE and drives a real `_handle_rate_limited` on the production default provider — the fail-open the deleted conftest fixture used to cover, now closed at the source. `test_a_live_foreign_lock_does_not_change_the_answer` plants a foreign-host LOCK (live by definition) and requires the smoke refusal, not a `LockHeldError`. |
| 2026-08-25 | brw-16 | Advisory run 2 caught the last one: `test_restart_wiring.py::test_the_shipped_template_restarts_via_the_module` reads `config.example.toml` through the real loader, and that template no longer ships a `[browser]` section. The pin has no subject — nothing executes a restart command while no provider is browser-backed — so it INVERTS into `test_the_shipped_template_names_no_restart_command_at_all`, and a new sibling pins that a config which DOES name the module is still honoured verbatim. The module-exists and tombstone tests are untouched. |
| 2026-08-25 | brw-16 | Run 3 then failed that REPLACEMENT, on its own first assertion: `"[browser]" not in <template text>` matched the comment explaining the removal, so a correct template failed. Rewritten to ask `tomllib` whether the parsed document declares the table — the actual property, and one prose cannot fool. Worth knowing generally: a substring search over a config template is answered by its comments as readily as by its settings. |
| 2026-08-25 | brw-16 | Advisory budget accounting, stated because the last edit is not covered: three runs, all `ruff check .` PASS. Run 1 FAIL (5 files), run 2 FAIL (1 test), run 3 FAIL (1 test — the one above). `python3 -m pytest tests/` was NOT RUN in all three (§4h stops at the first failure), so the root pipeline suite has no evidence either way here; it is untouched by this change. The tomllib fix landed AFTER run 3 with the budget spent — the executor's own run is the verdict on it. |
| 2026-08-25 | brw-16 | Found by the adversarial pass, not by a run: `test_conversation_retirement.py` gets its browser-backed provider through an IMPORTED autouse fixture, and nearly every assertion there is a NEGATIVE (no `browser_error`, no `browser_restarted`, `browser_restart_skips == 0`) that passes vacuously if that import stopped carrying it. `test_a_dead_session_in_awaiting_still_takes_the_restart_path` now asserts `orch._transport_is_browser_backed()` first — the one question whose answer differs between the two worlds. |
| 2026-08-25 | brw-16 | Revision round: `smoke-browser` is RETIRED, not repurposed, so `test_smoke.py` is rewritten end to end (13 functions, 16 collected) and no longer imports `autoloop.browser.chatgpt`. The round-trip coverage is gone because its subject is — the command smoked the browser transport. What replaces it is mostly ABSENCES, since a retired command's failure mode is doing something: no provider, orchestrator, policy engine or git gateway constructed, `LoopLock` never called, `.autoloop/smoke/` byte-identical, no state dir created. |
| 2026-08-25 | brw-16 | Three of those pin that no config is READ (missing path, malformed TOML, a `[conversation]` still naming the retired provider) — each lands on the identical message, where a `ConfigError` would exit 1. `test_it_never_exits_zero` is the fail-open guard: 0 would read to a wrapper, a cron job or the `doctor` + `smoke-browser` upgrade runbook as proof the transport works. Two more pin DISABLED-not-deleted: deleting the subparser also exits 2, from argparse, saying "invalid choice". |
| 2026-08-25 | brw-16 | Two rows above describe tests that went with the command. Left VERBATIM and flagged here rather than rewritten. The auto-04 row says `test_smoke.py`'s three refusal tests are unchanged and are the real regression test for the fault stop — those three are gone, and that distinction is now pinned only by `test_blockers.py::test_contract_stop_is_classified_and_is_not_a_fault` plus its fault sibling. The `test_self_upgrade.py` row names `smoke-browser`'s own orchestrator as the opt-out case; that test's docstring is corrected, the row is not. |
| 2026-08-25 | del-01 | New `autoloop/tests/test_scoped_delete.py`: 34 test functions, 42 collected with the two parametrized sets (hand count), in six sections mirroring the claim — the gate, the request line, the prompt, the deletion end to end, disclosure, and what stays refused. Real git and a real `ImplementExecutor` driven through the orchestrator, like `test_scope_cleanup.py`, because the claim spans the prompt the agent reads, the gate, the unlink, and the commit git actually makes. No existing test file is modified. |
| 2026-08-25 | del-01 | The load-bearing one is `test_a_round_deletes_a_tracked_file_inside_its_approved_paths`: round 1 COMMITS the file, so round 2 deletes a TRACKED path and `commit_and_capture`'s `git add -- <path>` has to stage a removal. If it could not, the round would raise `GitCommandError` and park `commit_refused`. A test over an untracked file would never have found that — an untracked-then-deleted path never reaches `changed_paths` at all, so it never reaches `git add` either. |
| 2026-08-25 | del-01 | The echo negatives, where this class of round dies: the prompt's own placeholder `<repository-relative path>` deletes nothing, a `-`/`*`/`>` prefix is prose about the rule rather than a use of it, and `test_the_report_is_computed_from_the_unlink_not_from_the_agents_account` has the agent CLAIM forty deletions while requesting none — the summary must say nothing. The scripted agent double can only WRITE files and never delete one, so every deletion in the file goes through the executor. |
| 2026-08-25 | del-01 | Guard tests aimed straight at `_apply_scoped_deletes`: a directory at an EXACT approved entry is refused rather than descended into, `pkg/../../secret.txt` passes the scope gate and is stopped by the unlink guard with the outside file intact, a symlink is unlinked as the LINK and its target survives, an authorized path with no file at it is reported rather than counted a failure, and a path the out-of-scope record governs is left to the other instruction. |
| 2026-08-25 | del-01 | Bound tests, without which the feature is satisfied by deleting the mechanism: `WRITE_ALLOWED_TOOLS` asserted byte-identical with `Bash` still disallowed; every `TRACKER_PATHS` member refused (parametrized), including when the task declares the tracker itself or covers it by a `docs/` prefix; an unscoped task told nothing and able to delete nothing; and `allowed_paths`/`approved_paths` unchanged after a deletion round. |
| 2026-08-25 | del-01 | Validation ran in the worker through impl-02's advisory channel. Runs 1 and 2 of 3 both PASSED all three configured commands in order — `ruff check .`, `python3 -m pytest -n 4 autoloop/tests -q`, `python3 -m pytest -n 4 tests/ -q`. Nothing was left NOT RUN in either, so ruff and both suites are confirmed against the same trees. Run 1 covered the implementation and the test file; run 2 added the documentation; run 3 is over the final tree. The backend suite is not among the configured commands and is untouched here. |
| 2026-08-25 | del-01 | Run 3 PASSED all three too, but TWO tests postdate its snapshot with the budget spent, so read them as unproven until the executor's own run: `test_a_round_that_commits_nothing_still_discloses_the_deletion` (an uncommitted round's note travels `outcome.summary` into `state.outbox`, NOT `execution.report_summary`, which is written after the early return — a different route from every other disclosure test here) and `test_the_named_deletion_list_is_bounded_and_says_what_it_dropped`. |
| 2026-08-25 | ceil-01 | New `autoloop/tests/test_task_split.py`: 44 test functions (hand count), real git and real worker repos, in eight sections mirroring the claim — the ceiling asks, converging feedback extends, relocating feedback decomposes, the anti-refund rule, the recursion bound, the identical plan, the answers that are not answers, and the states that must not reach the reviewer. No existing test file is modified. |
| 2026-08-25 | ceil-01 | The anti-refund test pins BOTH halves, because satisfying one is easy and useless: the children's caps sum to strictly less than `n x MAX_TASK_ATTEMPTS` AND every child still has usable attempts. A rule that handed each child the parent's full spend as debt would pass the first and rebuild the park this removes. `test_even_an_extended_child_never_reaches_a_full_fresh_budget` states the constant relationship, so a later change to any of the four numbers fails here rather than silently refunding. |
| 2026-08-25 | ceil-01 | The ping-pong bound is `test_the_loop_asks_once_and_then_parks`: the ceiling check spends neither the attempt budget nor the denial budget, so a loop that merely re-asked would re-ask forever with every signal green. Its siblings cover the other two fail-opens — a registry write that fails must PARK rather than ask an unrecorded question, and an audit unit or an id the registry does not hold must park under the original `attempt_count_ceiling` code. |
| 2026-08-25 | ceil-01 | The echo test is `test_a_plan_identical_to_the_stored_one_is_refused`, with a whitespace-and-case twin: the request SHOWS the reviewer the stored plan so the new one can differ from it, which is exactly why handing it straight back must not read as a classification. `test_the_stored_plan_is_shown_so_the_new_one_can_differ_from_it` pins the two halves together — the requirement is meaningless if the reviewer cannot see what it must differ from. |
| 2026-08-25 | ceil-01 | Three tests exist only to bound the feature rather than exercise it: an ordinary reshape buys no extension, an ordinary `plan` with nothing waiting is untouched, and a `stop` at the ceiling is still a `stop`. Without them every other assertion is satisfied by a version that widens the ceiling for every task. `test_a_dependent_of_the_split_parent_is_repointed_at_the_children` covers the strand a retirement would otherwise create. |
| 2026-08-25 | ceil-01 | TWO EXISTING TESTS CHANGED, both outside this task's approved paths and both pinning the ROUTE this task changes rather than the bound it keeps: `test_attempt_budget.py::test_the_task_budget_still_terminates_a_task_that_fails_its_own_work` and `test_m1_hardening.py::test_pre_commit_failures_consume_the_attempt_budget_across_restart`. Each now dispatches ONCE more — the ceiling asks, the unanswered dispatch parks — and every claim they carried is kept and re-asserted: the executor never runs again, `attempt_count` is never bumped by a refusal, the ledger is five task-budget failures, and the task ends parked. |
| 2026-08-25 | ceil-01 | Neither was weakened to pass. The park code they assert moved from `attempt_count_ceiling` to `ceiling_plan_unanswered`, which is the honest name for what now ends those runs; `attempt_count_ceiling` is still asserted, from a real dispatch, by three tests in `test_task_split.py` (an audit unit, an id the registry does not hold, and a subtask with both remedies spent). Advisory run 1 found both failures before this note was written. |
| 2026-08-25 | ceil-01 | Revision round: `test_task_split.py` gained 6 tests for the operator verbs that END a round, with helpers `operator_release` (the real `release_task_to_pending`, the one function `cli._cmd_release` calls), `operator_shelve` (`registry.shelve` + `preserve_execution`) and `authorize` (the real `PolicyEngine.authorize_directive`). Both halves are pinned for `release`: the next ordinary directive draws no `ceiling_plan_required` denial, and a differing plan grants no extension. |
| 2026-08-25 | ceil-01 | The half a plan-less directive cannot prove is why `test_after_a_release_a_differing_plan_does_not_buy_an_extension` carries `NAMED_FIX_PLAN`: `_ceiling_reply_ok` returns early when the directive has no plan, so a plan-less round is green against a stale marker and says nothing about the grant. Its twin covers the verb that KEEPS the round — after `shelve` the record is still at the ceiling, so the dispatch asks afresh, runs no round and grants nothing. |
| 2026-08-25 | ceil-01 | `test_unblocking_a_ceiling_park_deliberately_leaves_the_request_standing` is the counter-pin, and it is why the clear lives in `tasks._return_to_pending` rather than on every route to `pending`: `unblock` ends no round, and `_park_ceiling_plan_unanswered` tells the operator that answering the blocker does not re-ask the reviewer. Clearing there would make that sentence a lie. |
| 2026-08-25 | ceil-01 | Also new: `test_a_refused_return_to_pending_leaves_the_request_where_it_found_it`, a pure registry test putting all three verbs against a `pending` task. Each must raise `task_not_in_progress` AND the marker must still read its original timestamp — the clear sits after the status move, and that move raises rather than returning, so a verb that refused ends no round and un-asks nothing. The ordering is the guard; this is what pins it. |
| 2026-08-25 | ceil-01 | Revision-round validation, through the worker's advisory channel. Run 1 of 3 PASSED all three configured commands in order — `ruff check .`, `python3 -m pytest -n 4 autoloop/tests -q`, `python3 -m pytest -n 4 tests/ -q` — over the implementation, the six operator-verb tests and every documentation edit. Nothing was left NOT RUN, so ruff and both suites are confirmed against the same tree. The backend suite is not among the configured commands and is untouched by this change. |
| 2026-08-25 | ceil-01 | Run 2 of 3 then PASSED the same three commands over the FINAL tree, covering the one test written after run 1's snapshot (`test_a_refused_return_to_pending_leaves_the_request_where_it_found_it`) and every note above. Only this line postdates that snapshot. The executor's own run after the agent returns is the verdict. |
| 2026-08-25 | ceil-01 | Run 3 of 3 PASSED the same three commands again, over a tree whose only later change was prose in the two ceiling-park messages: both said "once this park has quarantined the task", but `_to_needs_user` quarantines nothing — the `blocked` status is written later by continuous mode's `task_fatal` handling, so a non-continuous operator was told `release`/`shelve` were refused when both applied. Reworded to name continuous mode. Only this line postdates run 3. |
| 2026-08-25 | intake-02 | New `autoloop/tests/test_intake.py`: 43 test functions, 43 collected (none parametrized; counted with `rg -c '^def test_'`, not by hand — the hand count said 38). Organised around the ways the claim could be TRUE-ish and still wrong rather than around the functions in `inbox.py`: the fail-open emission gate, echo, a fabricated evidence negative, a question reaching a model mid-round, three entry points that converge only in prose, an abandoned exchange leaving residue, a scope carrying its own permission slip. Hermetic — no DB, no network, no real model; real `git` IS shelled out to. |
| 2026-08-25 | intake-02 | The load-bearing one is `test_three_entry_points_produce_a_byte_identical_draft`: CLI text, a dropped `.md` and a real `POST /api/intake` over a socket, compared BYTE FOR BYTE. Asserting each route calls `create_draft` proves nothing — a second implementation can call it too and then decorate the result. Byte equality cannot be satisfied by two formats, which is why `create_draft` is clock-free: a timestamp would make the equality depend on three calls landing in the same second. |
| 2026-08-25 | intake-02 | The fail-open negatives, where this class of round dies. A provider returning `""` must NOT make the draft ready and must say so; a provider that RAISES must still add the two constant questions and still not advance; deleting a `?!` line must BLOCK rather than clear; a blank optional answer must be assumed WITH the assumption written into the file, while a blank required one blocks. `test_evidence_says_when_it_read_nothing_at_all` covers the git-unavailable case that `[]` and "no matches" share. |
| 2026-08-25 | intake-02 | The echo test hands the model's reply back as the prompt's own contents — a repeated question, the same question re-punctuated, a required question, the idea line, and an evidence line — and requires ZERO questions added plus a note saying so. `test_a_reply_contributes_questions_and_never_evidence` is its other half: a reply naming a file that does not exist must not become an `Evidence` row, and every row's source must read `git ls-files`. |
| 2026-08-25 | intake-02 | Bound tests, without which the feature is satisfied by refusing everything: writing, editing and SUBMITTING a draft must all still work with a LIVE lock planted, because `add-task` is documented safe at any moment and intake's file writes have the same property. `test_a_drain_ignores_everything_intake_writes` plants a draft and a decline ledger and then really drains the inbox, asserting zero requests and zero problems — the sibling-not-child placement, executed rather than asserted in prose. |
| 2026-08-25 | intake-02 | Also pinned: a hand-mangled file keeps every answer (arrow deleted, answer on the following lines, a second arrow inside an answer); a draft round-trips through `parse_draft`/`render_draft` unchanged; a slug cannot address a file outside the intake directory (`../escape`, absolute, `a/b`); a plan reply's `/etc/passwd` and `everything/` never reach `approved_paths`; a second `plan` is refused; and an unready draft is refused BEFORE any model is asked, asserted on the stub's empty prompt log. |
| 2026-08-25 | intake-02 | Advisory run 2 found the case four tests had assumed away rather than covered: an idea that names no file gets NO suggested scope, because `path_suggest` proposes only from what the text points at. That is ordinary, not a bug — readiness is about the ANSWERS — so the helper's idea now names a tracked file and `test_a_ready_draft_whose_scope_could_not_be_proposed_says_so` pins the bare case: the draft is still emitted, the empty list renders a "nothing was detected" note rather than a blank, and `draft_specs` refuses it. |
| 2026-08-25 | intake-02 | Revision adds the first EXECUTABLE JavaScript coverage of the intake panel: four tests lift the handlers out of `dashboard.PAGE` between the `INTAKE_PANEL` markers and run them under node against a stub DOM and a stub `fetch`, driving Submit and Ask. Skipped, never faked, when node is absent — `run_js` is a local copy because `autoloop/tests/` is not a package. Asserted by EQUALITY on the call sequence and on the first call's payload: a grep passes for a handler that posts the right two urls in the wrong order or with no `text`. |
| 2026-08-25 | intake-02 | The vacuous-pass trap those three are written around: if nothing bound a click handler, `CALLS` stays empty and "submit was never called" is trivially true. So the failure test asserts `urls == ["/api/intake/edit"]` — one call, and it HAPPENED — plus the payload it carried, plus the note naming what did not happen. The urls a handler must not reach are registered too, so a wrong call is asserted against rather than throwing. Paired with `test_the_dashboard_reads_and_writes_the_same_file_the_cli_does`: JS proves the button sends the box, Python proves the server files what was sent. |
| 2026-08-25 | intake-02 | The fourth is `test_the_dashboard_submit_button_reports_the_three_quiet_failures`: a save whose `fetch` REJECTS (server down — not a status code, so unreadable off `r.ok`), a refusal carrying no `error` field (the note would read " ✗ undefined"), and a save that worked followed by a SUBMIT that was refused. All three are fail-closed on authorization already; each was silent, and a button that appears to do nothing is pressed again. The stub gained a `throws` reply for the first, since a rejection is the failure a handler is likeliest to drop. |
| 2026-08-25 | sweep-01 | New `autoloop/tests/test_merge_backlog_health.py`, 30 functions / 32 collected — see its table row above. The two tests that stop the rest passing vacuously: `test_a_young_hold_is_reported_as_data_not_as_an_alarm` (a version escalating on PRESENCE fails it, and presence is what makes a field ignorable) and `test_what_the_sweep_wrote_is_what_health_reads`, which runs the real sweep and then the real check with no hand-written transcript between them — every other reader test authors the text it then reads, which proves the parse and not the pair. |
| 2026-08-25 | sweep-01 | FOUR existing assertions in `test_merge_sweep.py` changed, all the same one: `sweep_entries() == []` for a provably-clear run now expects exactly one `merge_sweep_nothing_to_do` entry. What "silently means silently" protected — one log line per already-merged BRANCH burying the handful that need integrating — is untouched and still pinned. Invocation-level silence is what made a hold unclearable: in a healthy loop `auto_merge` integrates each completion as it lands, so the startup sweep can be clean and silent for weeks. No other assertion in that file moved. |
| 2026-08-25 | sweep-01 | Deliberately NOT re-tested here: that the sweep still refuses to merge past an unjudgeable task. `test_merge_sweep.py`'s held tests (real git, real origin, a branch cut from a deleted-ref ancestor) are unchanged and still pin it, which is the checkable form of "the sweep's behaviour did not change" — a second copy of those assertions in a new file would prove only that I wrote them twice. `test_health.py` and `test_strand_recovery.py` are untouched: their configs write no `merge_sweep_*` entry, so every existing verdict is byte-identical. |
| 2026-08-25 | doc-drift-sweep | 37 of 58 per-file counts were stale, all in the autoloop table; worst were `test_dashboard.py` 51->218, `test_tasks.py` 35->198, `test_contract.py` 80->191, `test_codex_provider.py` 33->139. Re-measured per file with `--collect-only`; narratives untouched, only the count cell. |
| 2026-08-25 | doc-drift-sweep | Aggregates re-measured: backend 1,320 collected (ledger stopped at 894), autoloop 3,672 (doc said 790 as of 2026-08-01), bare root pytest 4,040. The A2 arithmetic now reconciles again: bare `pytest` 1,196 + `test_document_package.py` 124 = 1,320. Root 368 = 97+124+147 and frontend 322/44 were already exact. |
| 2026-08-25 | split-04 | `autoloop/tests/test_task_split.py` gains a ninth section: 15 test functions, 19 collected, since one is parametrized over the five entries of `CRASH_BOUNDARIES` — every durable write boundary in `_dispatch_ceiling_split`, in order. Each is crashed at with `Boom`, a RuntimeError nothing on the path catches: deliberately not one of the caught types, because a caught exception is the SYNCHRONOUS error path and what is being simulated is the process going away mid-sequence. |
| 2026-08-25 | split-04 | The load-bearing helper is `reopen`, a second process over the same state directory: every store is rebuilt and the registry is RE-READ from `tasks.json`. Asserting against the crashed process's own in-memory registry is the likeliest way a crash-recovery test passes while checking nothing, and re-reading also puts the `_persisted_superseded_by` round trip under every claim about which children the parent was retired into. |
| 2026-08-25 | split-04 | ONE predicate at all five boundaries, `assert_stores_agree` — deliberately not "the parent is retired and its artefacts are gone", which is false at the two boundaries before the registry write lands and would therefore grade only half the sequence. It asserts the three stores tell one story and no unfinished split is still recorded; each boundary carries the arm it provably produces. |
| 2026-08-25 | split-04 | `test_without_the_reconciliation_the_crash_really_does_leave_them_disagreeing` is the counter-pin: without it the parametrized test grades nothing, because a sequence that was already consistent would pass it. It asserts the crash after the registry save really does leave a retired parent with a live record and a live worker. |
| 2026-08-25 | split-04 | Fail-open negatives, where this class of round dies. A corrupt marker must be REPORTED, never read as absent. `child_ids` as a bare string must be refused — `tuple("t1-a")` is five single-character children, so the set comparison could never match and the marker would report AMBIGUOUS forever with a green log. A marker naming other children must move nothing. And a marker that cannot be WRITTEN must refuse the split rather than proceed unprotected. |
| 2026-08-25 | split-04 | Bound tests, without which the feature is satisfied by never reconciling at all: a start with nothing in flight logs nothing, a completed split leaves no marker, reconciling twice changes nothing and files no second copy of a half already moved, a retirement that still fails keeps the marker and is finished by the next start, and a run wired without an execution store keeps it rather than clearing it. `test_the_reconciliation_runs_at_startup_and_not_only_on_demand` asserts `run()`, not the method. |
| 2026-08-25 | split-04 | Validation through the worker's advisory channel. Run 1 of 3 FAILED at `ruff check .` on one unused import (`SplitAcceptance`, imported into `orchestrator.py` and never referenced there), leaving both pytest commands NOT RUN — which is no evidence either way, not a pass. Run 2 of 3 then PASSED all three configured commands in order: `ruff check .`, `python3 -m pytest -n 4 autoloop/tests -q`, `python3 -m pytest -n 4 tests/ -q`. |
| 2026-08-25 | split-04 | Run 2's snapshot predates every documentation edit, so run 3 of 3 was taken over what was then the final tree — the run covering the note-length and tracker-shape checks in `test_docs_merge.py`. The backend suite is not among the configured commands and is untouched by this change. |
| 2026-08-25 | split-04 | One change POSTDATES run 3's snapshot and is disclosed rather than re-run, the budget being spent: `_report_split_blocker` no longer returns early when its blocker is already open, so the transcript entry is written every start. New `test_every_start_reports_an_unreconciled_split_even_with_the_blocker_open` pins it — two starts, two records, one blocker. Reason in the next line; the executor's own run after the agent returns is the verdict. |
| 2026-08-25 | split-04 | Why: `_report_strand_blocker`'s early return is safe because its task is IN PROGRESS, and the parent here is RETIRED in every shape that reaches the method — `cli._reconcile_retired_blockers` resolves open blockers naming a retired task, so a report living only in the blocker store could be closed out from under the condition it describes, leaving the marker in place with nothing saying so. The transcript is the half nothing else closes. |
| 2026-08-25 | split-04 | Run 3 of 3 FAILED, and the failure is an artefact of the advisory channel rather than a defect — stated plainly because it is the run a reviewer sees. `orchestrator.py` was edited WHILE run 3 was executing (the blocker-logging change above), and all three failures are `inspect.getsource` assertions over that file: `test_m1_hardening.py::test_push_refused_protected_is_actually_emitted_for_a_protected_branch_refusal` and two `test_task_inbox.py` "both callers" tests. Shifting the lines under a live `getsource` is what broke them. |
| 2026-08-25 | split-04 | Grounds for that reading, not a hope: none of the three touches code this task changed — they read `_dispatch_task_push` and `_drain_task_inbox`, both untouched — and run 2 PASSED all three over byte-identical Python. `docs/COMMON_ERRORS.md` §2 already documents this exact artefact (recov-01, 2026-08-24), naming two of the same three tests. The budget was spent, so no quiescent re-run was possible; the executor's own run after the agent returns is the verdict and is the check that settles it. |
| 2026-08-25 | halt-02 | New `autoloop/tests/test_autonomous_recovery.py` — see its table row above for the sections. No existing test file is modified: the interception is inside `_to_needs_user` and is inert with the flag off, so every park test already in the suite grades the same behaviour it graded before. `test_blockers.py`'s default-classification and continuous-mode tests are the pre-existing regression for that, and they are deliberately not duplicated here. |
| 2026-08-25 | halt-02 | The test this round would be worthless without is `test_with_the_flag_off_every_named_code_parks_exactly_as_before`, parametrized over all seven codes halt-02 names (the retired rotation one included). Everything else grades a feature; that one grades the claim's reversibility half, which is the part a reviewer cannot check by reading a diff. Its sibling covers the case a blanket assertion would get wrong — `crash_reconciliation_ambiguous` is `task_fatal` at its OWN site, so "off means loop_fatal" would be false for it. |
| 2026-08-25 | halt-02 | Validation through the worker's advisory channel, reported exactly as it came back. Runs 1 and 2 of 3 BOTH: `ruff check .` PASS, `python3 -m pytest -n 4 autoloop/tests -q` TIMEOUT at `run_validation_commands`' 1800s bound, `python3 -m pytest -n 4 tests/ -q` NOT RUN. A TIMEOUT names no failing test and is not evidence of one; the NOT RUN line is no evidence either way. So the autoloop suite is UNPROVEN here and the executor's own run after the agent returns is the verdict. |
| 2026-08-25 | halt-02 | Grounds for reading those two timeouts as LOAD rather than a hang — checkable, not asserted. Nothing in the existing suite can reach the new code: autonomy is off by default and `_autonomy_plan` returns `None` before touching anything, so every pre-existing park test grades byte-identical behaviour. The one new test that drives `Orchestrator.run` is bounded by `max_steps=5` (added between runs 1 and 2; run 2 still timed out, which is what makes a loop in THAT test an unlikely cause). Every other new test is straight-line, with no sleep, no subprocess and no network. |
| 2026-08-25 | halt-02 | The fail-open the corrupt-record test exists for: `open_recurrences` reads through `open_blockers`, so an unreadable record RAISES. Had it swallowed the error and returned 0, the loop would have read a full unspent budget off evidence nobody could read and retried on it — the exact "a check that silently passes when what it needs is absent" shape. The test plants `{ not json` in the blockers directory and requires `StateCorruptError` out of the park itself. |
| 2026-08-25 | halt-02 | Run 3 of 3 WAS requested, over the final tree, and had NOT returned when the agent finished — each of the first two spent the full 1800s before reporting, so the third was still in flight. Recorded rather than left implied: a requested run nobody mentions reads as a result withheld. Nothing above depends on it, and the executor's own run after the agent returns is the verdict for the whole tree. |
| 2026-08-25 | halt-03 | New `autoloop/tests/test_stale_record_rebuild.py` — see its table row above for the sections. ONE assertion changed in an existing file: `test_autonomous_recovery.py`'s set-equality now reads `TRANSPORT_RECOVERIES`, halt-02's own half, rather than the merged table it would fail against on every later addition. Nothing was removed, and the same test still requires that half to be reachable through the one lookup. |
| 2026-08-25 | halt-03 | The test this round would be worthless without is `test_a_task_free_code_is_rebuilt_with_no_task_in_flight`. halt-02's gate turned every plan off when `_autonomous_set_aside_task` found nothing to quarantine, and three of the six codes here carry no task and never could — so a suite that seeded `state.task_execution`, as every halt-02 test does, would pass green against code that leaves exactly the loop-halting codes halting the loop. It runs with `in_flight=None`, which is the state the real park sites are in. |
| 2026-08-25 | halt-03 | The second is `test_a_corrupt_state_error_is_not_rebuilt_at_all`, driven through `run()`. `StateCorruptError` subclasses `StateError` and reaches the same handler, so without the `recoverable=False` veto a store the loop CANNOT READ would trigger a code whose action discards the round and re-dispatches — the fail-open shape halt-02's own corrupt-record test exists to refuse, inverted. Its counter-pin drives an ordinary `StateError` through the same path and requires the rebuild, so the veto cannot pass by disabling everything. |
| 2026-08-25 | halt-03 | Eight refusal tests assert the record SURVIVES, not merely that the loop parked. A rebuild that archived a published candidate would satisfy "no operator step" while destroying approvable work — budget-01's record was archived 54 seconds before the reviewer returned PUSH for that exact candidate — so `execution_store.load()` is asserted non-None in each. `test_an_unknown_record_kind_rebuilds_nothing` is the guard that keeps the table safe to grow: `_autonomous_rebuild` has no fallback branch. |
| 2026-08-25 | halt-03 | Two tests exist because of a bug the adversarial pass found: `test_the_changeset_arm_is_not_captured_by_an_unrelated_task_in_flight` and `test_a_refused_task_free_rebuild_quarantines_nobody`. The changeset arm names no task, `state.task_execution` routinely names an unrelated one, and the in-flight fallback captured it — so the rebuild forgot an innocent task's binding and left the stale changeset queued. Both were written to FAIL against the earlier cut. |
| 2026-08-25 | halt-03 | Two AST checks read the park sites the real-git fixture REPLAYS rather than drives: `task_base_behind_head` must still pass a `task_id` and `kind="task_fatal"`, and `state_inconsistent` must still pass `recoverable`. A replay that drifted from its site would be a test of nothing, and a site that stopped passing the veto would switch the corrupt-state guard off silently — which is the failure class this feature is built to refuse, so it is pinned rather than trusted. |
| 2026-08-25 | halt-03 | REVISION: `test_stale_record_rebuild.py` gained `ChangesetWiring` — a real repository, a real queued `review-changeset` and a real Publisher — because `changeset_binding_missing` now re-renders its packet from git objects, which a hand-written dict of fake shas cannot exercise. The test the revision exists for is `test_the_redispatched_request_binds_and_its_approval_publishes_that_candidate`: it drives `_step_ready` twice and then `_step_executing`, and requires the upstream ref to equal the operator's candidate with the blocker never answered. |
| 2026-08-25 | halt-03 | THREE EXISTING TESTS WOULD HAVE GONE VACUOUS AND STILL PASSED, so they were re-keyed: the no-blocker-store, non-boolean-`enabled` and ceiling-of-zero tests all discriminated on `state.changeset is not None`, which the rebuild now preserves on the SUCCESS path too. Each asserts the OUTBOX is still the unbindable payload instead, and a fourth (`enabled=False` against a real changeset) was added because the parametrized off-position test runs without a git gateway, where this one code would refuse whatever the flag said. |
| 2026-08-25 | halt-03 | REVISION 2: new section 5b, four tests on the TASK arm's presence probe. `assert_parked_with_everything_intact` is the point — "it parked" alone also passes against code that archived the record and THEN parked, since the archival is durable before the payload is built, so each park asserts the record, the worker, the `sent_postcommits` entries (incl. the production-shaped `alr-presented`) and `recut_count == 0`. `test_only_gits_own_absent_answer_archives_the_record` runs exit 1 and exit 128 side by side so the pair cannot drift. |
| 2026-08-25 | halt-03 | ONE existing test was REWRITTEN, not supplemented: `test_push_candidate_unresolvable_on_the_task_arm_archives_and_requeues` made `read_commit` raise while the object was still there, which under a tri-state probe means `cat-file -e` answers 0 and the correct outcome flips to a rebuild. It now scripts both probes through the new `scripted_worker_gateway` seam. `PostcommitWiring`'s `"missing"` sha moved from the null oid to `ABSENT_SHA`: the all-zeros name is git-special and can die 128, which is not an answer. |
| 2026-08-25 | halt-03 | Four new refusal tests, each written to fail against a fail-open: a queued entry missing `dest_ref` (re-deriving it would read `git.current_branch()` and could rebind to a destination nobody named), a candidate that no longer renders (a raise inside a park handler kills the process), no git gateway (a packet not rendered from the repository is a review of something nobody was shown), and a displaced packet's `outbox_attachment` riding onto the rebuilt one. |
| 2026-08-25 | halt-03 | REVISION: the two push-arm tests ran on the collaborator-free `build()` fixture, which passes NO execution store — so they could only ever assert that pointers were dropped, which is exactly the half remedy that was refused. Both are rewritten onto a new `PostcommitWiring` (real repo, real worker repo with a real committed candidate, real record, optional Publisher) and both now DRIVE `_dispatch_task_push` itself rather than replaying `_to_needs_user`, so the code, the `task_id` and the recovery are production ones. |
| 2026-08-25 | halt-03 | The test the revision exists for is `test_the_redispatched_task_request_binds_and_its_approval_publishes_that_candidate`: rebuild, then `_step_ready`, then a stamped approval through `_step_executing`, asserting `req.postcommit.candidate_sha` and that the upstream ref equals the candidate with the blocker never answered. Every assertion in it was false against the unbound payload — `req.postcommit` was `None`. |
| 2026-08-25 | halt-03 | Five more push-arm tests cover the shapes that must NOT re-present: an unresolvable record candidate (two ways — a pruned object at the real site via monkeypatched `read_commit`, and a record naming a sha or nothing), which archives through recut-01's path; an already-published candidate, which drops the pointer only; and two refusals that must park having dispatched nothing (`pending_request is None`, outbox untouched) — no execution store, and a task absent from the registry. |
| 2026-08-25 | halt-03 | A fourth AST check pins the split the dispatcher relies on: `push_candidate_stale` has one site and it names a task, `push_candidate_unresolvable` has two and exactly one names a task. A changeset site that started passing a `task_id` would route an operator's changeset into the task rebuild — the capture bug the earlier cut shipped, reached from the other end. |
| 2026-08-25 | halt-03 | `PostcommitWiring`'s ledger seeds an `alr-presented` entry naming the RECORD's own candidate, which is load-bearing rather than decoration: production always holds one, and without it `_recut_outstanding_verdict` never fires in any test here — the archive route would have been unreachable in production for the exact shape it exists for while the suite stayed green. It is what the `[missing]` parametrization now proves the `candidate_resolves` bypass against. |
| 2026-08-25 | halt-03 | Two tests for the record-is-gone split, both driving the real refusal site with `with_record=False`: a task returned to the queue has its stale pointer dropped and the loop carries on, and a task still `in_progress` with no record parks having dispatched nothing. The first fails against reading the record's absence alone, which parks the loop over a fault a recut had already cleared. |
| 2026-08-25 | halt-03 | REVISION: six tests now hold the changeset arm's drop to ESTABLISHED absence. One drops (`ABSENT_SHA`, the 40-hex name `cat-file -e` answers 1 for); five put the question beyond answering and assert the queued review is still there — no gateway, a probe dying with 128 through the new `probing_runner` seam, the repository deleted mid-test (OSError), an object git holds but will not read as a commit, and an entry naming no candidate. The old no-gateway test asserted the DROP and was the fail-open itself. |
| 2026-08-26 | halt-03 | REVISION follow-up: `test_a_probe_that_cannot_run_at_all_never_archives_the_record`'s docstring now names the EARLIEST escape on the unresolvable road — `_candidate_is_on_task_line`'s unguarded `is_descendant` at `orchestrator.py:9277`, not `read_commit`'s `except GitCommandError` one line later — and says why it is reported rather than fixed: both escapes are fail-LOUD and pre-date this change, so the code is never raised, no rebuild runs and no record is destroyed on evidence nobody gathered. Docstring only; no test added, removed or renamed. |
| 2026-08-26 | halt-03 | REVISION 2: the two-value parametrization named in the earlier note is split into three tests, so that note's mention of it describes a construct this line replaces. `ABSENT_SHA` still archives (git answered exit 1); a record naming NO candidate, and one naming no `worktree_path`, now PARK — each through `assert_parked_with_everything_intact`, which grew an `expected_ledger` keyword because the fixture seeds `alr-presented` only when the record has a candidate to have presented. Six probe tests now, not four. |
| 2026-08-26 | shrink-01 | ONE test line changed for the `execution_records.py` extraction, and it is the one that would have failed: `test_urgent_preemption.py::test_a_resumable_residue_is_left_paired_so_the_next_dispatch_continues_it` patched `worker_repo_is_reusable` on `orchestrator`, and `_worker_can_resume` now resolves that name in `execution_records`' globals. Patched there instead. No test was added, removed or weakened; the other three release tests run the REAL probe and were already correct. |
| 2026-08-25 | abort-01 | New `autoloop/tests/test_operator_abort.py`, in seven sections mirroring the claim: the agent kill, the validation kill, what an aborted round reports, the round's budgets/queue/mainline, the refund in isolation, which phases may be interrupted, and the CLI. Real git repositories and REAL SUBPROCESSES — the constraint is about process groups, and a fake handle cannot prove a grandchild died. No `claude` CLI runs, no network, everything under `tmp_path`. |
| 2026-08-25 | abort-01 | The load-bearing one is `test_an_abort_kills_the_agent_and_every_process_it_spawned`: a python process that spawns a python process and writes BOTH pids to a marker, aborted, then each pid probed with `os.kill(pid, 0)` until `ProcessLookupError`. The grandchild is asserted by its OWN pid, never inferred from the parent's exit — a wrapper that signalled only the direct child passes every behavioural test and fails this one. Its twin does the same for `killable_run`, which is the group `pytest -n 4` runs in. |
| 2026-08-25 | abort-01 | `alive()` counts `PermissionError` as ALIVE, not dead. Reading a probe failure as "the kill worked" is precisely the fail-open shape this file grades, and it would make every kill assertion pass on a machine where the probe cannot run. It cannot arise for a process the test itself started; it is written that way so the helper is not the thing that quietly turns the file green. |
| 2026-08-25 | abort-01 | The budget tests. `test_an_aborted_round_spends_no_attempt_and_counts_no_fault` is the direct one; `test_leaving_the_entry_open_would_have_become_a_fault_one_round_later` is the one that would otherwise be missed — it runs a SECOND dispatch, because `_reconcile_unfinished_attempts` settles an open entry as a fault at the START of the next round, so an abort that merely walked away is green for one round and charged on the next. Every arm re-checks `attempt_count + fault_attempt_count == len(attempt_ledger)`. |
| 2026-08-25 | abort-01 | `test_an_aborted_round_leaves_mainline_untouched` captures the primary checkout's HEAD sha, its `git status --porcelain` and its branch list before and after a whole aborted round, and compares all three. The task asked for this to be ASSERTED rather than argued: "the executor commits only after the agent returns" is why it holds, and this is the evidence that it does. Its sibling pins the worker side — no `candidate_sha`, no review round, and the work still uncommitted, which is what lets the next dispatch resume it. |
| 2026-08-25 | abort-01 | Two tests pin the two INDEPENDENT signals, since either alone would be a fail-open. `test_the_flag_alone_aborts_a_round_whose_outcome_never_said_so` drives a round returning `status="ok"` with the flag set: nothing is committed, nothing is charged, and the record must say `partial_work_measured: False` rather than present an ordinary summary as a measurement. `test_the_abort_is_reported_as_the_cause_not_the_killed_agents_own_failure` covers the other order — a killed agent comes back not-ok, and saying so would report a healthy agent as wedged. |
| 2026-08-25 | abort-01 | The phase rules. `test_an_abort_never_kills_a_step_while_a_review_packet_is_outstanding` puts the loop in `awaiting` with a pending request and requires the phase AND the request to survive and no `aborted_round` to be recorded — the "never abort in submitting/awaiting" constraint, checked by what SURVIVES rather than by what is refused. `test_a_parked_loop_reports_its_park_rather_than_the_abort` pins the check's position after the terminal test; `test_the_abort_flag_lives_outside_the_checkout_beside_pause` pins the escape-detector trap. |
| 2026-08-25 | abort-01 | Bound tests, without which the feature is satisfied by wrapping everything forever: an executor with no `abort_file` behaves exactly as before EVEN WITH a flag file present elsewhere on disk; `abort_aware_spawn(None)` IS `stall.spawn_supervised` by identity; `abort_aware_command_runner(base, None)` IS `base`. Plus a parity set for `killable_run` — exit codes, both captured streams, and the same two exceptions `validation._run_one_command` catches — because it replaces `subprocess.run` on the round's authoritative validation. |
| 2026-08-25 | abort-01 | `test_the_supervisor_sees_an_abort_as_an_ordinary_exit_not_a_stall` runs the REAL `stall.supervise` over the wrapper with an injected clock and sleep: the verdict must be `COMPLETED` with `report is None`, so no `StallReport` ever says a healthy agent an operator stopped had been silent for N seconds. `test_the_kill_escalates_to_sigkill_and_always_answers` covers the process that ignores SIGTERM — a `None` there would send `supervise` round its loop re-signalling forever — and that a second poll does not re-signal. |
| 2026-08-25 | abort-01 | NO existing test file is modified. That is the checkable form of "`pause` keeps its current meaning": every pause, preemption, shelve and attempt-budget test passes untouched, and `test_operator_abort.py` adds its own `test_abort_does_not_set_the_pause_flag_and_pause_does_not_set_this_one` on top. The backend suite is not among the configured commands and is untouched by this change. |
| 2026-08-26 | abort-01 | Added `test_the_agents_own_mid_round_validation_run_is_abortable_too`: the advisory channel `_advisory_for` builds is asserted to hold the WRAPPED command runner, by behaviour rather than by identity — one run before the flag reports PASSED and launches `ruff`, one after launches nothing and must not read as a pass. Until this, the constraint the task names FIRST (an abort landing while `pytest -n 4` runs mid-round) rested on a comment at `implement_executor.py:2695` saying both runs share one wrapped attribute; a comment is not evidence, and a green answer about commands nobody ran is the fail-open. |
| 2026-08-26 | abort-01 | REVISION, superseding this file's note above about the phase rules being checked by what SURVIVES: the survival assertions stay, and the refusal is now asserted too. `test_an_abort_never_kills_a_step_while_a_review_packet_is_outstanding` is renamed `..._is_refused_in_so_many_words_...` and requires `ABORT_REFUSED` plus an `abort_refused` transcript record; a new parametrized test covers ALL FIVE packet phases rather than the two the constraint named, and another covers a `pending_request` that outlived its phase in `ready`. |
| 2026-08-26 | abort-01 | Same revision: `test_the_loop_stops_between_steps_from_any_live_phase` loses its `SUBMITTING` arm to that refusal test and keeps `READY`/`EXECUTING` — a suite that asserted `ABORTED` from `submitting` would contradict the change. `test_the_packet_predicate_refuses_what_it_cannot_read` pins the fail-CLOSED direction of `state.packet_outstanding_reason` (no session, unrecognised phase), which is the one that matters: answering "no packet outstanding" when the input is missing would perform a kill, not skip one. |
| 2026-08-26 | abort-01 | New section 3b, the abort-then-resume race the executor report named. `test_a_flag_cleared_before_the_round_re_reads_it_is_still_an_abort` kills, unlinks the flag, and requires `EXECUTION_ABORTED` with an empty `fault_kind`; `test_the_executors_signal_alone_aborts_a_round_whose_flag_is_already_gone` runs the same shape through the real dispatch path and checks the budgets, the queue and mainline's sha. `test_the_ledger_records_the_kill_rather_than_inferring_it_from_the_flag` shows the record survives the file being deleted. |
| 2026-08-26 | abort-01 | The dangerous direction has its own test, because it is the one a stale ledger would break silently: `test_a_healthy_round_after_an_aborted_one_is_not_classified_as_aborted` runs two rounds through ONE executor and requires the second to validate and return `ok`. Also pinned — a validation TIMEOUT must not record itself as an operator abort (it would refund a round that genuinely failed), a refusal must not claim a kill in the report, and an executor with no flag path keeps a ledger nothing ever writes to. |
| 2026-08-26 | abort-01 | REVISION: three tests for the SUITE window, which section 3b left open — it covered a kill during the agent, not during the authoritative run. `test_a_suite_the_operator_killed_is_not_the_tasks_own_red_validation` and `test_a_killed_suite_is_never_reported_as_a_suite_that_did_not_run` drive the REAL executor through `flag armed mid-suite -> ledger recorded -> flag cleared`; `test_a_killed_suite_costs_the_task_no_attempt_once_dispatch_has_seen_it` replays that executor's OWN outcome through dispatch with the flag absent and reads the attempt/fault counters. |
| 2026-08-26 | abort-01 | Same revision, fourth test: `test_a_finished_suite_is_not_reported_as_one_the_abort_cut_short` covers the flag landing AFTER the suite completes — ledger empty, so nothing was killed. It pins the report against two overstatements ("cut short", "were killed") and pins the new check as UNCONDITIONAL rather than an extra clause on the failure branch: that suite PASSED and the round still ends as an abort, which is what the orchestrator's own flag read does with it anyway. |
| 2026-08-26 | abort-01 | Trap the first advisory run caught, worth knowing when writing any executor-level abort test: arming the flag BEFORE `execute()` does not exercise the race at all. `_run_implementation`'s pre-spawn check returns immediately, so the agent never runs, no files are written and the flag is still there to be re-read. The two race tests therefore arm, record and clear the flag INSIDE `WritingAgent.before_returning` — the only window in which an operator's abort can overlap a live agent. |
| 2026-08-26 | abort-01 | Second trap, this one about running the suite rather than writing it: three `inspect.getsource` structural tests (two in `test_task_inbox.py`, one in `test_urgent_preemption.py`) failed a run of this round's tree and passed the run before and after it, because `cli.py` and `orchestrator.py` were edited WHILE that run was executing — `getsource` holds import-time line numbers and re-reads the moved file. Entry in `docs/COMMON_ERRORS.md`; the rule is to let a run finish before touching either file. |
...
| 2026-08-26 | setaside-01 | `test_autonomous_recovery.py`: `test_a_park_site_that_names_a_task_wins_over_the_one_in_flight` is REPLACED — it pinned the bug (a park naming t2 during t1's round quarantined t2). New section 5b adds nine tests: the bystander is not set aside and the site's loop-fatal terminal survives, t2 stays READY through `cli._handle_parked_task`, a matching id still sets aside, whitespace normalises, and seven unusable `task_execution` shapes refuse. `build()` gains `dispatched=`. |
| 2026-08-26 | setaside-01 | The PAIR that guards my own fix rather than the bug: `test_the_dispatched_task_counts_as_the_round_even_before_its_record_exists` and `test_with_no_round_at_all_the_same_park_refuses_instead`. Same park, same code, same named task; only the seeded round differs. Asserting `task_fatal`/t1 alone would have echoed the fixture — the OLD code produced it too — so they discriminate on the refusal entry and on WHICH check `_rebuild_execution_record_at_head` refused at. Both fail if `state.current_task` leaves `_round_task_ids()`. |
| 2026-08-26 | setaside-01 | Sibling test EDITED, not mine: `test_stale_record_rebuild.py::test_a_task_missing_from_the_registry_is_never_archived` now seeds `task_execution` to `ghost-99` as well as naming it. Without that the set-aside guard refuses first and the test silently stops exercising the registry branch it is named for. Its claim is unchanged; the fixture now matches the shape that refusal is actually about — an execution record for a task the registry has lost. |
| 2026-08-26 | setaside-01 | REVISION: new section 5c in `test_autonomous_recovery.py`, five tests / ten cases (two parametrized) for the ONE-active-identity boundary. With `task_execution`=t1 and `current_task`=t2 disagreeing, a park naming EITHER id is refused as `round_identity_records_disagree`, `cli._handle_parked_task` leaves both tasks READY, the `task_base_behind_head` rebuild is never entered, and a park naming NO task is refused too. `test_the_active_identity_and_the_fallback_row_by_row` pins all five rows of the decision table straight off `_active_task_id` / `_autonomous_set_aside_task`. |
| 2026-08-26 | setaside-01 | Same revision, edits inside my own section 5b: the refusal payload's `round_task_ids` list became `active_task_id` + `execution_task_id` + `dispatched_task_id` + a two-valued `reason`, so its assertions read the new keys, and `test_a_task_named_by_neither_record_is_refused_even_with_both_seeded` now seeds the two records AGREEING — left disagreeing it would have proved 5c's refusal instead of its own. `build()`'s docstring and two test docstrings drop `_round_task_ids()`. No test removed or renamed. |
| 2026-08-26 | release-02 | `autoloop/tests/test_worker_publisher.py` gains section 15, 20 test functions (22 cases) for the new `discard` verb, beside section 14's `shelve` tests and reusing its `_stranded_round` fixture through a new `_quarantined_round`. The claim: both execution halves moved under ONE label, the blocker closed with the operator's reason and `answer` still `None`, the task `pending`. No test was removed, renamed or weakened. |
| 2026-08-26 | release-02 | Refusals pinned, each asserting that NOTHING moved on the way to refusing: completed task, operator hold, `in_progress` task, an open `loop_fatal` record, a blocker `kind` that is empty/blank/unrecognised (parametrized, 3 cases), an unreadable blocker store, a blank `--reason`, and an unknown task id. Regressions: `release` still refuses a `blocked` task with `task_not_in_progress`, and `answer` on a blocked task still leaves both execution halves on disk. |
| 2026-08-26 | release-02 | Atomicity is pinned with INJECTED faults rather than argued: `TaskExecutionStore.archive` raising (neither half moved), `WorkerRepoManager.quarantine` raising (record archived, worker survives — the split `retire_execution`'s record-first ordering produces), and `TaskStore.save` raising (every closed blocker written back open). All three require the task still `blocked` and the blocker still OPEN. The first also re-runs the command after undoing the fault and requires it to complete. |
| 2026-08-26 | release-02 | The merge-window test carries its CONTROL, because the obvious assertion is FALSE on this base: `state_of` maps every `blocked` row to BLOCKED_BY_OPERATOR and `cli._merge_window_blockers` exempts it, so a quarantined task's record holds nothing. The hazard begins when the task is QUEUED again (READY has no exemption). The test asserts exempt-while-blocked, held-when-requeued-without-retiring, then not-named after a real discard — the middle arm is what makes the third discriminating. |
| 2026-08-26 | halt-01 | New `autoloop/tests/test_budget_set_aside.py`, nine sections. Every one of the eight codes the claim names is exercised in BOTH flag positions; the discriminating cases are the two whose victim can only come from the in-flight fallback (`git_failure_budget_exhausted`, `policy_denial_budget_exhausted`), driven through the unmodified `cli._handle_parked_task` with two tasks seeded and asserting t1 BLOCKED_BY_OPERATOR, t2 READY, `next_ready()` == t2 and the session file gone. One sibling test was edited and renamed — see the fourth line below. |
| 2026-08-26 | halt-01 | Section 2 is the anti-drift guard: `_blocker_calls()` AST-walks `orchestrator.py` and asserts each of the eight codes is raised by the emitter, `kind` and task-naming shape the `PARK_SITES` table replays. Without it every behavioural test below could keep passing against arguments no site produces — the fail-silent shape `§9c-quinquies` describes. A call whose `code=` is a NAME (the delegation inside `_autonomous_fault_set_aside`) contributes nothing, which is correct: it re-raises another site's code. |
| 2026-08-26 | halt-01 | Section 6 pins the exclusion the task's own text corrects — with the flag ON and a task in flight, `iteration_budget_exhausted` still parks `loop_fatal`, quarantines nothing, keeps `resume_phase` and keeps the session file — and resists both "a site that names a task" and "repetition". Section 8 pins the ceil-01 composition: `_handle_attempt_ceiling` with a remedy left ASKS (no blocker written), and only the remedies-spent arm sets the task aside. |
| 2026-08-26 | halt-01 | Section 9's fail-closed edges, the ones a reviewer should check first: a refused set-aside on the fault-stop door keeps the FAULT STOP rather than becoming a `loop_fatal` park (no task in flight, a named bystander, disagreeing round records — three tests), a corrupt blocker record still RAISES rather than reading as unspent budget, `max_recovery_attempts = 10` buys these codes no retry, and a non-boolean `enabled` is not consent on the new door either. |
| 2026-08-26 | halt-01 | ONE sibling test EDITED and RENAMED, not mine: `test_stale_record_rebuild.py::test_the_two_halves_of_the_table_are_disjoint_and_merge_into_the_one_lookup` → `..._the_parts_of_the_table_...`. It asserted `AUTONOMOUS_RECOVERIES == TRANSPORT | STALE_RECORD`, which a third part necessarily breaks, and its name would have become false. Its claim is unchanged and now stated as a fold over every part the module exposes — pairwise disjoint, merge equals the union — so the NEXT part added fails there if it is not merged in. Nothing else was touched. |
| 2026-08-26 | halt-01 | `test_the_fault_stop_door_only_ever_performs_a_set_aside` INJECTS the three plans no table entry can produce for this door (`RECOVER_BY_RESUMING`, `_RESUBMITTING`, `_REBUILDING_AT_HEAD`) by patching `_autonomy_plan`, because the guard it tests would otherwise only ever be exercised by inputs that cannot occur — untested by construction, and the shape that quietly stops guarding when one CAN occur. Each must leave the fault stop intact: this terminal may become a set-aside and nothing else. |
| 2026-08-26 | split-03 | New `autoloop/tests/test_split_decision.py`, 49 test functions (55 cases; two are parametrized) in six sections, real git and real worker repos throughout: the `split` verb's contract, its acceptance, its refusals, dependency reachability, the outstanding-verdict bound, and the one-acceptance-two-triggers property. Every "nothing was changed" refusal is asserted as a registry snapshot compared byte for byte, plus no marker on disk and the parent's record and worker still live — the shapes a half-applied split would leave. |
| 2026-08-26 | split-03 | The discriminating tests, the ones a reviewer should read first. `test_a_successor_cannot_carry_a_split_of_its_own` proves ONE LEVEL is unrepresentable rather than merely refused (a nested `tasks` key dies at `unknown_keys`). `test_a_successor_depending_on_the_parent_is_refused` pins the cycle-from-inside-`retire` path a plan would otherwise reach. `test_a_split_answering_the_packet_that_presented_it_is_allowed` is the POSITIVE control the outstanding-verdict guard would be vacuous without. |
| 2026-08-26 | split-03 | Assertions written against fixtures the suite's own `dispatch()` helper makes easy to get wrong: it sets `last_response = None`, so an outstanding-verdict test that only calls it proves nothing. The refusal case seeds a `sent_postcommits` entry for a request this reply does not answer; the allowed case seeds a matching `LastResponse`. `test_a_successor_of_an_earlier_split_cannot_be_split_again` gives its target NO execution record, because the depth refusal must hold whatever state that successor's round is in. |
| 2026-08-26 | split-03 | ONE test renamed: `test_task_split.py::test_the_decision_vocabulary_is_unchanged` → `..._is_pinned`, now listing `split`. A test named "unchanged" asserting a changed set is a lie about what it pins; its ceil-01 claim (that task added no verb) is kept in the docstring. `test_contract.py` gains a `split` entry in `_COMPLETE_PAYLOADS`, which feeds three parametrized tests, and `test_tasks_forbidden_outside_plan` is generalised over every decision that does NOT carry a batch. No test was removed or weakened. |
| 2026-08-26 | split-03 | `test_recut.py`: the legacy-vocabulary parametrize now excludes `split` as well as `recut`, via the new `_ADDED_SINCE_THE_PREVIOUS_VOCABULARY` — including it would build a payload for a verb that had not been invented and assert it parses as legacy. Its `wanted_decision` case gains split's `task_id` and `tasks`, with a successor id distinct from the parent so the fixture does not read as the parser blessing a self-naming split. |
| 2026-08-26 | split-03 | Coverage LOST, stated rather than left to be discovered: the two ceiling park codes moved onto the `SplitOrigin` table, so they are no longer string literals in a `code=` keyword and `test_m1_hardening._emitted_blocker_codes` (an AST walk) no longer sees them. Neither is a `cli._RESOLUTION_PRECONDITIONS` key, so `test_every_precondition_key_matches_a_real_emitted_code` is unaffected; `test_split_decision.py` asserts both origins' full code sets directly, which is a stronger check than the walk gave them. |
| 2026-08-26 | split-03 | The contract's per-turn length ceiling MOVED, 4,550 → 5,300, for a tenth decision — the same "genuine new requirement" test that recut-01's move passed, with the accounting written into `test_contract_stays_within_its_budget`. TWO tests assert that number and both were changed: that one and `test_context.py::test_no_scheduling_advice_moved_into_the_context_block`, whose docstring names the other as the place the accounting lives. Move both or neither. |
| 2026-08-26 | split-03 | That number is the first here to be MEASURED rather than hand-summed twice over. A temporary parametrized probe (`len(CONTRACT_INSTRUCTIONS) > floor`, six floors) was run through the advisory channel — which reports failing test ids but no numbers — putting the length in (5,100, 5,200] and confirming the 5,112 hand sum from inside its own bracket. The probe was deleted; `test_split_decision.test_the_measured_length_brackets_the_ceiling` is the reading it left, asserting the tighter 5,200 so it fails first. |
| 2026-08-26 | split-03 | That bracket test is deliberately ONE-SIDED. A lower bound would fail on a legitimate compression of the instructions, which is the direction the whole budget exists to encourage; a positive anchor on the `split` entry's own first line is what stops it passing vacuously instead. Advisory run 1 also reproduced the `inspect.getsource` drift `docs/COMMON_ERRORS.md` records — three structural tests failed only because `orchestrator.py` was edited WHILE that run executed, and all three passed in run 2 with the file untouched. |
| 2026-08-26 | esc-02 | New `autoloop/tests/test_observed_checkout.py` (~28 tests, four sections): the `ObservedCheckout` lifecycle and every fail-safe branch of it; where the clone may live (`validate_observed_checkout`, both nesting directions); an end-to-end pair proving the SAME two writes park the loop inside the clone and do not outside it; and the two operator-facing rechecks. Real git repositories throughout, no fixtures imported from other modules. |
| 2026-08-26 | esc-02 | The ignored-path assertions are written against the vacuity a fresh clone invites: a brand-new clone has no ignored files at all, so "ignored paths are still enumerated" would pass while proving nothing. Each test CREATES `.ruff_cache/0.14.1/…` inside the window and requires the park, and `test_the_very_same_writes_inside_the_observed_clone_are_still_loop_fatal` additionally asserts `git status --porcelain` in the clone is empty, so the ignored half cannot be carried by the untracked one. |
| 2026-08-26 | esc-02 | `test_without_a_configured_clone_the_primary_checkout_is_still_watched` is the negative control for the opt-in wiring: it asserts the unconfigured deployment still parks for a write into the primary checkout AND still seeds workers from it, so "no clone configured" can never quietly become "nothing is watched". `test_extra_shas_are_pinned_as_refs…` exercises a real `git fetch` of a pinned commit rather than asserting the ref exists. |
| 2026-08-26 | esc-02 | `test_m1_hardening.py`'s curated `security_and_environment_codes` gained `observed_checkout_unusable`, so the new loop-fatal park is required to carry a `_RESOLUTION_PRECONDITIONS` recheck. Nothing else in that file changed; its escape-detector tests all build orchestrators with no observed checkout and therefore still exercise the primary-checkout path unchanged. |
| 2026-08-26 | esc-02 | Validation run inside the worker covers `ruff check .` and `pytest tests/` only — the repository's configured commands — so NONE of the tests above executed in it. `autoloop/tests` needs its own runner (`python3 -m pytest autoloop/tests`), which this round had no way to invoke; read the advisory PASS as lint plus the root pipeline suite, never as evidence about this file. |
| 2026-08-26 | esc-02 | Correction to an earlier esc-02 line above, measured this round: the configured validation IS three commands — `ruff check .`, `python3 -m pytest -n 4 autoloop/tests -q -p no:cacheprovider`, then `python3 -m pytest -n 4 tests/ -q -p no:cacheprovider` — so `autoloop/tests` DOES run in the advisory channel and every test in this file is covered by it. The earlier "ruff and tests/ only" reading was wrong; it stays where it is because these notes are append-only. Fail-fast still applies: a ruff finding leaves both suites NOT RUN, which is not a pass. |
| 2026-08-26 | esc-02 | `test_observed_checkout.py` §5 adds three, all for paths that touch a worker WITHOUT a fresh `WorkerRepoManager.create`: the carry-forward fetches the moved head from the clone (the recorded `merge_foreign_commit` argument, with the real merge then run against it), an unusable clone parks that path `observed_checkout_unusable` and not `task_base_behind_head` with the worker untouched, and the sync boundary is not gated on the worker-repo mechanism. The `Orchestrator.__new__` fixtures in `test_rebase_stale_base.py` and `test_base_refresh_notes.py` set the three `_observed*` fields. |
| 2026-08-26 | select-01 | `autoloop/tests/test_test_selection.py` gains a section of 13 tests for per-path attribution plus two real-repository tests. No test was removed, renamed or weakened. `test_a_non_python_change_runs_the_full_suite` is left byte-identical on purpose: nothing `scaffold()` writes names `docs`, `.md` or `NOTES`, so that path is still unattributable and still widens — the evidence that the fallback was narrowed rather than rewritten around. Only its docstring gained the reason. |
| 2026-08-26 | select-01 | The two discriminating tests. `test_an_unresolvable_path_no_longer_vetoes_the_paths_that_resolved` asserts both halves of the union in one diff — a graph-selected test AND an attribution-selected one, with two unrelated files omitted. `test_every_test_naming_a_tracker_still_runs_on_a_tracker_change` asserts the soundness claim over the WHOLE population read off the real checkout: every `autoloop/tests` file naming one of seven tracker filenames, none missing from a `docs/SUMMARY.md` selection. |
| 2026-08-26 | select-01 | The fail-open guard gets its own pair. `test_a_file_that_cannot_be_read_becomes_a_seed_for_every_path` pins the closed shape — an unreadable file is seeded for every unresolved path rather than dropped, so the scan cannot silently pass on input it never read, and the guard does not rest on `build_import_graph` having marked the same file `opaque` first. `test_an_unparseable_file_is_still_selected_on_a_documentation_only_round` pins that the opaque frontier reaches an attribution-only round too. |
| 2026-08-26 | select-01 | Validation evidence, stated as what it is. Advisory runs 1 AND 2 of 3 each gave `ruff check .` PASS, `python3 -m pytest -n 4 autoloop/tests -q -p no:cacheprovider` TIMEOUT, `tests/` NOT RUN — §4h stops at the first failure, so read NOT RUN as no evidence either way. The advisory channel caps ONE run at `implement_executor.ADVISORY_VALIDATION_TIMEOUT_SECONDS` = 600s, deliberately far below the 1800s `run_validation_commands` default the executor's own run uses, so a TIMEOUT there is not the executor's verdict. |
| 2026-08-26 | select-01 | Not dismissed as ambient, because del-01 (2026-08-25) recorded three advisory runs PASSING all three commands. So this file's real-repository cost was cut instead: `real_repository_graph()` and `tracker_round_selection()` cache the checkout walk, taking it from four graph builds to four — i.e. the two NEW real-repo tests add none. Six heavy test modules merged between del-01 and here (esc-02, split-03, halt-01, release-02, setaside-01, abort-01), which is the other candidate and is not this task's to fix. |
| 2026-08-26 | advis-01 | `autoloop/tests/test_agent_self_validation.py` gains section 10: 10a covers the zero-request hand-back (it fires, it is bounded at `1 + ADVISORY_ZERO_CALL_RETURNS` invocations for an agent that never asks, an allowance of 0 or -3 hands nothing back, an agent that DID ask is untouched, a failed agent is never handed back, a failed hand-back keeps the first result, every report is carried, and the section is echo-safe). No test was removed, renamed or weakened. |
| 2026-08-26 | advis-01 | 10b covers UNANSWERED: a `PENDING` with no `RESULT`, the port-05 shape (a delivered FAILED run plus an undelivered second ask — both facts present, "its last run FAILED" gone), a request the watcher never took, residue from a dead round counted as nothing, a completed red run still reported FAILED, a broken channel counted as answered, and `NOT OFFERED` outranking every counter under it. |
| 2026-08-26 | advis-01 | Two of those are deliberately deterministic rather than timing-dependent: `test_an_ask_the_watcher_never_took_is_still_an_ask` holds the watcher INSIDE a blocking run so the second request provably cannot be taken, and `test_a_broken_channel_answers_the_agent_and_is_not_reported_unanswered` calls `expose()` + `serve_once()` by hand instead of `start()`, whose watcher would race a single hand-driven serve. |
| 2026-08-26 | advis-01 | One existing test changed, in `test_operator_abort.py`: `test_an_executor_with_no_abort_file_behaves_exactly_as_before` now asserts `agent.runs == 2`. Nothing abort-related moved — `executor_with` configures validation, so the channel is offered, and that agent never uses it, which is now one hand-back. Its claim (the round ran to completion with no abort capability) is unchanged and still asserted. |
| 2026-08-26 | advis-01 | Three test helpers pin `advisory_zero_call_returns=0`: `test_scoped_delete.py`, `test_scope_cleanup.py` and `test_scope_revert.py`. Each drives a REAL executor with a scripted agent that indexes its script by CALL while every step is written as one call per ROUND, so a hand-back inside round 1 would consume round 2's step and each test would grade a mechanism it is not about. That is a property of the fake, not of the executor; the hand-back has its own coverage in §10a. |
| 2026-08-26 | advis-01 | Validation, stated as what it is. Advisory run 1 of 3 gave `ruff check .` PASS, `python3 -m pytest -n 4 autoloop/tests` TIMEOUT at the channel's 600s per-run cap, `tests/` NOT RUN — the same cap select-01 hit twice the same day, and §4h fail-fast makes NOT RUN no evidence either way. So NONE of the tests described in the four lines above has been observed to pass here; they are owed to the executor's own 1800s run, which is the verdict. |
| 2026-08-27 | advis-01 | REVISION. One §10a test is REPLACED: `test_the_hand_back_is_bounded_and_a_stubborn_agent_is_forwarded_anyway` becomes `..._and_a_stubborn_round_is_withheld_from_review`, asserting the same bound (`1 + ADVISORY_ZERO_CALL_RETURNS` invocations) plus `status="error"`, `WITHHELD from review`, empty `fault_kind`, `validation == "not run"` and NO command launched at all. A round that never ran the suite can no longer reach the reviewer as a candidate. |
| 2026-08-27 | advis-01 | `test_a_hand_back_whose_agent_fails_keeps_the_first_reports_round` is renamed `..._keeps_the_first_reports_account` and flipped to withheld: every report assertion survives (first invocation's account carried, the torn one's text not, no "implementation agent failed", no `fault_kind`), because its premise — "a round that would have been reviewed" — was void once `asked == 0` stopped being reviewable. |
| 2026-08-27 | advis-01 | Four new §10a tests. The allowance-zero pair now asserts BOTH halves are off (renamed `..._disables_both_halves`); `test_the_shipped_allowance_enforces_the_contract_and_no_operator_can_zero_it` reads `cli.py` and pins `>= 1` plus "not configurable"; `test_a_round_whose_ask_went_unanswered_is_never_withheld` is the interaction between the two behaviours; `test_an_abort_outranks_the_withhold` proves the operator's button charges no attempt. |
| 2026-08-27 | advis-01 | The unanswered-round test is deterministic, not timed: it substitutes an `AdvisoryRendezvous` subclass that signals after its FIRST poll and then parks for 300s, and the agent waits for that signal before writing its request — so the watcher provably cannot take it and `stop()`'s sweep is what counts the ask. Same technique as §10b's blocking-run tests, one level up. |
| 2026-08-27 | advis-01 | `test_postcommit_flow.py` gains `test_a_round_that_never_ran_the_suite_produces_no_candidate_to_review`: a REAL `ImplementExecutor` at the default allowance, driven through `Orchestrator._dispatch_executor`, asserting no `candidate_sha`, `review_round == 0`, no `CommitIntent`, neither validation gate run, `attempt_count == 1` / `fault_attempt_count == 0`, and the summary reaching the reviewer. Only this file can show the consequence. |
| 2026-08-27 | advis-01 | Eight more construction sites across seven files pin `advisory_zero_call_returns=0`, one line each and for one reason: their stand-in agents cannot use the channel, so every round they drive would end WITHHELD and stop grading what the file is about. `test_implement_executor.py`, `test_operator_abort.py` (both), `test_postcommit_flow.py`'s B4b executor, `test_test_selection.py`, `test_validation_env.py`, `test_validation_failfast.py`, `test_validation_parallelism.py`. Three others were already pinned by the 2026-08-26 round. |
| 2026-08-27 | advis-01 | Five sites inside `test_agent_self_validation.py` itself are pinned for the same reason — §2's same-launch test, §5's two executor-owns-the-verdict tests, §6's parametrized status test and §6's echo test. Each grades a branch a withheld round never reaches; §10a grades the withhold at the real allowance, and §6's echo claim is graded there too by `test_a_handback_cannot_be_talked_out_of`. |
| 2026-08-27 | advis-01 | Two §6 tests are deliberately NOT pinned and gained a `WITHHELD not in summary` assertion instead: `test_a_round_that_changed_nothing_still_reports_the_channel` and `test_a_missing_declared_validation_cwd_still_reports_the_channel`. Both make zero advisory requests, and both must keep reporting the MORE fundamental refusal — which is what pins the withhold's late placement rather than merely describing it. |
| 2026-08-27 | advis-01 | Correcting the 2026-08-26 line above about `test_operator_abort.py`: `test_an_executor_with_no_abort_file_behaves_exactly_as_before` now asserts `agent.runs == 1` again, because `executor_with` pins the allowance off for that whole file. Its claim is unchanged and still asserted — the round ran to completion with no abort capability. |
| 2026-08-27 | advis-01 | REVISION 2. `test_a_round_whose_ask_went_unanswered_is_never_withheld` was failing, and the implementation was wrong rather than the assertion: the hand-back loop ran before `stop()` swept, so an unconsumed request read as no ask and the round was re-invoked. Its `len(specs) == 1` is kept verbatim; the executor now reads `AdvisoryRendezvous.ask_outstanding()`. Its parked-watcher subclass is lifted to a shared `ParkedWatchRendezvous` and it subclasses that to capture the instance. |
| 2026-08-27 | advis-01 | Two new §10a tests grade that predicate directly. `test_an_unconsumed_ask_is_visible_before_the_sweep_counts_it` walks all four states — residue before `start()`, nothing written, a request nobody took (`asked` still 0), and after `stop()`. `test_a_broken_channel_leaves_no_outstanding_ask_to_count_twice` drives the unremovable-request branch by hand with the watcher parked and asserts the ask is counted once, never twice. Both deterministic, no sleeps. |
| 2026-08-27 | advis-01 | A third new §10a test, `test_the_ask_is_recorded_before_the_request_file_is_removed`, reads `_take_request`'s source and pins that `record_request_asked` precedes `_remove_entry`. Swapping them makes the ordered check fail OPEN, and no behavioural test would catch it. |
| 2026-08-27 | witness-01 | New `autoloop/tests/test_record_witness.py`, 29 tests. No existing test was added to, removed, renamed or weakened: `test_merge_sweep.py`'s archive-generation section is the regression guard for the extraction and had to keep passing UNCHANGED, since it reads the `unresolved` reason strings back (`no execution record, live or archived`, `merge it by hand`, `cannot be put in order`, `newest archived copy could not be read`, and the stamp naming the newest generation). |
| 2026-08-27 | witness-01 | Validation, stated as what it is. All THREE advisory runs gave `ruff check .` PASS, `python3 -m pytest -n 4 autoloop/tests` TIMEOUT at the channel's 600s cap, `tests/` NOT RUN — the same cap select-01, advis-01 and esc-02 hit, and §4h fail-fast makes NOT RUN no evidence either way. Ruff says nothing about any of this: `ruff.toml` excludes `autoloop/`. So NONE of the 29 tests has been observed to pass here; they are owed to the executor's own 1800s run, which is the verdict. |
| 2026-08-27 | witness-01 | What the timeouts DO show, as an inference rather than a fact: each run executed for the full 600s, which requires collection of the whole tree to have succeeded — so `dashboard.py`, `merge_sweep.py` and the new file import cleanly. A collection error under `-n 4` reports in seconds. That is import-level evidence only and says nothing about any assertion. |
| 2026-08-27 | witness-01 | The tests a reviewer should read first are the fail-closed ones, because every one of them is a way the new evidence source could clear a row it should not: an unorderable archive, a torn live record, a torn newest archived copy, an ancestry check answering `unknown`, a reader that RAISES, and no state directory at all — none may reach `witnessed` and none may read as `completed_unwitnessed`. `test_an_id_no_registry_could_have_issued_reads_no_file_at_all` fails without the traversal guard: the file `../sneaky` lands on is a real, readable, ancestral record. |
