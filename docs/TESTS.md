...
| 2026-08-22 | val-03 | The two halves are asserted together on purpose: that the later commands never reach the runner (the saving) AND that the summary still names them `NOT RUN` (the evidence). A test of the first alone passes against a runner that collapses to one verdict; a test of the second alone passes against one that ran everything anyway. |
| 2026-08-22 | val-03 | Four tests pin what must NOT change: a one-command passing run's summary asserted BYTE-EXACTLY (`"ruff check .: PASS"`, so an unconditional ordering note could not slip onto every round's `state.last_validation`), a failure in the LAST command adding no note, an empty list still passing, and a refused binary still launching nothing. |
| 2026-08-22 | val-03 | `test_the_note_names_only_levers_that_exist` asserts the summary contains neither `[audit]` nor `validation_run`. The note reaches the reviewer, and the first draft told them to set a config key this task decided not to ship — a false sentence inside a change about reporting honestly. |
| 2026-08-22 | val-03 | `test_neither_production_call_site_overrides_the_default` is structural (it reads `implement_executor.py` and `orchestrator.py` for the string `fail_fast`), because "production gets the default" is a claim about callers passing NOTHING, which no behavioural test at those sites can distinguish from a caller passing `True`. |
| 2026-08-22 | val-03 | `test_validation_parallelism.py::test_a_parallel_run_still_reports_pass_fail_per_command` now passes `fail_fast=False` — its subject is parallelism, and under the new default its third and fourth commands would legitimately be `NOT RUN`. That also gives the full-run mode an exercise from a test that predates it; no other existing test changed, and its count is unchanged. |
| 2026-08-22 | val-03 | Validation was NOT run inside the worker for this task: it has no shell, so `ruff check .`, a bare root `pytest` and `cd lexy-app/backend && python3 -m pytest -n auto` are the three commands owed to the orchestrator. The backend suite is untouched (nothing outside `autoloop/`, `docs/` and `CLAUDE.md` was edited). |
| 2026-08-22 | val-03 | The before/after timing figure the brief asked for could not be measured here either: `python3 -B ~/.autoloop/analysis/timing.py --section outcomes` lives outside the worker repo and needs a shell. What IS defensible is the arithmetic — a round refused on a docs or lint violation paid ruff + both full pytest suites + the serial `isolated` re-run against a 1282s median round, and now pays the first failing command alone. |
