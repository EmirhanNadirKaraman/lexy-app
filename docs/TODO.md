...
**Resolved 2026-07-27 — Unicode resolution.** Under the C-collation database Postgres folds ASCII only, so the old `lower(col) = ANY(python_lowered)` predicate never matched a surface containing an uppercase non-ASCII letter: 50 German `word_table` rows and 18 `phrase_table` canonicals were unreachable, reporting `unresolved` even for a byte-exact paste. Resolution now folds in Python via `services/text_norm.py`. Same root cause still open in `reading_service.find_catalog_item` and `get_page_word_statuses` (reading selections of umlaut words never reach the main SRS, and render without a status colour), `word_service.lookup_word_by_text` / `learn_word_anyway` (`ILIKE`), and `search_service` search + suggest — different features and query shapes, deliberately not fixed here. `subtitle-scraper` is clear: it matches exactly and never asks Postgres to fold.

**Resolved 2026-07-28 — word lookup + learn-anyway de-duplication.** `word_service` now resolves through `resolve_word_ids` (Python `normalize_key`) instead of `ILIKE`, so `/words/by-text?word=öl` finds a stored `Öl`. This closed a **data-corrupting** path, not just a display miss: the lookup miss routed users into "Learn anyway", whose `pos='X'` INSERT with `ON CONFLICT (word, language, pos)` cannot conflict with the same word stored as `pos=''` (backfill) or `pos='NOUN'` (scraper) — so accepting forked the surface into a second row and made it permanently `ambiguous` in vocabulary lists. `learn_word_anyway` now resolves first and reuses an existing row; `pos='X'` is written only for genuinely new words. Two forked rows (`gato`, `hola`, both `es`) predate the fix; cleaning them is a separate data decision. Autocomplete (`_suggest_words`) is the one remaining site that cannot be fixed this way — a Unicode-correct prefix match needs a normalized generated column plus an index, i.e. a migration, so it is its own task. A later audit also found `chat_service.match_learning_words:140` has the same bug — free-chat use of an umlaut word earns no progression credit. **Still open after this fix:** `chat_service.match_learning_words`; `reading_service.find_catalog_item` and `get_page_word_statuses` (the former additionally matches `phrase_table.surface_form` where vocabulary lists match `canonical` — **773 rows differ**, so the two entry points can bind the same phrase to different rows, independent of collation); `search_service` search; and autocomplete.

**Resolved 2026-07-28 — free-chat progression.** `chat_service.match_learning_words` pushed the fold into SQL (`LOWER(wt.word) = ANY($2)` fed Python-lowered tokens), so producing an umlaut word in free chat matched nothing and earned **no progression or SRS credit** — silently, with no error. The word half now inverts the join: fetch the user's tracked non-known words for the language and filter with `normalize_key`. Bounded by the user's own vocabulary (tens of rows), so it is cheaper than the old query as well as correct. The phrase half needed no change — it matches on `phrase_id`s the spaCy extractor returns and never asks Postgres to fold. **Still open:** `reading_service.find_catalog_item` (plus its `surface_form`/`canonical` mismatch), `reading_service.get_page_word_statuses`, `search_service` search, and autocomplete.

**Resolved 2026-07-28 — reading binding + shared resolver.** Extracted `services/catalog_resolver.py` so vocabulary lists and interactive reading answer "what does this text bind to?" the same way. `reading_service.find_catalog_item` had three bugs, all closed: it folded case in SQL (so an umlaut selection never bound and never reached the main SRS); it matched `phrase_table.surface_form` while lists matched `canonical`, and **773 German rows differ**; and a bare `fetchrow` silently first-matched an ambiguous surface, attaching mastery to a coin-flip sense. Ambiguity now **fails safe** — 410 German word surfaces have duplicate rows, so those selections stop propagating to the catalog rather than binding to a possibly-wrong one; they are still saved and reviewable on the reading schedule. That includes the `mastered` outcome, where the user explicitly declares mastery and now gets no catalog effect — intended, because the old path promoted a coin-flip row to `known` and auto-promotion is one-way, so a wrong `known` cannot be undone. Nothing real is lost by the `canonical` switch: every differing row is a placeholder blueprint whose `surface_form` is a fragment like `(Dat) (Akk) geben` that no book selection produces. `get_word_statuses_for_page` fixed too — its keys were SQL-lowered, so an umlaut word was filed under `'Öl'` while the frontend looks up `'öl'`. Keys are now plain `.lower()`, deliberately **not** NFC, because the frontend has no `.normalize()` call; adding NFC on both sides is a separate frontend task and no book block currently carries combining diacritics. **Resolved 2026-07-28 — autocomplete (migration 036). This closes the C-collation Unicode bug everywhere.** `/api/suggest` is the one lookup that could not fold in Python and filter: it fires per keystroke and needs an *indexed prefix*. So the normalization is persisted — generated columns `word_table.word_norm` and `phrase_blueprint.lookup_key_norm`, both `lower(btrim(normalize(col, NFC)) COLLATE "und-x-icu")`, verified to reproduce `normalize_key` on every current row (0 mismatches / 41,771 rows). `GENERATED` rather than app-maintained because three code paths insert into `word_table`, one of them the scraper in a separate process. Two things found while implementing: Postgres's `\m`/`\M` word boundaries are **ctype-dependent** and never match a non-ASCII token under the C locale (so phrase autocomplete was broken for umlauts independently of the fold — the boundary check moved to Python), and `_suggest_phrases` still carried the raw-regex sink that SECURITY.md **S19** wrongly claimed was gone file-wide; it is now **S20**, resolved, and S19's scope and verification check are corrected. `DISTINCT ON (word_norm)` now genuinely collapses casing variants — 417 German keys become one suggestion each instead of several, a visible dropdown change and the documented intent.

**Resolved 2026-07-28 — corpus search.** `search_service.search` folded case in SQL on both halves: `w.word ILIKE $1 OR w.lemma ILIKE $1` for words, and an `ILIKE`/word-boundary-regex pair against `phrase_blueprint` (6,921 of 43,549 rows carry non-ASCII text). Searching *öl* returned nothing though `Öl` was indexed against real sentences. Both halves now resolve to ids in Python via `normalize_key`; the SQL matches `= ANY(...)`. The two blueprint queries collapsed into one, with the whole-word/substring distinction moving into `_resolve_blueprint_ids`. That also removed a security sink: the old word-boundary predicate concatenated the raw search term into a Postgres regex, so `.*` matched all 43k blueprints — now `re.escape`d (SECURITY.md **S19**, RESOLVED). Multi-word search is untouched: it uses pg_trgm `similarity()`, which is fuzzy by design rather than a case-equality predicate. **Autocomplete remains deferred and is test-pinned as such** — `test_suggest_words_is_still_the_unfixed_sql_version` will fail when it is fixed, which is the signal to close this item. Separately and unrelated, 417 German / 1,944 Spanish surfaces have duplicate catalog rows and so report `ambiguous`.
...
2. ✅ **Pre-seed the permanent gloss cache** — **shipped 2026-07-28** as `scripts/seed_gloss_cache.py` + `services/gloss_seed_service.py`. **3,622 curated glosses seeded** from `words_4000_old.txt` column 3, all matched to existing `word_table` surfaces. Stored under the sentinel model `curated:words_4000_old`, which `translate_item_gloss` checks *before* the model-specific key — honest provenance, and the rows survive an `LLM_MODEL` switch (the key includes the model, so model-scoped rows miss after a switch). Skipped: 80 multi-entry cells, 36 multi-sense (`1) …; 2) …`), 287 translations longer than the 4-word gloss shape — all left to the LLM rather than guessed at. **Phrases deliberately not seeded**: only 29 curated headwords match a phrase surface out of 2,829, and the translations are for the bare headword, so it needs its own source.
3. ~~**Add phrase support to `word_list_service`**~~ — ✅ **done 2026-07-27** (+11 backend / +4 frontend tests).
4. **System-list schema + built-in lists — phase 1 ✅ shipped 2026-07-28 (migration 037).** `word_lists.user_id` is now nullable with `is_system BOOLEAN NOT NULL DEFAULT false`, plus a CHECK (`(is_system AND user_id IS NULL) OR (NOT is_system AND user_id IS NOT NULL)`) that makes a system list with an owner — and an ownerless private list — unrepresentable. That constraint, not the flag, is what makes the widened read filter safe. Reads are `(user_id = $2 OR is_system)`; writes stay user-only; `mark_unknown_as_learning` skips its late-binding UPDATE on shared rows while still applying per-user progression, so two users marking the same built-in list never interfere. A partial unique index on `(name) WHERE is_system` makes future seeding idempotent by name. **Phase 2 ✅ shipped 2026-07-28** as `scripts/seed_system_lists.py` + `services/system_list_seed_service.py`. Two lists seeded, **9,122 items**: *Top German Words* (4,087 surfaces — 3,831 resolved, 227 ambiguous, 29 unresolved) and *German Verb & Phrase Patterns* (5,035 — 4,780 resolved, 226 ambiguous, 29 unresolved). Ambiguous and unresolved rows are **kept** with `item_id=NULL`, matching user-list behaviour: the 227 ambiguous word entries are the most common words in the language (`ein`, `zu`, `im`, `auf`, `ich`), so dropping them would gut the list and first-matching would bind mastery to a coin-flip sense. Append-only and idempotent by constraint. **Worth knowing:** column 0 is stored verbatim, so nouns keep their article (`das Haus`) and bind to `phrase_table` collocations — 1,879 of *Top German Words* resolve as `phrase`, 1,952 as `word`. That is correct (the article is part of the German learning unit, and `das Haus`/`Haus` stay independently trackable) but the list name undersells it; consider renaming in phase 3. `b1_parsed.txt` (90%) is a later candidate; `b1_unparsed.txt` is not (69%, 641 unresolved). **Phase 3 ✅ shipped 2026-07-29.** `WordListsPage` splits the index into *Built-in lists* and *Your lists*; built-ins carry a “Built-in” badge and an explanatory line, show **no Delete control**, and keep Open / export / mark-unknown-learning. A list arriving without `is_system` is treated as user-owned, so a pre-037 backend does not silently make lists read-only. Hiding Delete is a courtesy — the guarantee is server-side (404). The *Top German Words* naming caveat is handled **display-only** via `SYSTEM_DISPLAY_NAME` (“Top German Words & Phrases”); the stored name is unchanged because it is the seeding idempotency key and renaming it would fork the list. **Mass-marking guard ✅ shipped 2026-07-29.** `mark_unknown_as_learning` caps each call at `MAX_LIST_WORDS` (500) and returns `remaining` + `capped`; the frontend confirms above 200 unknown words and reports the remaining count so the user can click again. Measured before the fix: reads are fine (22–25 ms for 4,087/5,035 entries) but each mark costs its own transaction — ≥4.4 s and ≥5.5 s floors for the two seeded lists, up to 9,560 SRS cards for one user, no undo. **Render guard ✅ shipped 2026-07-29 (frontend-only).** `WordListsPage` mounts `ENTRY_CHUNK` (200) detail entries at a time behind a “Showing N of M entries / Show more” control, so opening a built-in list no longer mounts 5,035 DOM nodes synchronously. No “Show all” — that would put the freeze one click away. Status counts, export and mark-learning all still read the full response; the slice is positional so ambiguous/unresolved entries are never filtered out. **Backend pagination ✅ phase 1 shipped 2026-07-29.** `GET /word-lists/{id}` takes optional `limit` (1–1000) and `offset` (≥0) that window `entries` only; `total` and `counts` remain whole-list on every page, so the status badges and the mark-learning count still describe the list. Omitting both is byte-identical to the old response, so no existing client changed. 514 KB → ~22 KB for a 200-entry window. **It shrinks the response, not the query** — `_load_entries` costs ~33–36 ms and counts need the fully resolved set, so every page still does the whole load; don't expect a latency win. **Frontend pagination ✅ phase 2 shipped 2026-07-29 (frontend-only).** `WordListsPage` consumes the params instead of slicing a full response: opening a list requests `limit=200&offset=0`, and *Show more* requests `offset=<entries already loaded>` and **appends**. `visibleCount` is gone — one paging concept, not two. The footer denominator is `detail.total`, so it reads "Showing 200 of 5,035" rather than the "Showing 200 of 200" a paged response would otherwise produce (a dedicated regression test pins this). *Show more* is disabled while in flight; a failed page keeps every already-loaded entry, shows an inline error, and stays retryable at the same offset. Two ordering hazards are handled and should not be re-introduced: a page arriving after the user opened a different list is dropped (`prev.list_id === listId` inside the functional `setDetail`), a late failure cannot write its error over the new list (`openListIdRef`), and the duplicate-page guard is a **set of `listId:offset:limit` keys** rather than a boolean — a single slot would forget list A's pending page while the user is on list B and re-issue it on the way back, and would also block list B's own paging behind list A's request. +16 frontend tests (306 → 322). **Note the payload win is the only win** — see the phase-1 note above: every page still runs the whole `_load_entries`, so this reduces bytes and mounted DOM nodes, not server time. **Optional follow-up, not started:** making `get_list` cheap enough that a page costs less than a whole load would mean computing `counts` in SQL rather than from the resolved entry list — a backend change with its own correctness risk, and worth doing only if list reads actually show up as slow. **Phase 4 open:** copy-to-my-lists, and admin management of system lists.
