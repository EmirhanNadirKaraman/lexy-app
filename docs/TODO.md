...
### 39. 🟡 Lemma-override layer — slices 1+2+3A shipped 2026-05-24; promotion (3B/3C) deferred
...
**Slice 3B/3C (DEFERRED) — promotion:** admin accept/reject endpoints; an accepted candidate writes a `lemma_override` row (`source='user_flag_reviewed'`) + flips the candidate status — the ONLY path from a user signal to extraction truth, through a human. 3C: optional LLM adjudication batch (no live calls in 3A/3B). Frontend flag button: none yet. Will need to invalidate the per-call override load if it moves to a cache.
