...
**Slice 3 (DEFERRED) — community signal + promotion (design items 3-5):** user flagging endpoint + `lemma_flag`/candidate table + promotion (admin/LLM adjudication). Plus an LLM seeding pass over existing canonicals. Will need to invalidate the per-call override load if it moves to a cache.
