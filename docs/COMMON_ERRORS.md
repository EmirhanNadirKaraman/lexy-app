...
| 2026-08-24 | split-02 | Second recurrence of §2's `inspect.getsource` entry, same day, cost one advisory run. Two symptom details worth adding to the ones there: it hits SEVERAL files at once (4 here — `test_m1_hardening`, `test_blockers`, two in `test_urgent_preemption`), and the body shown is consistently the method immediately BEFORE the subject, which reads like a plausible off-by-one regression rather than an artifact. Confirm by grepping the subject for the missing string; then wait for `RESULT #n` before editing again. |
