...
one, but `review-changeset` waits on a reviewer, and `smoke-browser` drove a
browser until brw-16 retired the command; a per-command wrapper would only
cover whichever ones somebody
...
| 2026-08-25 | brw-15 | Four entries in this file describe symptoms nothing can produce any more: `rotation_failed` naming an address, `rotation_cap_reached`, `rotation_unavailable` / "set browser.project_url" on a run with no browser, and the `conversation_rotated` line in the submission-never-appeared entry. They are left VERBATIM rather than rewritten — an OPEN blocker carrying one of those codes can still be on disk, and its recovery still applies to it. |
| 2026-08-25 | brw-15 | The new symptoms for those same faults: `send_rejected_twice` (two sends of one request disproven in a row — the park quotes the remedy for the transport in use) and `conversation_unusable` (the chat loaded and is broken, or a submission this loop made provably never appeared). Neither is recoverable by the loop: open the conversation, and if it is dead repoint `browser.conversation_url` and `reset` rather than resending. |
| 2026-08-25 | brw-16 | New symptom, and it is the expected one on an unmigrated deployment: `unknown conversation provider 'browser_chatgpt' — available: codex_app_server, codex_cli` on the first step, or `doctor` reporting `provider  FAIL`. That provider is retired; set `conversation.provider = "codex_cli"`. The config still LOADS and prints a notice saying exactly this, so the notice arrives one command earlier than the failure — read stderr on `status` or `doctor` before debugging the run. |
| 2026-08-25 | brw-16 | Two neighbouring shapes that are NOT bugs. `smoke-browser` is RETIRED: every invocation, flags or none, prints `smoke-browser: RETIRED — …` and exits 2, reading no config, building no provider and taking no lock. A command with nothing left to smoke, not a broken one — `doctor` is the preflight that still works. And `fallback_provider = "browser_chatgpt"` is read as `""` with a notice, so an exhausted codex allowance parks `quota_exhausted` instead of failing over — that park is the migrated behaviour, not a regression. |
| 2026-08-25 | brw-16 | `doctor` still runs `cdp` and `playwright` unconditionally, so both read FAIL on any host without the dedicated Chrome profile. Expected on a codex-only deployment and pre-existing rather than introduced here. The consequence to know: `login_expired`, `submission_ambiguous` and `git_failure_budget_exhausted` map to `cli._precondition_browser`, which re-checks those two names — so `answer` on one of those blockers is refused until a Chrome is running. Answer it with the profile up, or clear the blocker by hand. |
| 2026-08-25 | brw-16 | One more thing `smoke-browser`'s retirement takes with it: there is no command left that proves the configured transport with a live round-trip. `doctor` reaches it without submitting, and the first real exchange is now the first `run`. If you followed the self-upgrade runbook in `docs/AUTOLOOP_TODO.md` (`doctor` + `smoke-browser`, both green → promote), that gate now has one half — treat a green `doctor` plus a first successful `run` as the replacement rather than skipping the check. |
...
| 2026-08-25 | halt-03 | Trap when writing a test for `push_candidate_unresolvable` on the TASK arm: a fabricated 40-hex sha never reaches that refusal. `GitGateway.is_descendant` runs first and RAISES `GitCommandError` for an object git cannot resolve (rc 128) rather than returning False — deliberately, so "unresolvable" is never silently reported as "not a descendant". The site is only reachable when the object dies between that check and `read_commit`, so the honest test monkeypatches `read_commit` for one sha rather than inventing a missing one. |
| 2026-08-25 | halt-03 | REVISION, superseding the tail of the note above: monkeypatching `read_commit` alone is NOT enough once the handler probes presence tri-state. The object is still in the database, so `cat-file -e` answers 0 and the outcome flips from archive to rebuild — a test written that way now asserts the wrong branch. Script BOTH probes at the exit-code level (`test_stale_record_rebuild.scripted_worker_gateway`): `cat-file commit` 128 plus `cat-file -e` 1 means "git says absent", `cat-file -e` 128 means "git could not look". |
| 2026-08-25 | halt-03 | Related trap in the same tests: do not use the all-zeros 40-hex name as "a sha nothing holds". It is git's null oid and special-cased, so `cat-file -e` on it need not exit 1 — and under a tri-state probe an exit that is not 1 flips the case from archive to park, failing for a reason unrelated to what the test is about. `test_stale_record_rebuild.ABSENT_SHA` (`"0"*39 + "1"`) is the well-formed non-null name kept for this; a short or malformed name dies 128 instead. |
| 2026-08-26 | halt-03 | Silent trap anywhere a repo path comes off a record: `GitGateway(Path(""), policy)` does not fail. `Path("")` is `.`, so the gateway silently addresses the process's own CWD — in the loop that is the PRIMARY CHECKOUT — and every probe answers about the wrong repository, truthfully and with exit 0/1, so nothing anywhere reports an error. Guard the empty string BEFORE constructing the gateway; `GitGateway.__init__` touches no filesystem and cannot catch it for you. |
| 2026-08-26 | shrink-01 | Pointer fix for the preemption-residue entry above: `_repair_orphaned_record` is no longer in `orchestrator.py`. It, `release_task_to_pending`, `Release` and the five other release helpers moved to `autoloop/execution_records.py` (imported back, so `orchestrator.release_task_to_pending` still resolves and no remedy in that entry changes). Grep the new module when the entry sends you to the old one. |
| 2026-08-25 | abort-01 | New symptom, and it is WORKING AS INTENDED: `run` or `run --continuous` exits 0 immediately printing `aborted — resume clears the flag and starts working again`, with `status` showing `abort flag   yes`. Somebody ran `python -m autoloop abort`. The flag is `<workers_root>/../ABORT`, beside `PAUSE`; `python -m autoloop resume` (or `start`) clears both. Nothing is broken and nothing was lost — a `stopped` session with `stop_kind=aborted` carries the account in `status`. |
| 2026-08-25 | abort-01 | The trap that shaped this feature, worth knowing before adding a second kill switch anywhere: killing the AGENT is not killing the round. Since impl-02 the agent runs the validation suite mid-round through `AdvisoryRendezvous`, in the LOOP's process group rather than the agent's — so a kill that reached only the agent left `pytest -n 4`'s four workers writing into a worker repo nobody owned, and `_run_implementation`'s own `finally` then waited up to `ADVISORY_STOP_JOIN_SECONDS` (11 minutes) joining that thread. Two groups, two wrappers. |
| 2026-08-25 | abort-01 | Second half of the same trap, and it is the one a `subprocess.run` caller inherits silently: `subprocess.run`'s timeout path calls `Popen.kill()`, which signals the DIRECT CHILD only. A timed-out `pytest -n 4` therefore raises `TimeoutExpired` while four workers keep running. `implement_executor.killable_run` spawns with `start_new_session=True` and signals through `stall.ProcessGroupHandle` (`os.killpg`), killing the group BEFORE it raises. If you add a validation runner, spawn a session or inherit this one. |
| 2026-08-25 | abort-01 | No new RUNTIME error class was hit this round, so nothing was filed above the marker. One documented trap was avoided rather than hit and is worth restating because it applies to every new operator flag: an operator-writable file under `state_dir` is INSIDE the tree `escape_detector` snapshots around every write-capable agent call, so writing one mid-round parks the loop `loop_fatal`. That is why `ABORT` lives beside `PAUSE` outside the checkout — see the 2026-08-02 entry the pause flag earned. |
| 2026-08-25 | abort-01 | Worth knowing before you debug an advisory `TIMEOUT`: the AGENT's channel is bounded at `implement_executor.ADVISORY_VALIDATION_TIMEOUT_SECONDS` (600s PER COMMAND), while the EXECUTOR's own authoritative run uses `run_validation_commands`' default of 1800s. So a suite that fits in the second and not the first reports `TIMEOUT` to the agent and PASSES for the round. Read an advisory TIMEOUT as "no evidence either way" (§4h) about a slow suite, not as a failing tree — and check for a hanging test before assuming either. |
| 2026-08-26 | abort-01 | New symptom, also working as intended: `abort` prints `WARNING: the kill will be REFUSED where the loop is now` and the loop exits with `ABORT REFUSED — a review packet is outstanding`. Nothing is broken and nothing was killed: in `delivering`/`submitting`/`submission_*`/`awaiting` a reviewer may already hold — or have accepted — a request, so the loop stops between steps instead. `resume` continues from that phase with the packet intact; `pause` is the verb that means that stop. |
| 2026-08-26 | abort-01 | The trap behind this round's revision, and it generalises to every flag-driven kill switch: A FLAG IS A FILE AND SOMEBODY ELSE CAN DELETE IT. Reading it twice — once to kill, once to classify — means a `resume` landing between the two reads turns the operator's abort into a charged task failure, silently, and eventually into an `attempt_count_ceiling` park blaming the task for a wall the operator built. Record what your process DID (`implement_executor.AbortLedger`), and classify from that record. |
...
| 2026-08-27 | brw-18 | Two pointers above are now stale and are left VERBATIM rather than edited (append-only): the entry citing "the module is pinned by `autoloop/tests/test_chrome_restart.py`", and the `test_restart_wiring.py` `no_machine_access` reference. Both files were deleted this round. The behaviour each entry describes is unchanged — `autoloop/browser/` is untouched here — so both stay correct about the CODE; only their test citation is gone. `scripts/restart_autoloop_chrome.sh:31` cites the same deleted test and is outside this task's scope. |
| 2026-08-27 | brw-18 | Verified the two lines that note names, having first inferred one from a grep hit. Line 166 is the stronger case: "the module is pinned by `autoloop/tests/test_chrome_restart.py`" is now FALSE — nothing pins `chrome_restart.py`, which was that bullet's whole argument for the module over the retired `.sh`. Line 1607 is only an ANALOGY inside the `os.execv` entry, whose own fix (`test_self_upgrade.py`'s autouse fixture) is untouched; its referent merely dangles. |
