...
| 2026-08-25 | brw-16 | Two neighbouring shapes that are NOT bugs. `smoke-browser` with no `--provider` now smokes `conversation.provider`, so a bare run no longer touches a browser; asked for an unregistered one it prints `smoke-browser: FAIL — … is not registered` and exits 2 without taking the lock. And `fallback_provider = "browser_chatgpt"` is read as `""` with a notice, so an exhausted codex allowance parks `quota_exhausted` instead of failing over — that park is the migrated behaviour, not a regression. |
