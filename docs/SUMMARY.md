...
| Path | Purpose |
|---|---|
| `orchestrator.py` | Persisted state machine: ready → submitting → awaiting → executing (+ needs_user / stopped / failed). Failure routing, budgets, review-integrity enforcement. `_dispatch_executor` takes the produce-then-review commit path (pass 2a) for a real task when optional `worktrees`/`execution_store`/`intent_store` collaborators are supplied (all default `None` — every existing caller, incl. `cli.py`, is unaffected); audit always keeps the manifest-based authorize-then-produce path. On success the commit is created immediately (hooks enabled) and gated on structural + re-run-validation checks, never a prior chat approval; ANY failure parks in `needs_user` with the commit left exactly where it is (nothing here can roll it back). Building the actual review-packet message ChatGPT sees is pass 2b — a clean pass parks too today, saying so honestly. **Autoloop M2:** an optional `publisher: Publisher | None = None` constructor param, gated the same way (default `None` = unchanged); when set, `_dispatch_task_push` imports the candidate into the dedicated `Publisher` repo and publishes from there instead of pushing straight from the task's worktree — see `publisher.py`. |
...
| `git_gateway.py` | The only git runner — argv subprocess (no shell), policy-validated per call, idempotent commit, explicit-refspec push. Adopted commits use an **immutable-tree** path (`write-tree` → verify → `commit-tree` → `update-ref` CAS) and refuse when any commit hook is active, because `git commit` runs hooks that can rewrite the index after verification (see S21). **Produce-then-review path** (Autoloop M1): `commit_and_capture` commits with hooks ENABLED and reads the resulting sha honestly from `rev-parse HEAD` (never predicted), leaving review to `range_diff`/`commit_range_paths` against the immutable commit that already exists; `push_exact` publishes only one already-resolved `<sha>:<dest_ref>` refspec (F2/F5-hardened); `worktree_add`/`worktree_remove`/`worktree_prune`/`branch_exists` give each task its own linked worktree (pass 2a, see `worktree.py`). **Autoloop M2:** `fetch_object(source_path, want_sha)` pulls one already-existing object by literal 40-hex id from a local path (policy-whitelisted with F2-style checks); `__init__` gained an optional `env: dict | None = None` (default `None` = prior behavior for all 579 pre-M2 sites) so a gateway can be made to run under a scrubbed environment instead of silently inheriting the caller's ambient config — see `worker_env.py`. |
...
| `worktree.py` | Per-task worktree lifecycle (Autoloop M1 pass 2a): `WorktreeManager.create/remove/list_worktrees`. `task_id` doubles as a path component and a git ref component, so it's validated against `[A-Za-z0-9][A-Za-z0-9._-]*` plus an explicit `..` check before touching either. Relies on git's own refusal to check the same branch out into two worktrees at once as a fail-closed concurrency property, rather than reimplementing it. |
| `validation.py` | Shared `SAFE_VALIDATION_BINARIES` allowlist + `run_validation_commands` runner, used by both `audit/executor.py` (pre-commit repo-health summary) and the orchestrator's post-commit re-validation (pass 2a) — one definition so the two call sites can't drift on what's safe to launch. |
| `worker_env.py` | Autoloop M2: worker-side isolation. `worker_env()` builds the scrubbed subprocess environment (`GIT_CONFIG_NOSYSTEM=1` — NOT `GIT_CONFIG_SYSTEM=/dev/null`, which does not suppress Apple's compiled-in system gitconfig — plus `GIT_CONFIG_GLOBAL=/dev/null`, `GIT_TERMINAL_PROMPT=0`, SSH/askpass vars removed). `WorkerRepoManager` creates one isolated, no-remote repo per task (`git init` + a one-time local-path `git fetch`, never `git worktree add`, which would share the main checkout's `.git`). `verify_worker_isolation(git, expected_hooks_dir)` reports remote/pushurl/insteadOf/followTags/mirror/credential-helper/hook violations — `git` must be built with `env=worker_env()` (`WorkerRepo.gateway(policy)`) or it reflects the CALLING process's ambient config instead. Not wired into task execution yet — no repository task executor exists (`executor.NullExecutor` is a stub). See `docs/AUTOLOOP.md` §4c. |
| `publisher.py` | Autoloop M2: the only path through which a candidate commit is published. `provision_publisher_repo` idempotently creates a dedicated BARE repo (`state_dir/publisher.git`) with its own empty controlled hooks dir and a `remote.<n>.url` copied from the main checkout (a provision-time snapshot — see the class docstring's stated limitation). `Publisher.import_candidate` fetches a candidate by literal 40-hex sha from a worker's local repo path and verifies it via `read_commit` (never a separate `cat-file -t`, which the policy whitelist would deny); `Publisher.publish` reuses `GitGateway.push_exact` unmodified. `Orchestrator`'s optional `publisher=` constructor param routes `_dispatch_task_push` through it when supplied (default `None` = unchanged legacy behavior); `cli.py` does not construct one yet. See `docs/AUTOLOOP.md` §4c. |
...
| `tests/` | 611 hermetic tests — no network, no playwright, no live claude CLI (see `docs/TESTS.md`). |
