...
| Path | Purpose |
|---|---|
| `orchestrator.py` | Persisted state machine: ready → submitting → awaiting → executing (+ needs_user / stopped / failed). Failure routing, budgets, review-integrity enforcement. `_dispatch_executor` takes the produce-then-review commit path (pass 2a) for a real task when optional `worktrees`/`execution_store`/`intent_store` collaborators are supplied (all default `None` — every existing caller, incl. `cli.py`, is unaffected); audit always keeps the manifest-based authorize-then-produce path. On success the commit is created immediately (hooks enabled) and gated on structural + re-run-validation checks, never a prior chat approval; ANY failure parks in `needs_user` with the commit left exactly where it is (nothing here can roll it back). Building the actual review-packet message ChatGPT sees is pass 2b — a clean pass parks too today, saying so honestly. |
...
| `tests/` | 556 hermetic tests — no network, no playwright, no live claude CLI (see `docs/TESTS.md`). |
