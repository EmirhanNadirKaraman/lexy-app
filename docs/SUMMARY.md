...
| `word_seed_service.py` | German catalog backfill from `data/final_result.txt` column 0 (N5a step 1). Article-strips, keeps clean single tokens, inserts only surfaces absent by `lower(word)` with `pos=''` (a real POS would fork rows — `pos` is in the unique key). Never updates existing rows, never touches `phrase_table`. Driven by `scripts/backfill_word_catalog.py`. |
